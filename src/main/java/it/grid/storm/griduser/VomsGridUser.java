...
/**
 * Encapsulates user Grid credentials access, and maps those to a local user account. Has methods to
 * extract the permanent identifier (subject DN), VO and VOMS group/role membership from the X.509
 * certificate / GSI proxy that the user presented to StoRM. Will also invoke LCMAPS library to map
 * the Grid credentials to a local user account.
 *
 * @todo implement a flyweight pattern, so that we don't have 1'000 different GridUser objects for
 *     1'000 requests from the same user...
 */
class VomsGridUser extends GridUser implements Serializable {
  ...
  VomsGridUser(MapperInterface mapper, String distinguishedName, String proxy, FQAN[] fqansArray)
    ...
    if (fqansArray == null || fqansArray.length == 0) {
      throw new IllegalArgumentException(
          "Unable to create VomsGridUser. Inavlid fqansArray argument: " + fqansArray);
    }
    this.setFqans(fqansArray);
  }

  VomsGridUser(MapperInterface mapper, String distinguishedName, FQAN[] fqansArray)
      throws IllegalArgumentException {
   ...
   *
   * <p>If the explicit constructor {@link VomsGridUser(String, Fqan[], String)} was used, then this
   * flag will be true if the <code>Fqan[]</code> parameter was not null in the constructor
   * invocation.
   *
   * @return <code>true</code> if any VOMS attributes are stored in this object.
   */
  ...
  /**
   * Print a string representation of this object, in the form <code>GridUser:"</code><i>subject
   * DN</i><code>"</code>.
   */
  ...
  /**
   * Return true if other is a VomsGridUser with the same String representation, that is: - same DN,
   * and - same FQANs
   */
  @Override
