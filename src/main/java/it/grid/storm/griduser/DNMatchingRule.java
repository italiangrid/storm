...
public class DNMatchingRule {
	...
	private static final Logger log = LoggerFactory
		.getLogger(DNMatchingRule.class);

	private enum DNFields {
		COUNTRY("C"), ORGANIZATION("O"), ORGANIZATIONALUNIT("OU"), LOCALITY("L"), COMMONNAME(
			"CN"), DOMAINCOMPONENT("DC"), UNKNOWN("");
	...
	public static DNMatchingRule buildMatchAllDNMatchingRule() {
		...
		return new DNMatchingRule(ADMIT_ALL, ADMIT_ALL, ADMIT_ALL, ADMIT_ALL,
			ADMIT_ALL, ADMIT_ALL);
	}
	...
	public DNMatchingRule(String regularExpressionRule) {
		...
		if (!(regularExpressionRule == null
			|| regularExpressionRule.trim().equals("*") || regularExpressionRule
			.trim().equals(".*"))) {
	...
	public boolean match(DistinguishedName principalDN)
		...
		if (principalDN == null) {
			throw new IllegalArgumentException(
				"Unable to perform rule matching. Received null argument: principalDN="
					+ principalDN);
		}
		if (this.isMatchAll()) {
			return true;
		...
		if (localityName != null) {
			CharSequence locality = localityName
				.subSequence(0, localityName.length());
		...
		// OU ArrayList
		ArrayList<String> organizationalUnitNames = principalDN
