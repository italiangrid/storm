...
public class LcmapsJNAMapper implements MapperInterface
{
	...
	public LocalUser map(String dn, String[] fqans)
		...
		synchronized(LcmapsJNAMapper.lock)
		{
            ...
            try
            {
                retVal = LcmapsPoolindexInterface.INSTANCE.lcmaps_return_account_without_gsi(dn, fqans, numFqans, 0, account);
            }
            catch (LastErrorException e)
            {
                log.error("Unable to map user dn <" + dn + "> fqans <" + ArrayUtils.toString(fqans) + "> . LastErrorException: "
