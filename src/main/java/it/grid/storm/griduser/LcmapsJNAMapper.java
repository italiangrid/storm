...
public class LcmapsJNAMapper implements MapperInterface {
  ...
  public LocalUser map(String dn, String[] fqans) throws CannotMapUserException {
    ...
    synchronized (LcmapsJNAMapper.lock) {
      ...
      int retVal =
          LcmapsInterface.INSTANCE.lcmaps_init_and_logfile(lcmapsLogFile, null, LCMAPS_LOG_TYPE);
      if (retVal != 0) {
      ...
      try {
        retVal =
            LcmapsPoolindexInterface.INSTANCE.lcmaps_return_account_without_gsi(
                dn, fqans, numFqans, 0, account);
      } catch (LastErrorException e) {
        log.error(
            "Unable to map user dn <{}> fqans <{}>. Error: {}. Error code: {}",
            dn,
      ...
      if (retVal != 0) {
        log.error(
            "Unable to map user dn <{}> fqans <{}>. Retval: {}",
            dn,
            ArrayUtils.toString(fqans),
            retVal);
        throw new CannotMapUserException(
            "Unable to map user dn <"
                + dn
                + "> fqans <"
                + ArrayUtils.toString(fqans)
                + "> . Return value is "
      ...
      if (account.npgid < 0 || account.nsgid < 0) {
        log.error(
            "Negative primary or secondary gid array size. npgid: {} nsgid: {}",
            account.npgid,
            account.nsgid);
