...
public class LcmapsJNAMapper implements MapperInterface {
  ...
  public LocalUser map(String dn, String[] fqans) throws CannotMapUserException {
    ...
    synchronized (LcmapsJNAMapper.lock) {
      ...
      try {
        retVal = LcmapsPoolindexInterface.INSTANCE.lcmaps_return_account_without_gsi(dn, fqans,
            numFqans, 0, account);
      ...
      if (retVal != 0) {
        log.error("Unable to map user dn <{}> fqans <{}>. Retval: {}", dn,
            ArrayUtils.toString(fqans), retVal);
