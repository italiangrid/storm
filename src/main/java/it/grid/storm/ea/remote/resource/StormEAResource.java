...
public class StormEAResource {
  ...
  public String getAdler32Checksum(@PathParam("filePath") String filePath)
    ...
    log.info(
        "Getting {} checksum for file {}", Constants.ADLER_32, parameters.getFilePathDecoded());
  ...
  @Path("/" + Constants.ADLER_32)
  @Produces("text/plain")
  public void setAdler32Checksum(
      @PathParam("filePath") String filePath,
      @QueryParam(Constants.CHECKSUM_VALUE_KEY) String checksum)
      throws WebApplicationException {

    RequestParameters parameters =
        new RequestParameters.Builder(filePath).checksum(checksum).build();

    log.info(
        "Setting {} checksum for file {} with value {}",
        Constants.ADLER_32,
    ...
    try {
      StormEA.setChecksum(
          parameters.getFilePathDecoded(), parameters.getChecksumDecoded(), ADLER32);
    } catch (ExtendedAttributesException e) {
      log.error(e.getMessage(), e);
      throw new WebApplicationException(
          Response.status(INTERNAL_SERVER_ERROR)
