...
public class StormEA {
	...
	private static final ExtendedAttributes ea = ExtendedAttributesFactory.getExtendedAttributes();
	...
	public static void removeChecksum(String fileName) {
		...
		if (!ea.hasXAttr(fileName, EA_CHECKSUM)){
			log.info("Cannot remove '"
					+ EA_CHECKSUM
					+ "' EA because the attribute was not found: "
					+ fileName);
			return;
		}
	...
	public static void setChecksum(String fileName, String checksum,
		...
		ea.setXAttr(fileName, 
				EA_CHECKSUM + algorithm.toLowerCase(),
				checksum);
	}
	 ...
	 *            expiration time of the pin expressed as
		...
		try {
			...
			if (log.isDebugEnabled()) {
				if (existingPinValueInSEC == -1) {
					log.debug("Added the Pinned EA to '"
							+ fileName
							+ "' with expiration: "
							+ formatter.format(new Date(
									existingPinValueInSEC * 1000)));
				} else {
					log.debug("Updated the Pinned EA to '"
							+ fileName
							+ "' with expiration: "
							+ formatter.format(new Date(
									existingPinValueInSEC * 1000)));
