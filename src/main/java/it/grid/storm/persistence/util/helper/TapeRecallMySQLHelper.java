...
public class TapeRecallMySQLHelper extends SQLHelper {
  ...
  public PreparedStatement getQueryInsertTask(Connection conn, TapeRecallTO recallTask) {
    ...
    try {
      ...
      prepStat.setTimestamp(
          idx++, new java.sql.Timestamp(recallTask.getDeferredRecallInstant().getTime()));
      prepStat.setTimestamp(
          idx++, new java.sql.Timestamp(recallTask.getInsertionInstant().getTime()));
  ...
  public PreparedStatement getQueryGetGroupTaskIds(Connection conn, UUID taskId)
    ...
    str =
        "SELECT DISTINCT "
            ...
            + COL_STATUS
            + " , "
            + COL_IN_PROGRESS_DATE
            + " , "
            + COL_FINAL_STATUS_DATE
            + " FROM "
            + TABLE_NAME
            + " WHERE "
            + COL_TASK_ID
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryGetTakeoverTasksWithDoubles(
      Connection conn, int numberOfTasks, String voName) throws SQLException {
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryUpdateTasksStatus(
      Connection conn,
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryUpdateGroupTaskStatus(
      Connection conn, UUID groupTaskId, int status, String timestampColumn, Date timestamp)
      throws IllegalArgumentException, SQLException {
   ...
   * @throws SQLException
   */
  public PreparedStatement getQuerySetGroupTaskRetryValue(
      Connection conn, UUID groupTaskId, int value) throws SQLException {
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryDeleteCompletedTasks(
      Connection con, long expirationTime, int maxNumTasks) throws SQLException {
