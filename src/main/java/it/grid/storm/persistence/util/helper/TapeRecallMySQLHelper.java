...
import it.grid.storm.persistence.model.SQLHelper;
...
public class TapeRecallMySQLHelper extends SQLHelper {
  ...
  static {
    ...
    QUERY_DELETE_N_OLD_AND_COMPLETED_TASKS =
        "DELETE FROM tape_recall WHERE status<>1 AND status<>2 "
            + "AND timeStamp <= DATE_SUB(CURRENT_TIMESTAMP(), INTERVAL ? SECOND) " + "LIMIT ?";
  ...
  private static boolean validTimestampColumnName(String columnName) {
    ...
    return COL_DATE.equals(columnName) || COL_IN_PROGRESS_DATE.equals(columnName)
  ...
  public PreparedStatement getQueryInsertTask(Connection conn, TapeRecallTO recallTask) {
    ...
    String query = "INSERT INTO " + TABLE_NAME + " (" + COL_TASK_ID + ", " + COL_REQUEST_TOKEN
        + ", " + COL_REQUEST_TYPE + ", " + COL_FILE_NAME + ", " + COL_PIN_LIFETIME + ", "
        + COL_STATUS + ", " + COL_VO_NAME + ", " + COL_USER_ID + ", " + COL_RETRY_ATTEMPT + ", "
        + COL_DEFERRED_STARTTIME + ", " + COL_DATE + ", " + COL_GROUP_TASK_ID
        + ") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    try {
      ...
      prepStat.setTimestamp(idx++,
          new java.sql.Timestamp(recallTask.getDeferredRecallInstant().getTime()));
      prepStat.setTimestamp(idx++,
          new java.sql.Timestamp(recallTask.getInsertionInstant().getTime()));
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryGetTask(Connection conn, UUID taskId, String requestToken)
      throws SQLException {

    String str = null;
    PreparedStatement preparedStatement = null;

    str = "SELECT * FROM " + TABLE_NAME + " WHERE " + COL_TASK_ID + "=?" + " AND "
        + COL_REQUEST_TOKEN + "=?";
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryGetGroupTasks(Connection conn, UUID groupTaskId)
      throws SQLException {
  ...
  public PreparedStatement getQueryGetGroupTaskIds(Connection conn, UUID taskId)
    ...
    str = "SELECT DISTINCT " + COL_GROUP_TASK_ID + " , " + COL_STATUS + " , " + COL_IN_PROGRESS_DATE
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryGetGroupTaskIds(Connection conn, UUID taskId, int[] statuses)
      throws SQLException {
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryNumberQueued(Connection conn) throws SQLException {

    String str = null;
    PreparedStatement preparedStatement = null;

    str = "SELECT COUNT(DISTINCT " + COL_GROUP_TASK_ID + ") FROM " + TABLE_NAME + " WHERE "
        + COL_STATUS + "=?";
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryReadyForTakeOver(Connection conn) throws SQLException {

    String str = null;
    PreparedStatement preparedStatement = null;

    str = "SELECT COUNT(DISTINCT " + COL_GROUP_TASK_ID + ") FROM " + TABLE_NAME + " WHERE "
        + COL_STATUS + "=?" + " AND " + COL_DEFERRED_STARTTIME + "<=NOW()";
  ...
  public PreparedStatement getQueryReadyForTakeOver(Connection conn, String voName)
    ...
    str = "SELECT COUNT(DISTINCT " + COL_GROUP_TASK_ID + ") FROM " + TABLE_NAME + " WHERE "
        + COL_STATUS + "=?" + " AND " + COL_VO_NAME + "=?" + " AND " + COL_DEFERRED_STARTTIME
        + "<=NOW()";
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryNumberInProgress(Connection conn) throws SQLException {
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryGetTakeoverTasksWithDoubles(Connection conn, int numberOfTasks)
      throws SQLException {

    String str = null;
    PreparedStatement preparedStatement = null;

    str = "SELECT * FROM " + TABLE_NAME + " WHERE " + COL_STATUS + "=?" + " AND "
        + COL_DEFERRED_STARTTIME + "<=NOW() ORDER BY " + COL_DEFERRED_STARTTIME + " LIMIT ?";
  ...
  public PreparedStatement getQueryGetTakeoverTasksWithDoubles(Connection conn, int numberOfTasks,
    ...
    str = "SELECT * FROM " + TABLE_NAME + " WHERE " + COL_STATUS + "=?" + " AND " + COL_VO_NAME
  ...
  /**
   * Creates the query string for looking up all the information related to in progress tasks in the
   * recall database.
   * 
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryGetAllTasksInProgress(Connection conn, int numberOfTasks)
      throws SQLException {

    String str = null;
    PreparedStatement preparedStatement = null;

    str = "SELECT * FROM " + TABLE_NAME + " WHERE " + COL_STATUS + "=?" + " ORDER BY "
        + COL_IN_PROGRESS_DATE + " ASC LIMIT ?";
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryUpdateTasksStatus(Connection conn, List<TapeRecallTO> taskList,
    ...
    if (validTimestampColumnName(timestampColumn)) {
      str = "UPDATE " + TABLE_NAME + " SET " + COL_STATUS + "=?" + " , " + timestampColumn + "=?"
      ...
      preparedStatement.setString(3, taskList.get(0).getGroupTaskId().toString());
   ...
   * @throws SQLException
   */
  public PreparedStatement getQueryUpdateGroupTaskStatus(Connection conn, UUID groupTaskId,
      int status, String timestampColumn, Date timestamp)
      throws IllegalArgumentException, SQLException {
    ...
    if (validTimestampColumnName(timestampColumn)) {
      str = "UPDATE " + TABLE_NAME + " SET " + COL_STATUS + "=?" + " , " + timestampColumn + "=?"
          + " WHERE " + COL_GROUP_TASK_ID + "=?" + " AND " + COL_STATUS + "!=?";
   ...
   * @throws SQLException
   */
  public PreparedStatement getQuerySetGroupTaskStatus(Connection conn, UUID groupTaskId, int status)
      throws SQLException {

    String str = null;
    PreparedStatement preparedStatement = null;

    str = "UPDATE " + TABLE_NAME + " SET " + COL_STATUS + "=?" + " WHERE " + COL_GROUP_TASK_ID
        + "=?" + " AND " + COL_STATUS + "!=?";
   ...
   * @throws SQLException
   */
  public PreparedStatement getQuerySetGroupTaskRetryValue(Connection conn, UUID groupTaskId,
      int value) throws SQLException {

    String str = null;
    PreparedStatement preparedStatement = null;

    str = "UPDATE " + TABLE_NAME + " SET " + COL_RETRY_ATTEMPT + "=?" + " WHERE "
        + COL_GROUP_TASK_ID + "=?";
