...
/**
 * find = con.prepareStatement( "SELECT storm_get_filereq.rowid, storm_req.r_token,
 * storm_get_filereq.from_surl, storm_get_filereq.lifetime, storm_get_filereq.s_token,
 * storm_get_filereq.flags, storm_req.protocol, storm_get_filereq.actual_size,
 * storm_get_filereq.status, storm_get_filereq.errstring, storm_get_filereq.pfn FROM
 * storm_get_filereq, storm_req WHERE storm_get_filereq.r_token=storm_req.r_token AND
 * storm_get_filereq.r_token=?" );
 */
public class StorageSpaceDAOMySql extends AbstractDAO implements StorageSpaceDAO {
  ...
  public void addStorageSpace(StorageSpaceTO ss) throws DataAccessException {
    ...
    try {
      ...
      if (res <= 0) {
        log.error("No row inserted for statement : {}", prepStatement.toString());
  ...
  public Collection<StorageSpaceTO> getStorageSpaceByUnavailableUsedSpace(long unavailableSizeValue)
    ...
    try {
      prepStatement = helper.selectByUnavailableUsedSpaceSizeQuery(conn, unavailableSizeValue);
      log.debug("SELECT query = {}", prepStatement.toString());
  ...
  public Collection<StorageSpaceTO> getStorageSpaceByPreviousLastUpdate(Date lastUpdateTimestamp)
    ...
    try {
      prepStatement =
          helper.selectByPreviousOrNullLastUpdateQuery(conn, lastUpdateTimestamp.getTime());
      log.debug("SELECT query = {}", prepStatement.toString());
  ...
  public void removeStorageSpace(GridUserInterface user, String spaceToken)
    ...
    try {
      ...
      if (res <= 0) {
        log.error(
            "Error removing Storage Space with token = {} for " + "user {} not found",
            spaceToken,
            user.getDn());

        throw new DataAccessException(
            "Storage Space with token = '"
                + spaceToken
                + "' for user '"
                + user.getDn()
                + "' not found!");
  ...
  public void removeStorageSpace(String spaceToken) throws DataAccessException {
    ...
    try {
      ...
      if (res <= 0) {
        log.error("Error removing Storage Space with token = {}. Space not found", spaceToken);
  ...
  public void updateStorageSpace(StorageSpaceTO ssTO) throws DataAccessException {
    ...
    try {
      ...
      if (res != 1) {
        if (res < 1) {
          log.error("No storage space rows updated by query : {}", prepStatement.toString());
        } else {
          log.warn(
  ...
  public void updateStorageSpaceFreeSpace(StorageSpaceTO ssTO) throws DataAccessException {
    ...
    try {
      prepStatement =
          helper.updateFreeSpaceByTokenQuery(conn, ssTO.getSpaceToken(), freeSpace, new Date());
  ...
  public Collection<StorageSpaceTO> getExpired(long currentTimeInSecond)
    ...
    try {
      ...
      if (res.first() == false) {
        log.debug("No rows found for query : {}", prepStatement.toString());
        throw new DataAccessException(
            "No storage space expired found at time " + currentTimeInSecond);
      } else {
        // Fetch each row from the result set
        do {
          ssTO = helper.makeStorageSpaceTO(res);
