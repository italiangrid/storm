 ...
 * 
 * This class is intended to be used by srmPutDone, where only a limited amount of information is
...
public class ReducedPtPChunkData {
  ...
  public ReducedPtPChunkData(TSURL toSURL, TReturnStatus status, TFileStorageType fileStorageType,
      TLifeTimeInSeconds fileLifetime) throws InvalidReducedPtPChunkDataAttributesException {

    if (status == null || toSURL == null || fileStorageType == null || fileLifetime == null) {
      throw new InvalidReducedPtPChunkDataAttributesException(toSURL, status, fileStorageType,
          fileLifetime);
    }
    this.toSURL = toSURL;
    this.status = status;
  ...
  public boolean equals(Object o) {
    ...
    return (primaryKey == cd.primaryKey) && toSURL.equals(cd.toSURL) && status.equals(cd.status)
        && fileStorageType.equals(cd.fileStorageType) && fileLifetime.equals(cd.fileLifetime);
  }
