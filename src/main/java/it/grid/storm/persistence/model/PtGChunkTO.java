...
/**
 * This class represents a PrepareToGetChunkData, that is part of a multi-file PrepareToGet SRM
 * request. It contains data about: the requestToken, the fromSURL and the storageSystemInfo for
 * that SURL, the requested lifeTime of pinning, the requested fileStorageType and any available
 * spaceToken, the TDirOption which explains whether the requested SURL is a directory and if it
 * must be recursed at all levels, as well as the desired number of levels to recurse, the desired
 * transferProtocols in order of preference, the fileSize, the estimatedTimeOnQueue, the
...
public class PtGChunkTO {
  ...
  public PtGChunkTO(TRequestToken requestToken, TSURL fromSURL,
    ...
    boolean ok = requestToken != null && fromSURL != null && storageSystemInfo != null
        && lifeTime != null && fileStorageType != null && spaceToken != null && dirOption != null
    ...
    if (!ok) {
      throw new InvalidPtGChunkDataAttributesException(requestToken, fromSURL, storageSystemInfo,
          lifeTime, fileStorageType, spaceToken, dirOption, transferProtocols, fileSize, status,
          estimatedWaitTimeOnQueue, estimatedProcessingTime, transferURL, remainingPinTime);
    }
  ...
  /**
   * Method that sets the status of this request to SRM_DONE; it needs the explanation String which
   * describes the situation in greater detail; if a null is passed, then an empty String is used as
   * explanation.
   */
  ...
  /**
   * Method that sets the status of this request to SRM_REQUEST_INPROGRESS; it needs the explanation
   * String which describes the situation in greater detail; if a null is passed, then an empty
   * String is used as explanation.
   */
  ...
  /**
   * Method that sets the status of this request to SRM_FATAL_INTERNAL_ERROR; it needs the
   * explanation String which describes the situation in greater detail; if a null is passed, then
   * an empty String is used as explanation.
   */
  ...
  public boolean equals(Object o) {
    ...
    return requestToken.equals(cd.requestToken) && fromSURL.equals(cd.fromSURL)
        && storageSystemInfo.equals(cd.storageSystemInfo) && lifeTime.equals(cd.lifeTime)
        && fileStorageType.equals(cd.fileStorageType) && spaceToken.equals(cd.spaceToken)
        && dirOption.equals(cd.dirOption) && transferProtocols.equals(cd.transferProtocols)
        ...
        && transferURL.equals(cd.transferURL) && remainingPinTime.equals(cd.remainingPinTime);
  }

  /**
   * Method used to set the size of the file corresponding to the requested SURL. If the supplied
   * TSizeInByte is null, the nothing gets set!
   */
  ...
  /**
   * Method used to set the estimated time that the chunk will spend on the queue. If the supplied
   * TLifeTimeInSeconds is null, then nothing gets set!
   */
  ...
  /**
   * Method used to set the estimated time the processing will take. If the supplied
   * TLifeTimeInSeconds is null, then nothing gets set!
   */
  ...
  /**
   * Method used to set the transferURL associated to the SURL of this chunk. If TTURL is null, then
   * nothing gets set!
   */
  ...
  /**
   * Method used in the mechanism for suspending and resuming a request. To be implemented! For now
   * it always returns 0.
   */
