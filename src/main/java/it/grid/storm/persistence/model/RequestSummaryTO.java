...
/**
 * This class represents the SummaryData associated with the SRM request, that
 * is it contains info about: TRequestToken, TRequsetType, total files in this
 * request, number of files in queue, number of files progressing, number of
 * files finished, and whether the request is currently suspended.
 * 
...
public class RequestSummaryTO {
	...
	private int totalFilesInThisRequest = 0; // total number of files in SRM
																						// request
	private int numOfQueuedRequests = 0; // number of files in SRM request that
																				// are in queue
	private int numOfProgressingRequests = 0; // number of files in SRM request
																						// that are still in progress
	private int numFinished = 0; // number of files in SRM request whose
																// processing has completed
	private boolean isSuspended = false; // flag that indicates whether the SRM
																				// request is suspended

	public RequestSummaryTO(TRequestToken requestToken, TRequestType requestType,
		...
		boolean ok = requestToken != null && requestType != null
			&& totalFilesInThisRequest >= 0 && numOfQueuedRequests >= 0
			&& numOfProgressingRequests >= 0 && numFinished >= 0;
		if (!ok)
			throw new InvalidRequestSummaryDataAttributesException(requestToken,
				requestType, totalFilesInThisRequest, numOfQueuedRequests,
				numOfProgressingRequests, numFinished);
	...
	/**
	 * Method used to increment the counter for the number of progressing
	 * requests.
	 */
	...
	/**
	 * Method used to decrement the counter for the number of progressing
	 * requests.
	 */
	...
	/**
	 * Method used to increment the counter for the number of total files in the
	 * request.
	 */
	...
	/**
	 * Method used to decrement the counter fot the number of total files in this
	 * request.
	 */
	...
	/**
	 * Method used to set the SRM flag that signals the processing of the request
	 * this RequestSummaryData applies to, is suspended.
	 */
	...
	public boolean equals(Object o) {
		...
		return requestToken.equals(rsd.requestToken)
			...
			&& (numFinished == rsd.numFinished) && (isSuspended == rsd.isSuspended);
	}
}
