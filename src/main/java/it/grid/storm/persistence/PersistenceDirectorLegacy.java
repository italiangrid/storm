...
public class PersistenceDirectorLegacy {

	private static final Logger log = LoggerFactory.getLogger("persistence");
	
	static String prop = "/etc/storm/backend-server/storm.properties";
	
	private static ConfigReader cr = new ConfigReader(prop, 0); 
	
	private static Configuration config = Configuration.getInstance();
	private static String dbVendor;
	private static DataBaseStrategy dbMan;
	private static MySqlDAOFactoryLegacy daoFactory;
	...
	private static MySqlDAOFactoryLegacy initializeFactory() {

		if (dbVendor.equalsIgnoreCase("MySql")) {
			return MySqlDAOFactoryLegacy.getInstance();
		} 
	...
	private static DataSourceConnectionFactory connectToDateSource() {
		...
		if (poolMode) {
			try {
				DBConnectionPoolLegacy.initPool(dbMan, maxActive, maxWait);
				result = DBConnectionPoolLegacy.getPoolInstance();
	...
	public static MySqlDAOFactoryLegacy getDAOFactory() {
		return daoFactory;
	}
