...
public class PtGChunkCatalog {
   ...
   *
   * <p>Only fileSize, StatusCode, errString and transferURL are updated. Likewise for the request
   * pinLifetime.
   */
  ...
  /**
   * Refresh method. THIS IS A WORK IN PROGRESS!!!! This method have to synch the ChunkData
   * information with the database status intended as the status code and the TURL
   *
   * @param auxTO
  ...
  public synchronized PtGPersistentChunkData refreshStatus(PtGPersistentChunkData inputChunk) {
    ...
    if (chunkDataTO == null) {
      log.warn(
          "PtG CHUNK CATALOG! Empty TO found in persistence for specified " + "request: {}",
    ...
    try {
      ...
      log.info(
          "PtGChunkCatalog (FALSE-ERROR-in-abort-refresh-status?):"
   ...
   *
   * <p>If any of the data associated to the TRequestToken is not well formed and so does not allow
   * a PtGChunkData Object to be created, then that part of the request is dropped and gets logged,
   * and the processing continues with the next part. All valid chunks get returned: the others get
   * dropped.
   *
   * <p>If there are no chunks to process then an empty Collection is returned, and a messagge gets
   * logged.
   */
  public synchronized Collection<PtGPersistentChunkData> lookup(TRequestToken rt) {
    ...
    if (chunkTOs.isEmpty()) {
      log.warn(
          "PtG CHUNK CATALOG! No chunks found in persistence for " + "specified request: {}", rt);
      return list;
    }
  ...
  private PtGPersistentChunkData makeOne(PtGChunkDataTO chunkDataTO, TRequestToken rt) {
    ...
    try {
      ...
      if (pinLifeTime > max) {
        log.warn(
            "PinLifeTime is greater than the max value allowed."
                + " Drop the value to the max = {} seconds",
            max);
        pinLifeTime = max;
      }
    ...
    try {
      dirOption =
          new TDirOption(
              chunkDataTO.dirOption(), chunkDataTO.allLevelRecursive(), chunkDataTO.numLevel());
    ...
    try {
      if (chunkDataTO.vomsAttributes() != null && !chunkDataTO.vomsAttributes().trim().equals("")) {
        gridUser =
            GridUserManager.makeVOMSGridUser(
                chunkDataTO.clientDN(), chunkDataTO.vomsAttributesArray());
      } else {
      ...
      log.error(
          "Unexpected error on voms grid user creation." + " IllegalArgumentException: {}",
          e.getMessage(),
          e);
    }
    // transferURL
    ...
    try {
      ...
      log.warn(
          "PtG CHUNK CATALOG! Retrieved malformed PtG chunk data from "
              + "persistence. Dropping chunk from request {}",
          rt);
   ...
   */
  private ReducedPtGChunkDataTO completeTO(
      PtGChunkDataTO chunkTO, final PtGPersistentChunkData chunk)
      throws InvalidReducedPtGChunkDataAttributesException {
  ...
  /**
   * Checks if the received ReducedPtGChunkDataTO contains the fields not set by the front end but
   * required
   *
   ...
   * TRequestToken.
   *
   * <p>If any of the data retrieved for a given chunk is not well formed and so does not allow a
   * ReducedPtGChunkData Object to be created, then that chunk is dropped and gets logged, while
   * processing continues with the next one. All valid chunks get returned: the others get dropped.
   *
   * <p>If there are no chunks associated to the given TRequestToken, then an empty Collection is
  ...
  public Collection<ReducedChunkData> lookupReducedPtGChunkData(
    ...
    Collection<ReducedPtGChunkDataTO> chunkDataTOCollection =
        dao.findReduced(requestToken, surlsUniqueIDs, surlsArray);
  ...
  public Collection<PtGPersistentChunkData> lookupPtGChunkData(
    ...
    Collection<PtGChunkDataTO> chunkDataTOCollection =
        dao.find(surlsUniqueIDs, surlsArray, user.getDn());
    log.debug("PtG CHUNK CATALOG: retrieved data {}", chunkDataTOCollection);
  ...
  public Collection<PtGPersistentChunkData> lookupPtGChunkData(List<TSURL> surls) {
    ...
    Collection<PtGChunkDataTO> chunkDataTOCollection = dao.find(surlsUniqueIDs, surlsArray);
    log.debug("PtG CHUNK CATALOG: retrieved data {}", chunkDataTOCollection);
    return buildChunkDataList(chunkDataTOCollection);
  }
  ...
  /**
   * Method that returns a Collection of ReducedPtGChunkData Objects matching the supplied GridUser
   * and Collection of TSURLs. If any of the data retrieved for a given chunk is not well formed and
   * so does not allow a ReducedPtGChunkData Object to be created, then that chunk is dropped and
   * gets logged, while processing continues with the next one. All valid chunks get returned: the
   * others get dropped. If there are no chunks associated to the given GridUser and Collection of
   * TSURLs, then an empty Collection is returned and a message gets logged.
   */
  public synchronized Collection<ReducedChunkData> lookupReducedPtGChunkData(
    ...
    Collection<ReducedPtGChunkDataTO> chunkDataTOCollection =
        dao.findReduced(gu.getDn(), surlsUniqueIDs, surls);
    log.debug("PtG CHUNK CATALOG: retrieved data {}", chunkDataTOCollection);
    return buildReducedChunkDataList(chunkDataTOCollection);
  }
  ...
  private ReducedPtGChunkData makeOneReduced(ReducedPtGChunkDataTO reducedChunkDataTO) {
    ...
    try {
      ...
      log.warn(
          "PtG CHUNK CATALOG! Retrieved malformed Reduced PtG chunk "
   ...
   *
   * <p>This method is intended to be used by a recursive PtG request: the parent request supplies a
   ...
   *
   * <p>So this method does _not_ add a new SRM prepare_to_get request into the DB!
   *
   * <p>The only children data written into the DB are: sourceSURL, TDirOption, statusCode and
   * explanation.
   *
   * <p>In case of any error the operation does not proceed, but no Exception is thrown! Proper
  ...
  /**
   * Method used to add into Persistence a new entry. The supplied PtGChunkData gets the primary key
   * changed to the value assigned in the Persistence. The method requires the GridUser to whom
   * associate the added request.
   *
   * <p>This method is intended to be used by an srmCopy request in push mode which implies a local
   * srmPtG. The only fields from PtGChunkData that are considered are: the requestToken, the
   * sourceSURL, the pinLifetime, the dirOption, the protocolList, the status and error string.
   *
   * <p>So this method _adds_ a new SRM prepare_to_get request into the DB!
   *
   * <p>In case of any error the operation does not proceed, but no Exception is thrown! The
   * underlaying DAO logs proper error messagges.
   */
  ...
  /**
   * Method used to establish if in Persistence there is a PtGChunkData working on the supplied
   * SURL, and whose state is SRM_FILE_PINNED, in which case true is returned. In case none are
   * found or there is any problem, false is returned. This method is intended to be used by srmMv.
   */
   ...
   * SRM_RELEASED. Chunks in any other starting state are not transited. In case of any error
   * nothing is done, but proper error messages get logged by the underlaying DAO.
   */
  public synchronized void transitSRM_FILE_PINNEDtoSRM_RELEASED(
  ...
  /**
   * Method used to force transition to SRM_RELEASED from SRM_FILE_PINNED, of all PtG Requests whose
   * pinLifetime has expired and the state still has not been changed (a user forgot to run
   * srmReleaseFiles)!
   */
  ...
  public void updateStatus(
      TRequestToken requestToken, TSURL surl, TStatusCode statusCode, String explanation) {

    dao.updateStatus(
        requestToken,
        new int[] {surl.uniqueId()},
        new String[] {surl.rawSurl()},
        statusCode,
        explanation);
  }

  public void updateFromPreviousStatus(
      TSURL surl, TStatusCode expectedStatusCode, TStatusCode newStatusCode, String explanation) {

    dao.updateStatusOnMatchingStatus(
        new int[] {surl.uniqueId()},
        new String[] {surl.rawSurl()},
        expectedStatusCode,
        newStatusCode,
  ...
  public void updateFromPreviousStatus(
    ...
    dao.updateStatusOnMatchingStatus(
        requestToken, surlsUniqueIDs, surls, expectedStatusCode, newStatusCode);
  }
}
