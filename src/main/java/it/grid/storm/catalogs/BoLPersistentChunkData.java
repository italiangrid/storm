 ...
 * @date Aug 2009
 */
public class BoLPersistentChunkData extends BoLData implements PersistentChunkData {
    private static final Logger log = LoggerFactory.getLogger(BoLPersistentChunkData.class);
    ...
    public BoLPersistentChunkData(TRequestToken requestToken, TSURL fromSURL, TLifeTimeInSeconds lifeTime,
            TDirOption dirOption, TURLPrefix desiredProtocols, TSizeInBytes fileSize, TReturnStatus status,
            TTURL transferURL, int deferredStartTime) throws InvalidBoLPersistentChunkDataAttributesException, InvalidFileTransferDataAttributesException, InvalidBoLDataAttributesException, InvalidSurlRequestDataAttributesException {
        
        super(fromSURL, lifeTime, dirOption, desiredProtocols, fileSize, status, transferURL, deferredStartTime);
        if (requestToken == null)
        {
            throw new InvalidBoLPersistentChunkDataAttributesException(requestToken, fromSURL, lifeTime, dirOption, desiredProtocols, fileSize, status, transferURL);
        }
