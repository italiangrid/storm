...
public class BoLChunkCatalog {
   ...
   * a BoLChunkData Object to be created, then that part of the request is dropped and gets logged,
   * and the processing continues with the next part. All valid chunks get returned: the others get
   * dropped.
   *
   * <p>If there are no chunks to process then an empty Collection is returned, and a message gets
   * logged.
   */
  public synchronized Collection<BoLPersistentChunkData> lookup(TRequestToken rt) {
    ...
    if (chunkCollection.isEmpty()) {
      log.warn(
          "BoL CHUNK CATALOG! No chunks found in persistence for specified " + "request: {}", rt);
      return list;
    }
  ...
  private BoLPersistentChunkData makeOne(BoLChunkDataTO auxTO, TRequestToken rt) {
    ...
    try {
      ...
      log.warn(
          "BoL CHUNK CATALOG! Retrieved malformed BoL "
              + "chunk data from persistence. Dropping chunk from request {}",
          rt);
   ...
   */
  private ReducedBoLChunkDataTO completeTO(
      BoLChunkDataTO chunkTO, final BoLPersistentChunkData chunk)
      throws InvalidReducedBoLChunkDataAttributesException {
  ...
  /**
   * Checks if the received ReducedBoLChunkDataTO contains the fields not set by the front end but
   * required
   *
   ...
   *
   * @param auxTO
   * @param BoLPersistentChunkData inputChunk
  ...
  public synchronized BoLPersistentChunkData refreshStatus(BoLPersistentChunkData inputChunk) {
    ...
    if (auxTO == null) {
      log.warn(
          "BoL CHUNK CATALOG! Empty TO found in persistence for specified " + "request: {}",
  ...
  /**
   * Method that returns a Collection of ReducedBoLChunkData Objects matching the supplied GridUser
   * and Collection of TSURLs.
   *
   * <p>If any of the data retrieved for a given chunk is not well formed and so does not allow a
   * ReducedBoLChunkData Object to be created, then that chunk is dropped and gets logged, while
   * processing continues with the next one. All valid chunks get returned: the others get dropped.
   *
   * <p>If there are no chunks associated to the given GridUser and Collection of TSURLs, then an
   * empty Collection is returned and a message gets logged.
   */
  public synchronized Collection<ReducedBoLChunkData> lookupReducedBoLChunkData(
  ...
  private ReducedBoLChunkData makeOneReduced(ReducedBoLChunkDataTO reducedChunkDataTO) {
    ...
    try {
      ...
      log.warn(
          "BoL CHUNK CATALOG! Retrieved malformed "
   ...
   *
   * <p>This method is intended to be used by a recursive BoL request: the parent request supplies a
   ...
   */
  public synchronized void transitSRM_SUCCESStoSRM_RELEASED(
   ...
   *
   * <p>The TURL of those requests will automatically be set to empty. Notice that both
   * removeAllJit(SURL) and removeVolatile(SURL) are automatically invoked on PinnedFilesCatalog, to
   * remove any entry and corresponding physical ACLs.
   *
   * <p>Beware, that the chunks may be part of requests that have finished, or that still have not
  ...
  /**
   * Method used to force transition to SRM_RELEASED from SRM_FILE_PINNED, of all BoL Requests whose
