...
public class RequestSummaryCatalog {
  ...
  private RequestSummaryCatalog() {
    ...
    clock.schedule(
        new RequestsGarbageCollector(clock, config.getRequestPurgerPeriod() * 1000),
   ...
   *
   * <p>If no new request is found, an empty Collection is returned. if a request is malformed, then
  ...
  public synchronized Collection<RequestSummaryData> fetchNewRequests(int capacity) {
    ...
    for (RequestSummaryDataTO auxTO : c) {
      ...
      try {
        ...
        log.error(
            "REQUEST SUMMARY CATALOG: Failure while performing makeOne "
      ...
      if (aux != null) {
        log.debug(
            "REQUEST SUMMARY CATALOG: {} associated to {} included " + "for processing",
            aux.requestToken(),
            aux.gridUser().getDn());
        list.add(aux);
      }
    ...
    if (ret < fetched) {
      log.warn(
          "REQUEST SUMMARY CATALOG: including {} requests for processing, "
              + "since the dropped ones were malformed!",
          ret);
    } else {
   ...
   * chunk cannot be created, an error messagge gets logged and an attempt is made to signal in the
  ...
  private RequestSummaryData makeOne(RequestSummaryDataTO to) throws IllegalArgumentException {
    ...
    try {
      ...
      log.warn(
          "Unable to create TRequestToken from RequestSummaryDataTO. "
              + "InvalidTRequestTokenAttributesException: {}",
          e.getMessage());
      throw new IllegalArgumentException(
          "Unable to create TRequestToken from RequestSummaryDataTO.");
    }
    ...
    try {
      ...
      log.warn(
          "REQUEST SUMMARY CATALOG! Unable to create RequestSummaryData. "
              + "InvalidRequestSummaryDataAttributesException: {}",
          e.getMessage(),
          e);
    ...
    if (to.getStatus() != null) {
      ...
      if (code == TStatusCode.EMPTY) {
        log.warn(
            "RequestSummaryDataTO retrieved StatusCode was not " + "recognised: {}",
            to.getStatus());
      } else {
    ...
    if (to.getPinLifetime() != null) {
      data.setPinLifetime(
          TLifeTimeInSeconds.make(
              PinLifetimeConverter.getInstance().toStoRM(to.getPinLifetime()), TimeUnit.SECONDS));
    }
  ...
  public synchronized void updateGlobalStatus(TRequestToken rt, TReturnStatus status) {
    ...
    dao.updateGlobalStatus(
        rt.toString(),
        StatusCodeConverter.getInstance().toDB(status.getStatusCode()),
        status.getExplanation());
  }

  public void updateFromPreviousGlobalStatus(
      TRequestToken requestToken,
      TStatusCode expectedStatusCode,
      TStatusCode newStatusCode,
      String explanation) {

    dao.updateGlobalStatusOnMatchingGlobalStatus(
        requestToken, expectedStatusCode, newStatusCode, explanation);
  }

  /**
   * Method used to update the global status of a request identified by TRequestToken, to the
   * supplied TReturnStatus. The pin lifetime and the file lifetime are updated in order to start
   * the countdown from the moment the status is updated. In case of any exception nothing happens.
   */
  public synchronized void updateGlobalStatusPinFileLifetime(
      TRequestToken rt, TReturnStatus status) {
   ...
   * logic.
   *
   * <p>If the supplied RequestSummaryData is null, nothing gets done; if any DB error occurs, no
   * exception gets thrown but proper messagges get logged.
   */
  ...
  /**
   * Method used to abort a request that has not yet been fetched for processing; if the status of
   * the request associated to the supplied request token tok is different from SRM_REQUEST_QUEUED,
  ...
  /**
   * Method used to abort a request that has not yet been fetched for processing; abort is only
   * applied to those SURLs of the request specified in the Collection; if the status of the request
   * associated to the supplied request token is different from SRM_REQUEST_QUEUED, then nothing
   * takes place; likewise if the supplied token does not correspond to any request, if it is null,
   * if the Collection is null, or the Collection does not contain TSURLs.
   */
  public synchronized void abortChunksOfRequest(TRequestToken rt, Collection<TSURL> c) {
    ...
    if ((rt != null) && (c != null) && (!c.isEmpty())) {
      try {
        ...
        log.error(
            "REQUEST SUMMARY CATALOG! Unexpected error in "
   ...
   */
  public synchronized void abortChunksOfInProgressRequest(
      TRequestToken rt, Collection<TSURL> tsurls) {
      ...
      try {
        ...
        log.error(
            "REQUEST SUMMARY CATALOG! Unexpected error in "
                ...
                + "contain TSURLs! Error: {}",
            e.getMessage());
      }
    }
  }

  public synchronized RequestSummaryData find(TRequestToken requestToken)
    ...
    if (to != null) {
      try {
        ...
        if (data != null) {
          log.debug(
              "REQUEST SUMMARY CATALOG: {} associated to {} retrieved",
              data.requestToken(),
              data.gridUser().getDn());
          return data;
        }
        ...
        log.error(
            "REQUEST SUMMARY CATALOG; Failure performing makeOne operation. "
