...
public class PtPChunkCatalog {
   ...
   * empty Collection is returned, and a messagge gets logged. NOTE! Chunks in SRM_ABORTED status
   * are NOT returned! This is imporant because this method is intended to be used by the Feeders to
  ...
  private PtPPersistentChunkData makeOne(PtPChunkDataTO auxTO, TRequestToken rt) {
    ...
    try {
      ...
      if (pinLifeTime > max) {
        log.warn(
            "PinLifeTime is greater than the max value allowed. Drop the "
                + "value to the max = {} seconds",
            max);
        pinLifeTime = max;
      }
    ...
    try {
      fileLifetime =
          TLifeTimeInSeconds.make(
              FileLifetimeConverter.getInstance().toStoRM(auxTO.fileLifetime()), TimeUnit.SECONDS);
    } catch (IllegalArgumentException e) {
      errorSb.append("\n");
      errorSb.append(e);
    }
    ...
    if (overwriteOption == TOverwriteMode.EMPTY) {
      errorSb.append(
          "\nTOverwriteMode could not be translated "
              + "from its String representation! String: "
    ...
    try {
      ...
      log.warn(
          "PtP CHUNK CATALOG! Retrieved malformed PtP chunk data"
              + " from persistence. Dropping chunk from request: {}",
          rt);
   ...
   */
  private ReducedPtPChunkDataTO completeTO(
      PtPChunkDataTO chunkTO, final PtPPersistentChunkData chunk)
      throws InvalidReducedPtPChunkDataAttributesException {
  ...
  private ReducedPtPChunkData reduce(PtPPersistentChunkData chunk)
    ...
    ReducedPtPChunkData reducedChunk =
        new ReducedPtPChunkData(
            chunk.getSURL(), chunk.getStatus(), chunk.fileStorageType(), chunk.fileLifetime());
  ...
  public Collection<ReducedPtPChunkData> lookupReducedPtPChunkData(
    ...
    Collection<ReducedPtPChunkDataTO> reducedChunkDataTOs =
        dao.findReduced(requestToken.getValue(), surls);
  ...
  private ReducedPtPChunkData makeOneReduced(ReducedPtPChunkDataTO reducedChunkDataTO) {
    ...
    // fileStorageType
    TFileStorageType fileStorageType =
        FileStorageTypeConverter.getInstance().toSTORM(reducedChunkDataTO.fileStorageType());
    if (fileStorageType == TFileStorageType.EMPTY) {
      errorSb.append(
    ...
    try {
      fileLifetime =
          TLifeTimeInSeconds.make(
              FileLifetimeConverter.getInstance().toStoRM(reducedChunkDataTO.fileLifetime()),
              TimeUnit.SECONDS);
    } catch (IllegalArgumentException e) {
      errorSb.append("\n");
      errorSb.append(e);
    ...
    try {
      ...
      log.warn(
          "PtP CHUNK CATALOG! Retrieved malformed Reduced PtP"
  ...
  public int updateFromPreviousStatus(
      TRequestToken requestToken,
      TStatusCode expectedStatusCode,
      TStatusCode newStatusCode,
      String explanation) {

    return dao.updateStatusOnMatchingStatus(
