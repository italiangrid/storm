...
/**
 * Class that represents StoRMs CopyChunkCatalog: it collects CopyChunkData and provides methods for
 * looking up a CopyChunkData based on TRequestToken, as well as for updating an existing one.
 *
...
public class CopyChunkCatalog {
  ...
  /**
   * Method used to update into Persistence a retrieved CopyChunkData. In case any error occurs, the
   * operation does not proceed and no Exception is thrown.
   *
   * <p>Beware that the only fields updated into persistence are the StatusCode and the errorString.
   */
  ...
  /**
   * Method that returns a Collection of CopyChunkData Objects matching the supplied TRequestToken.
   *
   * <p>If any of the data associated to the TRequestToken is not well formed and so does not allow
   * a CopyChunkData Object to be created, then that part of the request is dropped and gets logged,
  ...
  private Collection<CopyPersistentChunkData> buildChunkDataList(
    ...
    for (CopyChunkDataTO chunkTO : chunkDataTOs) {
      ...
      try {
        ...
        log.warn(
            "COPY CHUNK CATALOG! unable to add missing informations on " + "DB to the request: {}",
            e.getMessage());
      }
    }
  ...
  public Collection<CopyPersistentChunkData> lookupCopyChunkData(
    ...
    Collection<CopyChunkDataTO> chunkDataTOs = dao.find(requestToken, surlsUniqueIDs, surlsArray);
  ...
  private CopyPersistentChunkData makeOne(CopyChunkDataTO chunkDataTO, TRequestToken rt) {
    ...
    try {
      lifeTime =
          TLifeTimeInSeconds.make(
              FileLifetimeConverter.getInstance().toStoRM(chunkDataTO.lifeTime()),
              TimeUnit.SECONDS);
    ...
    TFileStorageType fileStorageType =
        FileStorageTypeConverter.getInstance().toSTORM(chunkDataTO.fileStorageType());
    ...
    String spaceTokenTranslation =
        SpaceTokenStringConverter.getInstance().toStoRM(chunkDataTO.spaceToken());
    ...
    TOverwriteMode globalOverwriteOption =
        OverwriteModeConverter.getInstance().toSTORM(chunkDataTO.overwriteOption());
    ...
    try {
      ...
      log.warn(
          "COPY CHUNK CATALOG! Retrieved malformed Copy"
  ...
  /**
   * Adds to the received CopyChunkDataTO the normalized StFN and the SURL unique ID taken from the
   * CopyChunkData
   *
   * @param chunkTO
   * @param chunk
  ...
  /**
   * Creates a ReducedCopyChunkDataTO from the received CopyChunkDataTO and completes it with the
   ...
   */
  private ReducedCopyChunkDataTO completeTO(
      CopyChunkDataTO chunkTO, final CopyPersistentChunkData chunk)
      throws InvalidReducedCopyChunkDataAttributesException {
  ...
  private ReducedCopyChunkData reduce(CopyPersistentChunkData chunk)
    ...
    ReducedCopyChunkData reducedChunk =
        new ReducedCopyChunkData(chunk.getSURL(), chunk.getDestinationSURL(), chunk.getStatus());
  ...
  /**
   * Checks if the received CopyChunkDataTO contains the fields not set by the front end but
   * required
   *
   * @param chunkTO
   * @return
  ...
  private boolean isComplete(ReducedCopyChunkDataTO reducedChunkTO) {
    ...
    return (reducedChunkTO.normalizedSourceStFN() != null)
        && (reducedChunkTO.sourceSurlUniqueID() != null
            && reducedChunkTO.normalizedTargetStFN() != null)
