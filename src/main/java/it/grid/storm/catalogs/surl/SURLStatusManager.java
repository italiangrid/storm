...
public interface SURLStatusManager {
  ...
  /**
   * Checks that the input sulrs are pinned in requests coming from user user
   * and returns a map of the status where surls may be pinned or have an error 
   * status (which happens when one of the surl passed as argument is not 
   * found in the storm db)  
   * 
   * @param user
   * @param surls
   * @return
   */
  public Map<TSURL, TReturnStatus> getPinnedSURLsForUser(GridUserInterface 
    user, List<TSURL> surls);
