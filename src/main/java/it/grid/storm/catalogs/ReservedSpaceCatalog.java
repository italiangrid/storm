...
public class ReservedSpaceCatalog {
  ...
  public void updateAllStorageSpace(StorageSpaceData ssd)
      throws NoDataFoundException, InvalidRetrievedDataException, MultipleDataEntriesException {
  ...
  public void updateAllStorageSpace(StorageSpaceData ssd, Date updateTime)
    ...
    try {
      ...
      log.error("Error while retrieving StorageSpaceDAO: {}", daEx.getMessage(), daEx);
    }
  ...
  public StorageSpaceData getStorageSpaceByAlias(String desc) {
    ...
    try {
      ...
      log.debug("Error while retrieving StorageSpaceDAO: {}", daEx.getMessage(), daEx);
    }
    ...
    try {
      ...
      for (StorageSpaceTO storagesSpaceTO : storagesSpaceTOCollection) {
        if (storagesSpaceTO != null) {
          ...
          log.warn(
              "Received a collection of StorageSpaceTO containing null "
  ...
  public List<StorageSpaceData> getStorageSpaceByLastUpdate(Date lastUpdateTimestamp) {
    ...
    try {
      Collection<StorageSpaceTO> storagesSpaceTOCollection =
          ssDAO.getStorageSpaceByPreviousLastUpdate(lastUpdateTimestamp);
  ...
  public ArrayOfTSpaceToken getSpaceTokens(GridUserInterface user, String spaceAlias) {
    ...
    try {
      ...
      log.debug(
          "getSpaceTokens : Number of Storage spaces retrieved with " + "Alias '{}': {}",
          spaceAlias,
          nItems);
  ...
  public boolean increaseUsedSpace(String spaceToken, Long usedSpaceToAdd) {
    ...
    if (n == 0) {
      log.warn(
          "No errors caught but it seems no used space updates done on space token {}", spaceToken);
    }
  ...
  public boolean decreaseUsedSpace(String spaceToken, Long usedSpaceToRemove) {
    ...
    log.debug(
        "Decrease {} the used space of storage spaceToken: {}", usedSpaceToRemove, spaceToken);
