...
public class ChecksumManager {
	...
	// /**
	// * Computes the checksum of the given file and stores it in to an extended
	// attribute.
	// *
	...
	// *
	// * @param Temporary workaround that allows separation of checksum servers
	// given a file path.
	// * @return the checksum service URL. Return <code>null</code> if all the
	...
	//
	// if (!isAlive) {
	// log.warn("Skipping checksum service because it doesn't respond or cannot access the requested file: "
	// + url.toString());
	// }
	//
	// iter++;
	...
	// }
	//
	// log.info("Selected checksum server: " + url + " (requestQueue=" +
	// status.getRequestQueue()
	...
	//
	// int[] servicePortArray =
	// Configuration.getInstance().getChecksumServicePorts();
	...
	//
	// int[] statusPortArray =
	// Configuration.getInstance().getChecksumStatusPorts();
	// if (servicePortArray.length != urlListSize) {
	...
	//
	// if (servicePort == -1) {
	// log.error("Configuration error: invalid service port element '" + i +
	// "', '" + servicePort + "'. Skipping checksum server '" + hostname + "'.");
	// continue;
	// }
	// if (statusPort == -1) {
	// log.error("Configuration error: invalid status port element '" + i + "', '"
	// + statusPort + "'. Skipping checksum server '" + hostname + "'.");
	// continue;
	// }
	//
	// URL url;
	...
	// }
	//
	// private String retrieveChecksumFromExternalService(String fileName, String
	// algorithm) {
	//
	// if (serviceUrlList.isEmpty()) {
	//
	// log.warn("No external checksum servers found, no checksum returned for file: "
	// + fileName);
	// return null;
	// }
	//
