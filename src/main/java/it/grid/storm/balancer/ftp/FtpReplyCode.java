...
public enum FtpReplyCode {
	...
	GENERIC_PRELIMINARY(199), COMMAND_OK(200), SYSTEM_STATUS(211), SERVICE_READY(
		220), SERVICE_CLOSING(221), GENERIC_POSITIVE(299), GENERIC_POSITIVE_INTERMEDIATE(
		399), SERVICE_NOT_AVAILABLE(421), GENERIC_TRANSIENT_NEGATIVE(499), NOT_LOGGED_IN(
		530), GENERIC_PERMANENT_NEGATIVE(599), GENERIC_PROTECTED(699), UNKNOWN_ERROR(
		999);
	...
	public static FtpReplyCode find(int v) {
		...
		if (isPositivePreliminary(v))
			return GENERIC_PRELIMINARY;
		if (isPositiveCompletion(v))
			return GENERIC_POSITIVE;
		if (isPositiveIntermediate(v))
			return GENERIC_POSITIVE_INTERMEDIATE;
		if (isTransientNegative(v))
			return GENERIC_TRANSIENT_NEGATIVE;
		if (isPermanentNegative(v))
