...
/**
 * This class represents an TDirOption Object. TDirOption contains information about directory
 * visit.
 *
 * @author Magnoni Luca
...
public class TDirOption {
  ...
  /**
   * Constructor that requires boolean isDirectory indicating whether the SURL refers to a Directory
   * or not, and a boolean allLevel to indicate if recursion on all sub-directories is wanted. If
   * allLevel is false, an InvalidTDirOptionAttributesException is thrown.
   */
  public TDirOption(boolean isDirectory, boolean allLevel)
    ...
    if (allLevelRecursive == false) throw new InvalidTDirOptionAttributesException(allLevel, -1);
  }
  ...
  /**
   * Constructor that requires boolean isDirectory, boolean allLevel, int numLevel. An exception is
   * thrown if allLevel is true, and numLevel>0.
   */
  ...
  /** Return num of recursive level to visit. If isAllLevelRecursive then 0 is returned. */
  public int getNumLevel() {

    if (!allLevelRecursive) return numOfLevels;
    else return 0;
  }
