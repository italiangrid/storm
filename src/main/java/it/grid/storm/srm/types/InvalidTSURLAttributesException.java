...
/**
 * This class represents an Exception thrown when a TSURL constructor is invoked
 * with null or empty SiteProtocol or SFN.
 * 
 * @author Ezio Corso
...
public class InvalidTSURLAttributesException extends Exception {
	...
	public InvalidTSURLAttributesException(SiteProtocol prt, SFN sfn) {
		...
		if (!nullProtocol)
			emptyProtocol = (prt == SiteProtocol.EMPTY);
		nullSFN = (sfn == null);
		if (!nullSFN)
			emptySFN = sfn.isEmpty();
	}

	public String toString() {

		return "Invalid TSURL Attibutes: nullProtocol=" + nullProtocol
			+ "; emptyProtocol=" + emptyProtocol + "; nullSFN=" + nullSFN
			+ "; emptySFN=" + emptySFN + ".";
	}
}
