...
public class TFileLocality {
  ...
  public static TFileLocality getTFileLocality(String loc) {
    ...
    if (loc.toLowerCase().replaceAll(" ", "").equals(ONLINE.getValue().toLowerCase())) {
      return ONLINE;
    } else if (loc.toLowerCase().replaceAll(" ", "").equals(NEARLINE.getValue().toLowerCase())) {
      return NEARLINE;
    } else if (loc.toLowerCase()
        .replaceAll(" ", "")
        .equals(ONLINE_AND_NEARLINE.getValue().toLowerCase())) {
      return ONLINE_AND_NEARLINE;
    } else if (loc.toLowerCase().replaceAll(" ", "").equals(LOST.getValue().toLowerCase())) {
      return LOST;
    } else if (loc.toLowerCase().replaceAll(" ", "").equals(NONE.getValue().toLowerCase())) {
      return NONE;
    } else if (loc.toLowerCase().replaceAll(" ", "").equals(UNAVAILABLE.getValue().toLowerCase())) {
      return UNAVAILABLE;
    } else {
      return EMPTY;
    }
  ...
  /**
   * Facility method to obtain a TFileStorageType object given its int representation. If an invalid
   * String is supplied, then an EMPTY TFileStorageType is returned.
   */
  ...
  /**
   * Decode method, use to create a TFileLocaliy object from the information contained into a
   * structure parametet received from FE.
   *
   * @param inputParam
   * @param name
   * @return
  ...
  public static TFileLocality decode(Map<?, ?> inputParam, String name) {
    ...
    if (fileLoc != null) return TFileLocality.getTFileLocality(fileLoc.intValue());
    else return TFileLocality.EMPTY;
  }

  /**
   * Encode method use to create a structured paramter that represents this object, used for pass
   * information to FE.
   *
   * @param param
   * @param name
  ...
  public void encode(Map<String, Integer> param, String name) {
    ...
    Integer value = null;
    if (this.equals(TFileLocality.ONLINE)) value = Integer.valueOf(0);
    if (this.equals(TFileLocality.NEARLINE)) value = Integer.valueOf(1);
    if (this.equals(TFileLocality.ONLINE_AND_NEARLINE)) value = Integer.valueOf(2);
    if (this.equals(TFileLocality.LOST)) value = Integer.valueOf(3);
    if (this.equals(TFileLocality.NONE)) value = Integer.valueOf(4);
    if (this.equals(TFileLocality.UNAVAILABLE)) value = Integer.valueOf(5);
