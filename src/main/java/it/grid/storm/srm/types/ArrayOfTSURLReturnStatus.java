...
public class ArrayOfTSURLReturnStatus {
	...
	public ArrayOfTSURLReturnStatus(TSURLReturnStatus[] surlArray)
		...
		this.surlRetList = new ArrayList<TSURLReturnStatus>(
			Arrays.asList(surlArray));
	}
	 ...
	 *           if null argument or not contained surl
	 */
	public void updateStatus(TSURLReturnStatus surlStatus, TReturnStatus newStatus)
		...
		if (surlStatus == null || newStatus == null) {
			throw new IllegalArgumentException(
				"Unable to update the status,null arguments: surlStatus=" + surlStatus
					+ " newStatus=" + newStatus);
		}
	...
	public int hashCode() {
		...
		int result = 1;
		result = prime * result
			+ ((surlRetList == null) ? 0 : surlRetList.hashCode());
		return result;
	}
