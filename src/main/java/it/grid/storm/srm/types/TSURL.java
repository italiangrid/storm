...
public class TSURL {
  ...
  private static List<Authority> managedSrmEndpoints = Configuration.getInstance().getManagedSrmEndpoints();
  ...
  public static boolean isValid(TSURL surl) {
    return isManaged(surl, TSURL.managedSrmEndpoints);
  }

  public static boolean isManaged(TSURL surl, List<Authority> managedEndpoints) {

    boolean result = false;
    for (Authority e : managedEndpoints) {
      if (isSURLManaged(surl, e)) {
        result = true;
        break;
      }
    }
  ...
  private static boolean isSURLManaged(TSURL comingSURL, Authority endpoint) {

    String serviceHost = comingSURL.sfn().machine().toString();

    log.debug("SURL VALID [ coming-service-host = {}, expected = {} ]", serviceHost, endpoint.getServiceHostname());

    if (serviceHost.equalsIgnoreCase(endpoint.getServiceHostname())) {
      if (comingSURL.sfn().port().isEmpty()) {
        return true;
      } else {
        int servicePort = comingSURL.sfn().port().toInt();
        log.debug("SURL VALID [ coming-service-port = {}, expected = {} ]", servicePort, endpoint.getServicePort());
        return servicePort == endpoint.getServicePort();
      }
    } else {
      return false;
    }
