...
public class TSURL {
	...
	/**
	 * Build a TSURL by extracting the content of the received SURL object and
	 * storing the received raw surl string
	 * 
	 * @param surl
	 * @param rawSurl
	 * @return
  ...
  public static TSURL getWellFormed(SURL surl, String rawSurl)
	 ...
	 * SiteProtocol and the SFN of this TSURL: if any is null or empty an
	 * InvalidTSURLAttributesException is thrown. Check for ".." in Storage File
	 * Name for security issues.
	 */
	private static TSURL make(SiteProtocol sp, SFN sfn, String userSurl)
		throws InvalidTSURLAttributesException {

		if ((sp == null) || (sfn == null) || (sp == SiteProtocol.EMPTY)
			|| sfn.isEmpty()) {
	...
	/**
	 * Static factory method that returns a TSURL from a String representation: if
	 * it is null or malformed then an Invalid TSURLAttributesException is thrown.
	 */
	...
	/**
	 * A SURL is managed by a managed SURL if their hosts are the same and if the
	 * comingSURL specifies a port this port is the same as the one specified on
	 * the managed SURL or, if the managed SURL doesn't specifies a port this port
	 * is listed in the default ports
	 * 
	 * @param comingSURL
	 * @param managedSURL
	 * @return
		...
		if (comingSURL.sfn().port().isEmpty()) {
			...
			if (!managedSURL.sfn().port().isEmpty()) {
				...
				try {
					...
					if ((serviceHost.equalsIgnoreCase(expectedServiceHost))
						&& (defaultPorts.contains(comingPort))) {
						result = true;
					}
				} catch (InvalidPortAttributeException e) {
	...
	public int hashCode() {
		...
		result = prime * result
			+ ((normalizedStFN() == null) ? 0 : normalizedStFN().hashCode());
	...
	/**
	 * Method that returns the SiteProtocol of this TSURL. If this is empty, then
	 * an empty SiteProtocol is returned.
	 */
	...
	/**
	 * Method that returns the SFN of this SURL. If this is empty, then an empty
	 * SFN is returned.
	 */
	public SFN sfn() {
