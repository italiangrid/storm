...
public class TPermissionMode {
  ...
  public static TPermissionMode getTPermissionMode(String type) {
    ...
    if (type.equals(NONE.getValue())) return NONE;
    if (type.equals(X.getValue())) return X;
    if (type.equals(W.getValue())) return W;
    if (type.equals(WX.getValue())) return WX;
    if (type.equals(R.getValue())) return R;
    if (type.equals(RX.getValue())) return RX;
    if (type.equals(RW.getValue())) return RW;
    if (type.equals(RWX.getValue())) return RWX;
    return null;
  }
  ...
  public static TPermissionMode getTPermissionMode(FilesystemPermission type) {
    ...
    if (type.canReadFile() || type.canListDirectory()) perm += "R";
    if (type.canWriteFile()) perm += "W";
    if (type.canTraverseDirectory()) perm += "X";
    if (perm.length() == 0) perm = "None";
  ...
  public void encode(Map param, String name) {
    ...
    if (this.equals(NONE)) permissionInt = Integer.valueOf(0);
    if (this.equals(X)) permissionInt = Integer.valueOf(1);
    if (this.equals(W)) permissionInt = Integer.valueOf(2);
    if (this.equals(WX)) permissionInt = Integer.valueOf(3);
    if (this.equals(R)) permissionInt = Integer.valueOf(4);
    if (this.equals(RX)) permissionInt = Integer.valueOf(5);
    if (this.equals(RW)) permissionInt = Integer.valueOf(6);
    if (this.equals(RWX)) permissionInt = Integer.valueOf(7);
