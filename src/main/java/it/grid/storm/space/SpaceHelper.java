...
public class SpaceHelper {
  ...
  public TSpaceToken createVOSA_Token(String spaceTokenAlias, TSizeInBytes totalOnLineSize,
    ...
    if (tokenArray == null || tokenArray.size() == 0) {
      ...
      try {
        ...
        try {
          ssd.setUnavailableSpaceSize(TSizeInBytes.make(0));
          ssd.setReservedSpaceSize(TSizeInBytes.make(0));
       ...
       * needed. Only static information changes determine an update of the existing row
       */
      ...
      // false otherwise
      if (equal) {
        // Do nothing if equals, everything are already present into the DB
   ...
   * createVO_SA_token(...)) to remove from the database the old VO_SA_token inserted from the
