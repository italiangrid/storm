...
public class SpaceHelper {
  ...
  public boolean isSAInitialized(Logger log, StoRI stori) {
    ...
    if (spaceData != null
        && spaceData.getUsedSpaceSize() != null
    ...
    log.debug(
        "The storage area is initialized with token alias {} is {} initialized",
        spaceData.getSpaceTokenAlias(),
        (response ? "" : "not"));
    return response;
  }
   ...
   *
   * <p>The following code check if a SA_token with the same space description is already present
   * into the catalog, if no data are found the new data are inserted, if yes the new data and the
   * data already present are compared, and if needed an update operation is performed.
   *
   ...
   */
  public TSpaceToken createVOSA_Token(
      String spaceTokenAlias, TSizeInBytes totalOnLineSize, String spaceFileName) {
    ...
    if (tokenArray == null || tokenArray.size() == 0) {
      ...
      SpaceHelper.log.debug(
          "VoSpaceArea {} still does not exists. Start creation process.", spaceTokenAlias);
       ...
       */
      SpaceHelper.log.debug(
          "VOSpaceArea for space token description "
              + spaceTokenAlias
      ...
      if (equal) {
        ...
        // the DB
        SpaceHelper.log.debug(
            "VOSpaceArea for space token description {} is already up to date.", spaceTokenAlias);
        ReservedSpaceCatalog.addSpaceToken(spaceToken);
    ...
    if ((token_a != null) && (token_a.size() > 0)) {
      for (int i = 0; i < token_a.size(); i++) {
        ...
        if (!ReservedSpaceCatalog.getTokenSet().contains(token_a.getTSpaceToken(i))) {
          ...
          log.debug(
              "VO SA token {}  is no more used, removing it from persistence.", tokenToRemove);
