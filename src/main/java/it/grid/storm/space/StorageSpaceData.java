...
public class StorageSpaceData {
	...
	private static final Logger log = LoggerFactory
		.getLogger(StorageSpaceData.class);
	...
	/**
	 * Used to create a new Storage Space entity. It could be a Storage Area Space
	 * (static reservation) or a Space Reservation (dynamic space reservation)
	 * 
	 * @param guOwner
	 * @param spaceType
	 ...
	 */
	public StorageSpaceData(GridUserInterface guOwner, TSpaceType spaceType,
		String spaceTokenAlias, TSizeInBytes totalDesiredSize,
		TSizeInBytes guaranteedSize, TLifeTimeInSeconds spaceLifetime,
		...
		try {
			this.spaceToken = TSpaceToken.make(new it.grid.storm.common.GUID()
				.toString());
		} catch (InvalidTSpaceTokenAttributesException ex) {
			log.error(
				"Exception occurred while trying to generate a Token with GUID", ex);
		}
	...
	public StorageSpaceData(StorageSpaceTO ssTO) throws IllegalArgumentException {
		...
		if (ssTO == null) {
			log
				.error("Unable to create StorageSpaceData object, provided StorageSpaceTO parameter is null");
			throw new IllegalArgumentException("Received null argument");
		} else {
			// Ownership of Storage Space
			if (!(ssTO.getOwnerName() == null || ssTO.getVoName() == null || ssTO
				.getVoName().equals(VO.NO_VO.getValue()))) {
				try {
					this.owner = GridUserManager.makeVOMSGridUser(ssTO.getOwnerName(),
						ssTO.getVoName());
			...
			try {
				this.spaceLifetime = TLifeTimeInSeconds.make(ssTO.getLifetime(),
					TimeUnit.SECONDS);
			} catch (IllegalArgumentException e) {
	...
	/**
	 * @param spaceGuaranteed
	 *          the spaceGuaranteed to set
	 */
	...
	public final TSizeInBytes getFreeSpaceSize() {
		...
		if (!freeSpaceSizeForced) {
			...
			if (this.totalSpaceSize == null || this.totalSpaceSize.isEmpty()
				|| this.usedSpaceSize == null || this.usedSpaceSize.isEmpty()) {
				...
				if (size >= 0) {
					...
					log
						.warn("Unable to create a valid Free Space Size, computed free space is lower than zero");
					this.freeSpaceSize = TSizeInBytes.makeEmpty();
				}
			}
		}
	...
	private final void updateFreeSize() {
		...
		long freeSizeValue = this.totalSpaceSize.value()
			- this.getUsedSpaceSize().value();
	...
	private final void updateAvailableSize() {
		...
		if ((availableSizeValue < this.totalSpaceSize.value())
			&& (availableSizeValue >= 0)) {
			try {
	...
	/**
	 * @param unavailableSpaceSize
	 *          the unavailableSpaceSize to set
	 */
	...
	public final TSizeInBytes getBusySpaceSize() {
		...
		if (!this.busySpaceSizeForced) {
			if (this.usedSpaceSize == null || this.usedSpaceSize.isEmpty()
				...
				try {
					this.busySpaceSize = TSizeInBytes.make(
						this.usedSpaceSize.value() + this.unavailableSpaceSize.value()
	...
	public final TSizeInBytes getAvailableSpaceSize() {
		...
		if (!this.availableSpaceSizeForced) {
			if (this.totalSpaceSize == null || this.totalSpaceSize.isEmpty()
				...
				try {
					this.availableSpaceSize = TSizeInBytes.make(
	 ...
	 */
	// TODO This is not public because of private
	// ReserveSpaceCommand.updateReservation calls , hope can be changed soon
	...
	/**
	 * @param totalGuaranteedSize
	 *          the totalGuaranteedSize to set
	 */
	...
	/**
	 * This method is used to verify if the Space Reservation is expired, so the
	 * lifetime is no more valid.
	 * 
	...
	public boolean isExpired() {
		...
		long expirationTimeInMillisec = this.creationDate.getTime()
			+ this.spaceLifetime.value() * 1000;
