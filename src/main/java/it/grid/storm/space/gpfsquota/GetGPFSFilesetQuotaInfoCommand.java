...
public class GetGPFSFilesetQuotaInfoCommand implements Callable<GPFSFilesetQuotaInfo> {

  private static final Logger log = LoggerFactory.getLogger(GetGPFSFilesetQuotaInfoCommand.class);
  ...
  public GPFSFilesetQuotaInfo call() throws Exception {
    ...
    if (!(vfs.getFSDriverInstance() instanceof gpfs))
      throw new IllegalArgumentException("VFS driver is not GPFS for fs rooted at " + fsRoot);
    ...
    GPFSQuotaInfo info = GPFSQuotaInfo.fromNativeQuotaInfo(vfs, fs.get_fileset_quota_info(fsRoot));

    log.debug("Computed GPFS fileset quota info for fs rooted at {}: {}", fsRoot, info);
