...
public class GPFSMMLSQuotaCommand extends GPFSQuotaCommand {
	...
	private static final Logger log = LoggerFactory
		.getLogger(GPFSMMLSQuotaCommand.class);

	private static String pathSep = File.separator;
	private static String gpfsCommandPath = pathSep + "usr" + pathSep + "lpp"
		+ pathSep + "mmfs" + pathSep + "bin";
	 ...
	 * 
	 * @see
	 * it.grid.storm.space.quota.GPFSQuotaCommand#executeGetQuotaInfo(it.grid.
	 * storm.namespace.model.Quota, boolean)
	 */
	@Override
	public GPFSQuotaCommandResult executeGetQuotaInfo(Quota quotaElement,
		boolean test) throws QuotaException {
		...
		if (cmdExitCode.equals(ExitCode.SUCCESS)) {
			...
			// Manage failure
			log.warn("Failed to use MMLSQUOTA! on device: '"
				+ quotaElement.getDevice() + "' and element: '"
				+ quotaElement.getQuotaElementName() + "'");
			result.setFailure(true);
		}
		if (result.isFailure()) {
			log.warn("Quota execution returned nothing usefull. ("
				+ quotaElement.getDevice() + ":" + quotaElement.getQuotaElementName()
				+ ")  Check QUOTA on GPFS! ");
		}
		ec.stopExecution();
	...
	public GPFSQuotaCommandResult executeGetQuotaInfo(boolean test) {
		...
		List<VirtualFSInterface> vfsS = SpaceInfoManager.getInstance()
		...
		for (VirtualFSInterface vfs : vfsS) {
			...
			if (cap != null) {
				...
				qInfo = quotaResult.getQuotaResults().get(0); // Supposed to be unique
																											// result
				result.add(qInfo);
			} else {
	...
	private GPFSQuotaInfo manageSuccess(String output, Quota quotaElement) {
		...
		if (output != null) {
			...
			if (outputArray.length <= 1) {
				...
				log.debug("Splittin 1 line into more lines.. Remark-index:"
					+ remarkIndex);
				if (remarkIndex > 0) {
					if (output.length() > 7) {
			...
			for (String line : outputList) {
				if (GPFSQuotaInfo.meaningfullLineForLS(line)) {
					...
					log.trace("MMLSQUOTA - line: '" + line
						+ "' doesn't contain any usefull info.");
				}
			}
	 ...
	 *          grace | files quota limit in_doubt grace Remarks gemss_test
	 *          FILESET 512 2147483648 2147483648 0 none | 3128 0 0 0 none
	 * @return
	 */
