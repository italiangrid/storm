...
public class BackgroundGPFSQuota {
	...
	private static final Logger LOG = LoggerFactory
		.getLogger(BackgroundGPFSQuota.class);
	...
	private BackgroundGPFSQuota() {
		...
		int waitRefreshPeriod = Configuration.getInstance()
	...
	private static final long RELAX_PERIOD = 15000; // between two GPFSQuota at
																									// least 15 seconds
	private static final long WAIT_TIMEOUT_DURATION = 10000; // The executor can
																														// wait until to 10
																														// seconds
	...
	private final CompletionService<GPFSQuotaCommandResult> completionService = new ExecutorCompletionService<GPFSQuotaCommandResult>(
		exec);
	...
	public boolean submitGPFSQuota() {
		...
		if (((lastExecutionTime > 0))
			&& (currTime - lastExecutionTime) < RELAX_PERIOD) {
			// It is not the first execution && I'm in relax , then
			// "Refuse submission"
			long remainngRelaxTime = RELAX_PERIOD - (currTime - lastExecutionTime);
			LOG
				.debug("GPFS Quota Background submission. Refused because relax period is not ended ("
					+ remainngRelaxTime + " ms yet.)");
		} else {
	...
	private boolean processCompletedTask(GPFSQuotaCommandResult quotaResult) {
		...
		boolean result = false;
		int nrFailires = QuotaManager.getInstance().updateSAwithQuotaResult(
			quotaResult);

		Date startDate = new Date(quotaResult.getStartTime());
		Date endDate = new Date(quotaResult.getStartTime()
			+ quotaResult.getDurationTime());
		Date creationDate = new Date(creationTime);
		LOG.debug("GPFS Quota execution: Submitted [" + creationDate + "], Start ["
			+ startDate + "], End[" + endDate + "], wait.duration = "
			+ (quotaResult.getStartTime() - creationTime) + ",exec.duration = "
			+ quotaResult.getDurationTime());
	...
	private class CompletionTask implements Runnable {
		...
		private boolean completeTask() throws InterruptedException {
			...
			try {
				...
				quotaResult = completedTask.get(WAIT_TIMEOUT_DURATION,
    ...
    */
	public class RefreshQuota {
		...
		public void start() {
			...
			timer.scheduleAtFixedRate(new RefreshTask(), delay,
