...
public class QuotaManager {
	...
	public int updateSAwithQuotaResult(GPFSQuotaCommandResult quotaResult) {
		...
		int nrFailures = 0;
		if (quotaResult != null) {
			List<String> quotaNames = SpaceInfoManager.getInstance()
				.retrieveQuotaNamesToUse();
			...
			log.debug("vfs Quotas (size) : "
				+ (vfsQuotas != null ? vfsQuotas.size() : 0));
			...
			for (GPFSQuotaInfo gpfsQuotaInfoEntry : quotaResult.getQuotaResults()) {
				if (gpfsQuotaInfoEntry.isFailure()) {
					...
					if (quotaNames.contains(qName)) {
						...
						VirtualFSInterface vfsItem = retrieveCorrespondingVFS(vfsQuotas,
							qName);
						if (vfsItem != null) {
							vfsQuotaEnabled.add(vfsItem);
						}
						// Elaborate gpfsQuotaInfoEntry;
						StorageSpaceData ssd = SpaceInfoManager.getInstance()
							.getSSDfromQuotaName(qName);
						if (ssd != null) {
							process(gpfsQuotaInfoEntry, ssd);
						} else {
							log
								.error("Retrieved null StorageSpaceData. Unable to update the database");
							nrFailures++;
						}
					} else {
						log.error("Found '" + qName
							+ "' quota Name not corresponding to a Storage Area.");
						nrFailures++;
					}
				}
			}
			log.debug("vfs Enabled Quotas : "
				+ (vfsQuotaEnabled != null ? vfsQuotaEnabled.size() : 0));
	...
	public void updateSAwithQuotaAsynch(boolean test) {
		...
		if (accepted) {
			...
			log
				.debug("Asynchronous GPFS Quota execution is NOT started because too early..");
		}
	}
	...
	private VirtualFSInterface retrieveCorrespondingVFS(
		List<VirtualFSInterface> vfsQuotas, String dName) {

		VirtualFSInterface result = null;
		for (VirtualFSInterface vfsItem : vfsQuotas) {
			if (vfsItem.getCapabilities().getQuota().getQuotaElementName()
				.equals(dName)) {
				result = vfsItem;
			}
		}
	...
	private void process(GPFSQuotaInfo gpfsQuotaInfoEntry, StorageSpaceData ssd) {
		...
		log.debug("Used size for '" + gpfsQuotaInfoEntry.getQuotaEntryName()
			+ "' is " + usedSize + " KB.");
		try {
			...
			TSizeInBytes totalSizeInBytes = TSizeInBytes.make(totalSize,
				SizeUnit.BYTES);
			...
			log.error(
				"Negative values from gpfsQuotaCommand. Used size={} , Total size={}",
				usedSize, totalSize);
		}
