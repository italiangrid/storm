...
public class SpaceUtil {
	...
	// storageSpaceData.setUsedSpaceSize(TSizeInBytes.make(unusedD,
	// SizeUnit.BYTES));
	...
	// (TSizeInBytes.make(quotaInfo.getBlockSoftLimit(),
	// quotaInfo.getSizeUnit()));
	// long totalSizeL = new
	// Double(totalSize.getSizeIn(SizeUnit.BYTES)).longValue();
	// storageSpaceData.setTotalSpaceSize(TSizeInBytes.make(totalSizeL,
	// SizeUnit.BYTES));
	// storageSpaceData.setReservedSpaceSize(TSizeInBytes.make(totalSizeL,
	// SizeUnit.BYTES));
	// } catch (InvalidTSizeAttributesException ex3) {
	// log.error(" TotalSize (" + quotaInfo.getBlockSoftLimit()+
	// " KBytes) returned by Quota is wrong.");
	...
	// log.error("srmGetSpaceMetaData: Unable to complete Quota Command." +
	// qe.getMessage());
	 ...
	 */
	// private QuotaInfoInterface retrieveInfoFromQuota(TSpaceToken token) throws
	// QuotaException {
	...
	// log.error("srmGetSpaceMetaData: Unable to retrieve the Storage Area (VFS) with Root :"
	// + rootPFN, ex1);
	...
	// log.debug("srmGetSpaceMetaData: Retrieved Quota from VFS ('" + vfsName +
	// "'):" + quota);
	// // Check if Quota is defined
	// if (quota == null) {
	// throw new QuotaException("Quota Element is not defined for the VFS :" +
	// vfsName);
	// }
	// } catch (NamespaceException ex2) {
	// log.error("srmGetSpaceMetaData: Unable to retrieve the Quota Info from the VFS :"
	// + vfsName, ex2);
	// throw new QuotaException("Unable to retrieve the Quota Info from the VFS :"
	// + vfsName);
	// }
	//
	// /**
	...
	// + ") is " + fsType + " type");
	// throw new
	// QuotaException("Unable to execute a quota command for FS Type = '" + fsType
	// + "'.");
	// } else {
	...
	// throw new
	// QuotaException("Unable to execute a quota command because Quota is DISABLED");
	// }
	//
	...
	// default:
	// throw new
	// QuotaException("Unable to execute a quota command because Quota Type '"
