...
public class SpaceStatusSummary {
  ...
  public SpaceStatusSummary(String saAlias, long totalSpace) throws IllegalArgumentException {
    ...
    if (totalSpace < 0 || saAlias == null) {
      log.error(
          "Unable to create SpaceStatusSummary. Received illegal parameter: saAlias: "
              + saAlias
              + " totalSpace: "
              + totalSpace);
  ...
  private SpaceStatusSummary(
      String saAlias, long usedSpace, long unavailableSpace, long reservedSpace, long totalSpace) {
  ...
  public static SpaceStatusSummary createFromDB(String saAlias) throws IllegalArgumentException {
    ...
    if (storageSpaceData == null) {
      ...
      if (!storageSpaceData.isInitialized()) {
        log.warn(
            "Building the SpaceStatusSummary from non initialized space with alias \'"
                + saAlias
                + "\'");
      }
      SpaceStatusSummary summary =
          new SpaceStatusSummary(
              saAlias,
  ...
  public String getJsonFormat() {
    ...
    try {
      ...
      log.error(
          "Unable to produce Json representation of the object. XMLStreamException: "
              + e.getMessage());
    }
    try {
      strWriter.close();
  ...
  public String toString() {
    ...
    return "SpaceStatusSummary [getSaAlias()="
        + getSaAlias()
        + ", getBusySpace()="
        + getBusySpace()
        + ", getAvailableSpace()="
