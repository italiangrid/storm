...
public class TURLBuilder {
  ...
  private static TTURL buildTURL(
      Protocol protocol, Authority authority, String extraSlashes, PFN physicalFN) {

    TTURL turl = null;
    String turlString = null;
    try {
      turlString =
          protocol.getProtocolPrefix()
              + authority.toString()
              + extraSlashes
              + physicalFN.getValue();
      ...
      log.error(
          "Error while constructing TURL with Authority '{}': {}", authority, ex.getMessage(), ex);
    }
    return turl;
  }
