 ...
 *
 * <p>Edited by Michele Dibenedetto
 */
...
public class PathCreator {
  ...
  /**
   * If file is a directory returns the list of files contained in the directory, then on each
   * subfolder if recursive is true or level is greater than zero call itself recursively on it
   * decreasing the level. If file is file returns the file itself
   *
   * @param file
   * @param recursive
   * @param level
          ...
          for (String filePath : arrayOfNames) {
            ...
            if (!(filePath.startsWith("."))) {
              ...
              if (child.isDirectory()) {
                log.debug(
                    "It's a directory, calling recursive procedure" + " with level " + (level - 1));
                children.addAll(generateChildrenNoFolders(child, recursive, level - 1));
              } else {
    ...
    if (file.isDirectory() && (recursive || level > 0)) {
      ...
      if (arrayOfNames != null) {
        for (String filePath : arrayOfNames) {
          log.debug("Analizing child = " + filePath);
          if (!(filePath.startsWith("."))) {
            PathCreator path = new PathCreator(new File(file, filePath), recursive, level - 1);
