...
public class Capability implements CapabilityInterface {
	 ...
	 *          Protocol
	 */
	public void addTransportProtocolByScheme(Protocol protocol,
	...
	public void addProtocolPoolBySchema(Protocol protocol, ProtocolPool protPool)
		...
		if (protocol.equals(Protocol.GSIFTP)) {
			...
			for (PoolMember member : protPool.getPoolMembers()) {
				String hostname = member.getMemberProtocol().getAuthority()
					.getServiceHostname();
				...
				if (weighedPool) {
					try {
						...
						throw new NamespaceException("Unable to build pool for protocol "
							+ protocol);
					}
				} else {
					try {
			...
			try {
				balancingStrategy = BalancingStrategyFactory.getBalancingStrategy(
					protPool.getBalanceStrategy(), nodeList);
			} catch (IllegalArgumentException e) {
				log.error("Unable to get " + protPool.getBalanceStrategy().toString()
					+ " balacing strategy for nodes " + nodeList.toString());
				throw new NamespaceException(
					"Unable to create a balancing schema from the protocol pool");
			}
	...
	private Node buildNode(Protocol protocol, String hostname, int port)
		throws Exception {
	...
	private Node buildNode(Protocol protocol, String hostname, int port,
		int memberWeight) throws Exception {
	...
	public String toString() {
		...
		/**
		 * for (Map.Entry entry : m.entrySet()) { String key = entry.getKey();
		 * Vector value = entry.getValue(); }
		 **/

		// Print TransportProtocol
		int count = 0;
		for (Map.Entry<Protocol, TransportProtocol> transP : transpProtocolsByScheme
			.entrySet()) {
			count++;
			sb.append("[TP(" + count + ")] "
				+ (transP.getKey() + ": " + transP.getValue()));
		}
		// Print ProtocolPool
		count = 0;
		for (Map.Entry<Protocol, ProtocolPool> protPool : protocolPoolsByScheme
			.entrySet()) {
			count++;
			sb.append("[TP(" + count + ")] "
				+ (protPool.getKey() + ": " + protPool.getValue()));
		}
		return sb.toString();
	}
	...
	public static class ACLMode {
		...
		private static ACLMode makeFromString(String aclMode)
			throws NamespaceException {

			ACLMode result = ACLMode.UNDEF;
			if (aclMode.toLowerCase().equals(
				ACLMode.AHEAD_OF_TIME.toString().toLowerCase())) {
				result = ACLMode.AHEAD_OF_TIME;
			} else if (aclMode.toLowerCase().equals(
				ACLMode.JUST_IN_TIME.toString().toLowerCase())) {
		...
		public boolean equals(Object obj) {
			...
			if (obj instanceof ACLMode) {
				...
				if (aclMode.toString().toLowerCase()
