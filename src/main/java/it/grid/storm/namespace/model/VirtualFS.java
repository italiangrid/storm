...
public class VirtualFS implements VirtualFSInterface {
	...
	public List<MappingRule> getMappingRules() throws NamespaceException {
		...
		if (this.mappingRules.isEmpty()) {
			throw new NamespaceException("No one MAPPING RULES bound with this VFS ("
				+ aliasName + "). ");
		}
	...
	private SpaceSystem makeSpaceSystemInstance() throws NamespaceException {
		...
		if ((this.spaceSystemDriver.getName().equals(GPFSSpaceSystem.class
			.getName()))
			|| (this.spaceSystemDriver.getName().equals(MockSpaceSystem.class
				.getName()))) {
			...
			try {
				...
				log.error(
					"Unable to instantiate the SpaceSystem Driver. Wrong target.", ex1);
	...
	public synchronized void useSpaceForFile(TSpaceToken token, StoRI file,
		TSizeInBytes sizePresumed) throws NamespaceException,
		...
		try {
			defaultFileSize = TSizeInBytes.make(Configuration.getInstance()
				.getFileDefaultSize(), SizeUnit.BYTES);
		...
		if (spaceData == null) {
			throw new NamespaceException("No Storage Space stored with this token :"
				+ token);
		}
		...
		if (!(sizePresumed.isEmpty())) {
			...
			if ((!(spaceFile.getLocalFile().exists()))
				...
				log
					.error("Unable to get  the correct space file!spaceFile does not exsists or it is a directory.");
				return;
			}
			 ...
			 *       the half of the original space file, the original spacefile is
			 *       renamed to the desired ptp file name and then truncated to the
			 *       requested size. A new space pre_allocation is perfored and bound
			 *       with the old original space file name.
			 * 
			 */
			...
			try {
				newUsedSpaceSize = TSizeInBytes.make(totalSpaceSize.value()
					- remainingSize, SizeUnit.BYTES);
				newAvailableSpaceSize = TSizeInBytes
	 ...
	 * 
	 * @param spaceOrig
	 *          StoRI bounds to the original space file.
	 * @param file
	 *          StoRI bounds to the desired new PtP file.
	...
	public TSizeInBytes splitSpace(StoRI spaceOrig, StoRI file, long sizePresumed)
		...
		String relativeSpacePFN = NamespaceUtil.extractRelativePath(
			this.getRootPath(), spacePFN);
		/**
		...
		if (failure) {
			log
				.warn("SpacePFN does not refer to this VFS root! Something goes wrong in app-rule?");
			try {
				newSize = TSizeInBytes.make(sizePresumed, SizeUnit.BYTES);
				file = createSpace(
					NamespaceUtil.extractRelativePath(this.getRootPath(),
	...
	public String toString() {
		...
		sb.append(" VFS Space driver : '" + this.spaceSystemDriver.getName() + "'"
			+ sep);
	 ...
	 * this.aliasName, ex); throw new
	 * NamespaceException("Error while retrieving Space System Driver for VFS :" +
	 * this.aliasName, ex); } catch (IllegalAccessException ex) {
	 * log.error("Error while accessing Space System driver for VFS :" +
	 * this.aliasName, ex); throw new
	 * NamespaceException("Error while accessing Space System driver for VFS :" +
	 * this.aliasName, ex); } catch (InstantiationException ex) {
	 * log.error("Error while instancianging Space System driver for VFS :" +
	 * this.aliasName, ex); throw new NamespaceException(
	 * "Error while instancianging Space System driver for VFS :" +
	 * this.aliasName, ex); } return ss; }
	 **/
