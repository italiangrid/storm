...
public class ApproachableRule implements Comparable<Object> {
	...
	public ApproachableRule(String rulename, SubjectRules subjectRules,
		String relativePath, boolean anonymousHttpReadAccess) {
	...
	private boolean matchVoms(GridUserInterface gUser) {
		...
		if (gUser instanceof AbstractGridUser
			&& ((AbstractGridUser) gUser).hasVoms()) {
			log.debug("Grid User Requestor   : "
				+ ((AbstractGridUser) gUser).toString());
			if (subjectRules.getVONameMatchingRule().match(
				((AbstractGridUser) gUser).getVO().getValue())) {
				return true;
			}
		}
		return false;
	...
	public String toString() {
		...
		sb.append("     SUBJECT - dn         : "
			+ this.getSubjectRules().getDNMatchingRule() + sep);
		if (!this.getSubjectRules().getVONameMatchingRule().isMatchAll()) {
			sb.append("     -- VOMS cert IS MANDATORY!" + sep);
			sb.append("       -- SUBJECT - vo_name    : "
				+ this.getSubjectRules().getVONameMatchingRule() + sep);
		} else {
			sb.append("     -- VOMS cert is not mandatory" + sep);
		}
		sb.append("     Relative-Path for Space : " + this.getSpaceRelativePath()
			+ sep);
	...
	public int hashCode() {
		...
		result = prime * result
			+ ((subjectRules == null) ? 0 : subjectRules.hashCode());
		return result;
	}
