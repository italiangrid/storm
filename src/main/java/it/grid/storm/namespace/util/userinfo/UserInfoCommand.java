...
public class UserInfoCommand {
	...
	private static final Logger log = LoggerFactory
		.getLogger(UserInfoCommand.class);
	 ...
	 */
	public int retrieveGroupID(UserInfoParameters parameters)
		throws UserInfoException {
		...
		if ((output != null) && (output.length() > 0)) {
			try {
				...
				if (groupLong.intValue() == groupLong.longValue()) {
					...
					log.warn("Group named '" + parameters + "' has a 32 bit GID "
						+ groupLong.longValue()
	...
	/**
	 * Creates an has map of coupplse <group name,group ID> parsing the
	 * "getent group" command output
	 * 
	 * @return
	 */
	...
	/**
	 * Command "getent group <groupname>" if parameters contain a string
	 * representing the groupname Command "getent group" if parameters is empty or
	 * null
	 * 
	 * @param
	 * @return String[]
			...
			if (stdInput != null) {
				...
				if (stdError != null) {
					while ((errLine = stdError.readLine()) != null) {
						log.warn("User Info Command Output contains an ERROR message "
							+ errLine);
		...
		if ((fields != null) && (fields.length > 2) && (fields[2] != null)) {
			...
			try {
				...
				if (groupLong.intValue() == groupLong.longValue()) {
					...
					// The number in the output string does not fits in an integer (is
					// between 17 and 32 bits)
					log.warn("Group named '" + fields[2] + "' has a 32 bit GID "
						+ groupLong.longValue()
						+ " . Long GID are not managed by LCMAPS. Ignoring the group");
				}
			} catch (NumberFormatException nfe) {
