...
public class XmlRpcTokenFilter implements Filter {
  ...
  /** */
  public static final Pattern STORM_HEADER_PATTERN = Pattern.compile(STORM_HEADER_PATTERN_STRING);
  ...
  /**
   * Pass over to the next filter if and only if the request contains a security token that match
   * the expected.
   */
  @Override
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (token == null) {
      ...
      log.error(
          "The XML-RPC request security token is missing. The calling service is probably misconfigured.");

      ((HttpServletResponse) response)
          .getWriter()
          .write(prepareXml("The request security token is missing"));
    ...
    if (!token.equals(this.secret)) {
      ...
      log.error(
          "The XML-RPC request security token does not match. The calling service is probably misconfigured.");

      ((HttpServletResponse) response)
          .getWriter()
          .write(prepareXml("The request security token does not match"));
