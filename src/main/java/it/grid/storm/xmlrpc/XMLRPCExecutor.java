...
public class XMLRPCExecutor {
  ...
  private static ArrayList<BookKeeper> bookKeepers =
      HealthDirector.getHealthMonitor().getBookKeepers();
  ...
  public Map<String, Object> execute(OperationType type, Map<String, Object> inputParam)
    ...
    try {
      ...
      log.error(
          "Unable to process the request. Error from the SynchcallDispatcher. IllegalArgumentException: {}",
          e.getMessage(), e);
      throw new StoRMXmlRpcException(
          "Unable to process the request. IllegalArgumentException: " + e.getMessage());
    } catch (CommandException e) {
      log.error(
          "Unable to execute the request. Error from the SynchcallDispatcher. CommandException: {}",
          e.getMessage(), e);
      throw new StoRMXmlRpcException(
    ...
    logExecution(convertOperationType(type), DataHelper.getRequestor(inputData), startTime,
        TimeUnit.NANOSECONDS.toMillis(duration), outputData.isSuccess());
   ...
   */
  private void logExecution(it.grid.storm.health.OperationType opType, String dn, long startTime,
  ...
  /**
   * TOREMOVE! this is a temporary code since two different class of OperationTYpe are defined. This
   * is to convert the two kind of operation type, from the onw used here, enum based, to the one
