 ...
 * Type that can be used to invoke the GetSpaceMetaDataManager
 * 
 * @author lucamag
 * @date May 29, 2008
 * 
...
public class GetSpaceMetaDataConverter implements Converter {
	...
	private static final Logger log = LoggerFactory
		.getLogger(GetSpaceMetaDataConverter.class);
	 ...
	 * an xmlrpc spaceReservation v2.2 call. SpaceResData can be used to invoke
	...
	public InputData convertToInputData(Map inputParam) {
		...
		if (guser != null) {
			inputData = new IdentityGetSpaceMetaDataInputData(guser,
				arrayOfSpaceTokens);
		} else {
	...
	public Map convertFromOutputData(OutputData data) {
		...
		log
			.debug("GetSpaceMetaDataConverter: Creation of XMLRPC Output Structure! ");
		...
		ArrayOfTMetaDataSpace arrayOfSpaceDetails = outputData
			.getMetaDataSpaceArray();
		if (arrayOfSpaceDetails != null) {
			arrayOfSpaceDetails.encode(outputParam,
				ArrayOfTMetaDataSpace.PNAME_ARRAYOFSPACEDETAILS);
		}
