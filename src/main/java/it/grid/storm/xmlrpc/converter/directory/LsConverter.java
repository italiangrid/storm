 ...
 *
 * <p>This class represents the Type Converter for LS function . This class have get an input data
 * from xmlrpc call anc convert it into a StoRM Type that can be used to invoke the LSManager
 *
 * <p>Authors:
 *
...
public class LsConverter implements Converter {
  ...
  /**
   * This method return a LSInputData created from input Hashtable structure of an xmlrpc
   * spaceReservation v2.1 call. SpaceResData can be used to invoke LS method of Directory Functions
   * Manager
   */
  public InputData convertToInputData(Map inputParam) {

    log.debug(
        "SrmLs: LSConverter :Call received :Creation of SpaceResData = {}", inputParam.size());
    log.debug(
        "SrmLs: LSConverter: Input Structure toString: {}",
    ...
    TFileStorageType fileStorageType =
        TFileStorageType.decode(inputParam, TFileStorageType.PNAME_FILESTORAGETYPE);
    ...
    if (guser != null) {
      inputData =
          new IdentityLSInputData(
              guser, surlArray, fileStorageType, fullDL, allLR, numOL, offset, count);
    } else {
      inputData =
          new AnonymousLSInputData(surlArray, fileStorageType, fullDL, allLR, numOL, offset, count);
    }
    return inputData;
  }
