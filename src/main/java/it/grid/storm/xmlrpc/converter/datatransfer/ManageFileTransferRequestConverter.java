...
public class ManageFileTransferRequestConverter extends
	ManageFileTransferConverter implements Converter {

	static final Logger log = LoggerFactory
		.getLogger(ManageFileTransferRequestConverter.class);

	/**
	 * This method returns a ReleaseFilesInputData created from the input
	 * Hashtable structure of an xmlrpc ReleaseFiles v2.2 call.
	 * 
	 * @param inputParam
		...
		if (guser != null) {
			if (requestToken != null) {
				if (arrayOfSURLs != null && arrayOfSURLs.size() > 0) {
					inputData = new IdentityManageFileTransferRequestFilesInputData(
						guser, requestToken, arrayOfSURLs);
				} else {
					inputData = new IdentityReleaseRequestInputData(guser, requestToken);
				}
			} else {
				inputData = new IdentityManageFileTransferFilesInputData(guser,
					arrayOfSURLs);
			}
		} else {
			if (requestToken != null) {
				if (arrayOfSURLs != null && arrayOfSURLs.size() > 0) {
					inputData = new AnonymousManageFileTransferRequestFilesInputData(
						requestToken, arrayOfSURLs);
				} else {
