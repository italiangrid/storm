...
public abstract class FileTransferRequestInputConverter implements Converter {
  ...
  public InputData convertToInputData(Map<String, Object> inputParam)
    ...
    if (surl == null) {
      log.error(
          "Missing mandatory parameter '{}' Unable to build FileTransferInputData",
          TSURL.PNAME_SURL);
      throw new IllegalArgumentException(
          "Missing mandatory parameter \'" + TSURL.PNAME_SURL + "\'");
    }
    ...
    if (transferProtocols == null) {
      ...
      throw new IllegalArgumentException(
          "Missing mandatory parameter \'" + TURLPrefix.PNAME_TURL_PREFIX + "\'");
    }
    ...
    try {
      ...
      log.error(
          "Unable to build FileTransferInputData. IllegalArgumentException: {}", e.getMessage(), e);
  ...
  public Map<String, Object> convertFromOutputData(OutputData outputData)
    ...
    if (surl == null
        || surl.isEmpty()
        || surl.getSURLString().trim().isEmpty()
        || turl == null
        || status == null
        || requestToken == null
        || requestToken.getValue() == null
        || requestToken.getValue().isEmpty()) {
      log.error(
          "Unable to build a valid output map. Missing mandatory values from FileTransferOutputData: {}",
  ...
  protected TSURL decodeSURL(Map<String, Object> inputParam) throws IllegalArgumentException {
    ...
    try {
      ...
      log.error(
          "Unable to decode '{}' parameter as TSURL. InvalidTSURLAttributesException: {}",
          TSURL.PNAME_SURL,
          e.getMessage(),
          e);
      throw new IllegalArgumentException(
          "Unable to decode \'" + TSURL.PNAME_SURL + "\' parameter as TSURL");
    }
    return surl;
  }
}
