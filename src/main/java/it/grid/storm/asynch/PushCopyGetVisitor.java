...
public class PushCopyGetVisitor implements CopyVisitor
{
    ...
    public Copy.Result visit(VisitableCopy copy)
    {
        try
        {
            RequestSummaryData ptgrsd = new RequestSummaryData(TRequestType.PREPARE_TO_GET,
                                                                             ...
                                                                             new TReturnStatus(
                                                                                               ...
                                                                                               "PushCopyChunk has queued this local srmPrepareToGet operation; srmCopy request on SURL"
                                                                                                       + copy.getRequestData()
                                                                                                             .getSURL()),
                                                                             TTURL.makeEmpty());
            copy.getLog().debug("executeGetOperation: adding new chunkData to PtGCatalog!");
            PtGChunkCatalog.getInstance().add(ptgChunkData, copy.getGu());
            copy.getLog().debug("executeGetOperation: finished adding to PtGCatalog!");
        ...
        {
            copy.getLog().error("ERROR IN PushCopyChunk! Cannot initiate local PtG! Requested SURL: "
                                        + copy.getRequestData().getSURL());
            copy.getLog().error(e.getMessage(), e);
            return copy.buildOperationResult("Cannot initiate local PtG! " + e, ResultType.GET);
        }
    }
