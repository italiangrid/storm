  ...
  private void manageIsDirectory(BoLPersistentChunkData chunkData) {
    ...
    try {
      ...
      for (StoRI storiChild : storiChildren) {
        try {
          childData = new BoLPersistentChunkData(chunkData.getRequestToken(), storiChild.getSURL(),
              chunkData.getLifeTime(), notDir, chunkData.getTransferProtocols(),
              chunkData.getFileSize(), chunkData.getStatus(), chunkData.getTransferURL(),
          ...
          log.error("ERROR in BoLFeeder! While expanding recursive request, "
              + "it was not possible to create a new BoLChunkData! {}", e.getMessage(), e);
        }
      }
