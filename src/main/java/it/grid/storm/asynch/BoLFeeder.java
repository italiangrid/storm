...
/**
 * This class represents a BringOnLine Feeder: the Feeder that will handle the
 * srmBringOnLine statements. It chops a multifile request, and for each part it
 ...
 * queued requests is decreased, the number of progressing requests is
 * increased, the status of that chunk is changed to SRM_REQUEST_INPROGRESS; the
 * chunk is given to the scheduler for handling. In case the scheduler cannot
 * accept the chunk for any reason, a message with the requestToken and the
 * chunk s data is logged, status of the chunk passes to SRM_ABORTED, and at the
 * end the counters are such that the queued-requests is decreased while the
 * finished-requests is increased.
 * 
 * If the single part of the request DOES have a dirOption set, then it is
 * considered as an expansion job and it gets handled now! So the number of
 * queued requests is decreased and that for progressing ones is increased,
 * while the status is set to SRM_REQUEST_INPROGRESS. Each newly expanded file
 * gets handled as though it were part of the multifile request WITHOUT the
 * dirOption set, so it goes through the same steps as mentioned earlier on;
 * notice that a new entry in the persistence system is created, and the total
 * number of files in this request is updated. Finally the status of this
 * expansion request is set to SRM_DONE, the number of progressing requests is
 * decreased and the number of finished requests is increased.
 * 
 * At the beginning of the expansion stage, some anomalous situations are
 * considered and handled as follows:
 * 
 * (0) In case of internal errors, they get logged and the expansion request
 * gets failed: the status changes to SRM_FAILURE, number of progressing is
 * decreased, number of finished is increased.
 * 
 * (1) The expanded directory is empty: the request is set to SRM_SUCCESS with
 * an explanatory String saying so. The number of progressing is decreased, and
 * the number of finished is increased.
 * 
 ...
 * 
 * (4) No rights to directory: status set to SRM_AUTHORIZATION_FAILURE; number
 * of progressing is decreased; number of finished is increased.
 * 
 * 
 * @author CNAF
 * @date Aug, 2009
...
public final class BoLFeeder implements Delegable {
	...
	/**
	 * Public constructor requiring the RequestSummaryData to which this BoLFeeder
	 * refers, as well as the GridUser. If null objects are supplied, an
	 * InvalidBoLFeederAttributesException is thrown.
	 */
	public BoLFeeder(RequestSummaryData rsd)
		throws InvalidBoLFeederAttributesException {
	...
	/**
	 * This method splits a multifile request as well as exapanding recursive
	 * ones; it then creates the necessary tasks and loads them into the BoL chunk
	 * scheduler.
	 */
	public void doIt() {
		...
		if (chunks.isEmpty()) {
			log
				.warn("ATTENTION in BoLFeeder! This SRM BoL request contained nothing to process! "
					+ rsd.requestToken());
	...
	private void manageChunks(Collection<BoLPersistentChunkData> chunks) {
		...
		for (BoLPersistentChunkData chunkData : chunks) {
			...
			if (TSURL.isValid(chunkData.getSURL())) {
				/*
				 * fromSURL corresponds to This installation of StoRM: go on with
				 * processing!
				 */
	...
	private void manageNotDirectory(BoLPersistentChunkData auxChunkData) {
		...
		try {
			...
			SchedulerFacade.getInstance().chunkScheduler()
	...
	/**
	 * Private method that handles the case of a BoLChunkData having dirOption
	 * set!
	 */
