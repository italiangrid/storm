 ...
 * and file size are evaluated. If no size and no token are available, a mock file is created as
 * placeholder; in case the size is available but there is no token, an implicit space reservation
 * takes place so a special mock file is also produced; an implicit space reservation again is
 * carried out if there is no space token and no known size, but this time a default reserve size is
 * used resulting once more in a special mock file; finally if both space token and size are
...
public class PtP implements Delegable, Chooser, Request {
  ...
  public PtP(PtPData chunkData) throws InvalidRequestAttributesException {
    ...
    if (chunkData == null) {
      throw new IllegalArgumentException(
          "Unable to instanciate the object, invalid arguments: chunkData=" + chunkData);
    }
  ...
  public void doIt() {
    ...
    if (checker.isSURLBusy(rToken, surl)) {
      failure = true;
      requestData.changeStatusSRM_FILE_BUSY(
          "The surl " + surl + " is currently " + "busy (ongoing put requests)");
      log.info("Unable to perform the PTP request, surl busy " + "(ongoing put on the same surl)");
    ...
    if (checker.isSURLPinned(surl)) {
      failure = true;
      requestData.changeStatusSRM_FILE_BUSY(
          "There surl " + surl + " is currently busy (ongoing get requests).");

      log.info("Unable to perform the PTP request, surl busy " + "(ongoing get on the same surl)");
    ...
    try {
      ...
      log.info(
          "Unable to build a stori for surl {} for user {}. " + "UnapprochableSurlExceptions: {}",
          surl,
          user,
          e.getMessage());
    ...
    try {
      ...
      log.error(
          "ATTENTION in PtPChunk! PtPChunk received a SecurityException "
    ...
    if (!exists) {
      ...
      if (mode.equals(TOverwriteMode.ALWAYS) || mode.equals(TOverwriteMode.WHENFILESAREDIFFERENT)) {
        ...
        requestData.changeStatusSRM_DUPLICATION_ERROR(
            "Cannot srmPut file " + "because it already exists!");
        failure = true;

      } else {

        log.error(
            "UNEXPECTED ERROR in PtPChunk! The specified overwrite " + "option '{}' is unknown!",
            mode);
  ...
  /**
   * Private method that manages the case of a SURL referring to a file that does not exist: the
   * steps are the same regardless of the TOverwriteMode!
   */
    ...
    if (requestData instanceof IdentityInputData) {
      decision =
          AuthzDirector.getPathAuthz()
              .authorize(
                  ((IdentityInputData) requestData).getUser(), SRMFileRequest.PTP, fileStoRI);
    } else {
      decision =
          AuthzDirector.getPathAuthz().authorizeAnonymous(SRMFileRequest.PTP, fileStoRI.getStFN());
    }
    if (decision == null) {
   ...
   * TOverwriteMode is set to Always or WhenFilesAreDifferet, and a file already exists. Notice that
   * the logic for permit is exactly the same as that for the case of non existent files! This is
   * because the placeHolder/reserveSpace functions of the FileSystem object, in case of existing
   * files, they erase it and create a new mock file - thereby following the SRM specs that no
   * append is possible. So the only difference between the two cases is the kind of policies that
  ...
  private void manageOverwriteExistingFile(StoRI fileStoRI) {
    ...
    if (requestData instanceof IdentityInputData) {
      decision =
          AuthzDirector.getPathAuthz()
              .authorize(((IdentityInputData) requestData).getUser(), SRMFileRequest.RM, fileStoRI);
    } else {
      decision =
          AuthzDirector.getPathAuthz().authorizeAnonymous(SRMFileRequest.RM, fileStoRI.getStFN());
    }
    if (decision.equals(AuthzDecision.PERMIT)) {
      managePermit(fileStoRI);
    } else {
      if (decision.equals(AuthzDecision.DENY)) {
        requestData.changeStatusSRM_AUTHORIZATION_FAILURE(
            "Write access to " + requestData.getSURL() + " denied!");
        failure = true;
        log.debug(
            "Write access to {} for user {} denied!",
            requestData.getSURL(),
  ...
  private void managePermit(StoRI fileStoRI) {
    ...
    if (!isSpaceAuthorized) {
      ...
      failure = true;
      log.debug(
          "Create/Write access for {} in Storage Area: {} denied!", requestData.getSURL(), token);
      return;
    }
    TTURL auxTURL;
    try {
      ...
      failure = true;
      log.error(
          "ERROR in PtPChunk! Null TURLPrefix in PtPChunkData caused "
      ...
      failure = true;
      log.error(
          "ERROR in PtPChunk! There was a failure building the TURL. "
        ...
        if (!canWrite) {
          ...
          if (requestData.fileStorageType().equals(TFileStorageType.VOLATILE)) {
            VolatileAndJiTCatalog.getInstance()
                .trackVolatile(
                    fileStoRI.getPFN(), Calendar.getInstance(), requestData.fileLifetime());
          }
        }
      } else {
        // URGENT!!!
  ...
  private boolean prepareDirectory(LocalFile dir) {
    ...
    if (dir.exists()) {
      if (!dir.isDirectory()) {
        requestData.changeStatusSRM_INVALID_PATH(
            dir.getAbsolutePath() + " exists but is not a directory!");
        failure = true;
        return false;
      }
      return true;
    ...
    if (!automaticDirectoryCreation) {
      log.debug(
          "srmPtP: {} doesn't exist and automatic directory creation is " + "disabled",
          dir.getAbsolutePath());
      requestData.changeStatusSRM_INVALID_PATH(
          "Parent " + dir.getAbsolutePath() + " doesn't exist!");
      failure = true;
      return false;
    }
    ...
    try {
      ...
      failure = true;
      log.error(
          "ERROR in PtPChunk! Filesystem was unable to successfully " + "create directory: {}",
          dir);
      return false;
    }
    updateUsedSpace(dir);
  ...
  private boolean setParentAcl(StoRI fileStoRI, LocalUser localUser) {
    ...
    log.debug(
        "PtPChunk: setting parent traverse ACL for {} to user {}",
        fileStoRI.getAbsolutePath(),
        localUser);
    ...
    for (StoRI parentStoRI : fileStoRI.getParents()) {
      ...
      try {
        if (!setAcl(
            parentStoRI, localUser, FilesystemPermission.Traverse, fileStoRI.hasJustInTimeACLs())) {
          requestData.changeStatusSRM_FAILURE(
        ...
        requestData.changeStatusSRM_INTERNAL_ERROR(
            "Local filesystem has" + " problems manipulating ACE!");
        failure = true;
        return false;
      }
    }
  ...
  private boolean managePermitSetFileStep(StoRI fileStoRI) throws CannotMapUserException {
    ...
    if (requestData instanceof IdentityInputData) {
      if (managePermitSetFileStep(
          fileStoRI, ((IdentityInputData) requestData).getUser().getLocalUser())) {
  ...
  private boolean managePermitSetFileStep(StoRI fileStoRI, LocalUser localUser) {
    ...
    log.debug(
        "PtPChunk: setting RW ACL for {} for user {}", fileStoRI.getAbsolutePath(), localUser);

    try {
      if (!setAcl(
          fileStoRI, localUser, FilesystemPermission.ReadWrite, fileStoRI.hasJustInTimeACLs())) {
        requestData.changeStatusSRM_FAILURE(
            "Local filesystem mask does " + "not allow setting up correct ACLs for PtP!");
        failure = true;
        return false;
      }
    } catch (Exception e) {
  ...
  private boolean setJiTAcl(StoRI fileStori, LocalUser localUser, FilesystemPermission permission)
      throws Exception {

    log.debug(
        "SrmMkdir: Adding JiT ACL {} to user {} for directory: '{}'",
        permission,
        localUser,
        fileStori.getAbsolutePath());
    ...
    try {
      AclManagerFS.getInstance()
          .grantUserPermission(fileStori.getLocalFile(), localUser, permission);
    } catch (IllegalArgumentException e) {
      log.error(
          "Unable to grant user traverse permission on parent file. "
    ...
    if (fp != null) {
      if (fp.allows(permission)) {
        VolatileAndJiTCatalog.getInstance()
            .trackJiT(fileStori.getPFN(), localUser, permission, start, requestData.pinLifetime());
        response = true;
      } else {
        log.error(
            "ATTENTION in PtPChunk! The local filesystem has"
      ...
      log.error(
          "ERROR in PtPChunk! A {} User-ACL was set on {} for user {} but "
  ...
  private boolean setAoTAcl(StoRI fileStori, LocalUser localUser, FilesystemPermission permission)
    ...
    log.debug(
        "SrmMkdir: Adding AoT ACL {} to user {} for directory: '{}'",
        permission,
        localUser,
        fileStori.getAbsolutePath());
    ...
    try {
      AclManagerFS.getInstance()
          .grantGroupPermission(fileStori.getLocalFile(), localUser, permission);
    } catch (IllegalArgumentException e) {
      log.error(
          "Unable to grant user traverse permission on parent file. "
              + "IllegalArgumentException: {}",
    ...
    if (fp != null) {
      if (fp.allows(permission)) {
        ...
        log.error(
            "ATTENTION in PtPChunk! The local filesystem has a mask"
      ...
      log.error(
          "ERROR in PtPChunk! A Traverse Group-ACL was set on {} for "
  ...
  private void setHttpsServiceParentAcl(StoRI fileStoRI) {
    ...
    log.debug(
        "SrmMkdir: Adding parent https ACL for directory: '{}' parents",
        fileStoRI.getAbsolutePath());
    for (StoRI parentStoRI : fileStoRI.getParents()) {
  ...
  private void setHttpsServiceAcl(LocalFile file, FilesystemPermission permission) {
    ...
    try {
      ...
      log.error(
          "Unable to grant user permission on the created folder. "
              + "IllegalArgumentException: {}",
          e.getMessage(),
          e);
      requestData
    ...
    if (fs != null && fs.getProperties().isOnlineSpaceLimited()) {
      ...
      if ((sp.isSAFull(PtP.log, fileStoRI))
        ...
        if (SASpaceToken == null || SASpaceToken.isEmpty()) {
          log.error(
              "PtPChunk - ReserveSpaceStep: Unable to get a valid "
          ...
          if (!sp.isSAInitialized(PtP.log, fileStoRI)
            ...
            log.debug(
                "PtPChunk: ReserveSpaceStep: the storage area space "
    ...
    try {
      ...
      if (!fileWasCreated) {
        ...
        if (requestData.overwriteOption().equals(TOverwriteMode.NEVER)) {
          ...
          log.debug(
              "PtP Chunk(localFile={}): error creating local file. " + "Overwrite mode is NEVER.",
              fileStoRI.getAbsolutePath());
      ...
      if (spaceToken.isEmpty() && (!size.isEmpty())) {
        log.debug(
            "PtPChunk:  no SpaceToken available but "
      ...
      if (!spaceToken.isEmpty()) {
        ...
        if (!isExistingSpaceToken(spaceToken)) {
          requestData.changeStatusSRM_INVALID_REQUEST(
              "The provided Space Token " + "does not exists");
          log.info(
              "PtPChunk execution failed. The space token {} provided by " + "user does not exists",
              spaceToken);
      ...
      failure = true;
      log.error(
          "ERROR in PtPChunk! During space reservation step in PtP, "
      ...
      failure = true;
      log.error(
          "ERROR in PtPChunk! During space reservation step in PtP, an "
              + "attempt to create file {} failed because StoRM lacks the privileges "
              + "to do so! Exception follows: {}",
          localFile.toString(),
          e.getMessage(),
          e);
      return false;
      ...
      failure = true;
      log.error(
          "ERROR in PtPChunk! Space component failed! Exception " + "follows: {}",
          e.getMessage(),
          e);
      return false;
    } catch (ExpiredSpaceTokenException e) {
      // The supplied space token is expired
      requestData.changeStatusSRM_SPACE_LIFETIME_EXPIRED(
          "The provided Space " + "Token has expired its lifetime");
      spacefailure = true;
      ...
      failure = true;
      log.error(
          "ERROR in PtPChunk - space Step! Unexpected error in reserve "
    ...
    try {
      ...
      log.error(
          "Unable to build StorageSpaceData from StorageSpaceTO."
              + " TransferObjectDecodingException: {}",
          e.getMessage());
      throw new Exception(
          "Error retrieving Storage Area information from Token."
              + " TransferObjectDecodingException: "
              + e.getMessage());
      ...
      for (ACLEntry ace : dacl.getACL()) {
        if (ace.isValid()) {
          ...
          if (ace.getFilesystemPermission() == null) {
            ...
            try {
              AclManagerFS.getInstance()
                  .grantGroupPermission(fileStoRI.getLocalFile(), u, ace.getFilesystemPermission());
            } catch (IllegalArgumentException e) {
              log.error(
                  "Unable to grant group permission on the file. " + "IllegalArgumentException: {}",
                  e.getMessage(),
                  e);
            }
          }
  ...
  /**
   * Method that handles the case of either Create or Write policies are Deny: beware that only one
   * policy needs to be Deny for this method to get invoked!
   */
  private void manageDeny() {

    requestData.changeStatusSRM_AUTHORIZATION_FAILURE(
        "Create/Write access to " + requestData.getSURL() + " denied!");
    failure = true;
    log.debug(
        "Create/Write access to {}, for user {} denied!",
        requestData.getSURL(),
        DataHelper.getRequestor(requestData));
  }

  /**
   * Private method that handles all cases where: new states are added to AuthorizationCollector but
   * this PtP logic does not get updated, or there are missing policies, or there are problems with
   * the PolicyCollector.
   */
  private void manageAnomaly(AuthzDecision decision) {
    ...
    switch (decision) {
      case NOT_APPLICABLE:
        ...
        failure = true;
        log.error(
            "PtPChunk: PolicyCollector warned of missing policies for the " + "supplied SURL!");
        log.error("Requested SURL: {}", requestData.getSURL());
        break;
      case INDETERMINATE:
        // PolicyCollector error
        requestData.changeStatusSRM_FAILURE(
            "PolicyCollector error! Access rights cannot be established!");
        failure = true;
  ...
  /**
   * Method that supplies a String describing this PtPChunk - for scheduler Log purposes! It returns
   * the request token and the SURL asked for in This request.
   */
  @Override
  ...
  /**
   * Method used in a callback fashion in the scheduler for separately handling PtG, PtP and Copy
   * chunks.
   */
  @Override
  ...
  protected void printRequestOutcome(PtPData inputData) {
    ...
    if (inputData != null) {
      if (inputData.getSURL() != null) {
        ...
        if (inputData.getRequestToken() != null) {
          CommandHelper.printRequestOutcome(
              SRM_COMMAND, log, inputData.getStatus(), inputData, inputData.getRequestToken());
        } else {
