...
public class PtP implements Delegable, Chooser, Request {
  ...
  public void doIt() {
    ...
    if (checker.isSURLBusy(rToken, surl)) {
      failure = true;
      requestData.changeStatusSRM_FILE_BUSY("The surl " + surl
        + " is currently " + "busy (ongoing put requests)");
    ...
    if (checker.isSURLPinned(surl)) {
      failure = true;
      requestData.changeStatusSRM_FILE_BUSY("There surl " + surl
        + " is currently busy (ongoing get requests).");
    ...
    if (!exists) {
      ...
      if (mode.equals(TOverwriteMode.ALWAYS)
        || mode.equals(TOverwriteMode.WHENFILESAREDIFFERENT)) {
    ...
    if (canTraverse) {
      ...
      if (managePermitReserveSpaceStep(fileStoRI)) {
        ...
        if (!canWrite) {
          ...
          log.debug(
            "PTP CHUNK. Addition of ReadWrite ACL on file successfully "
  ...
  private void setHttpsServiceAcl(LocalFile file,
    ...
    log.debug("SrmMkdir: Adding https ACL {} for directory : '{}'", permission,
      file);
    ...
    try {
      ...
      if (!spaceToken.isEmpty()) {
        ...
        if (!isExistingSpaceToken(spaceToken)) {
          requestData
            .changeStatusSRM_INVALID_REQUEST("The provided Space Token "
              + "does not exists");
