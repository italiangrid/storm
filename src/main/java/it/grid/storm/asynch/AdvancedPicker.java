...
public class AdvancedPicker {
  ...
  private final CrusherScheduler s = SchedulerFacade.getInstance().crusherScheduler();
  ...
  /**
   * Method used to command This AdvancedPicker to stop periodic reading of data from the DB. Any
   * current operation continues until the end, but no further reading takes place.
   */
  public void stopIt() {
  ...
  /**
   * Private method that connects to the DB and retrieves requests in status SRM_REQUEST_QUEUED,
  ...
  public void retrieve() {
    ...
    SchedulerStatus status = s.getStatus();
    ...
    try {
      ...
      requests = RequestSummaryCatalog.getInstance().fetchNewRequests(crusherCapacity);

    } catch (Throwable t) {

      log.error("Error fetching new requests from database: {}", t.getMessage(), t);
