...
public class PushCopy extends Copy implements VisitableCopy {
	...
	public PushCopy(GridUserInterface gu, CopyData chunkData, int n)
	 ...
	 * srmCopy, the lifeTime from the srmCopy, empty TSpaceToken, dirOption set to
	 * non directory and non recursive, TURLPrefix with TransferProtocol.FILE,
	 ...
	 * completed before actually starting it; if this preparatory phase cannot be
	 * completed thereby not even allowing the initiation of the local operation,
	 * a Failed GetOperationResult is returned; but if it can start, then a
	 * successful GetOperationResult is returned regardless of the SRM status.
	 */
	@Override
	 ...
	 * operation, the same GridUser that made the srmCopy request, and the toSURL
	 * from the original srmCopy request. The time-out is calculated from the
	 * Configuration class from getSRMClientPutTimeOut(); the polling frequency is
	 * derived from the Configuration class from getSRMClientPutSleepTime().
	 * 
	 ...
	 * containing the TURL and the TReturnStatus; if the operation times out, then
	 * a failed PutOperationResult is returned; in case the SRMClient has any
	 * problem carrying out its functionality, a failed PutOperationResult is
	 * returned containing the error message returned by the exception thrown by
	 * SRMClient.
	 * 
