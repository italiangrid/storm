...
public class PushCopyTransferVisitor implements CopyVisitor {
	...
	public Result visit(VisitableCopy copy) {
		...
		try {
			...
			// do transfer
			GridFTPTransferClient gridFTPTransferClient = GridFTPTransferClientFactory
				.getInstance().client();
			gridFTPTransferClient.putFile(copy.getGu(), getTURL, putTURL);
			// Make an SRMClient
			copy.getLog().debug(
				"PUSH COPY CHUNK - executeTransfer: getting SRM client...");
			...
			try {
				do {
					copy.getLog().debug(
						"PUSH COPY CHUNK - executeTransfer: Going to sleep...");
					try {
					...
					copy
						.getLog()
						.debug(
							"PUSH COPY CHUNK - executeTransfer: Waking up and invoking srmPutDone...");
					reply = srmClient.srmPutDone(put.requestToken(), copy.getGu(), copy
			...
			if (timedOut) {
				...
				copy
					.getLog()
					.warn(
						"File transfer completed successfully, but there was a timeout waiting for srmPutDone to return a status different from SRM_INTERNAL_ERROR!");
				return copy
					.buildOperationResult(
						"File transfer completed successfully, but there was a timeout waiting for srmPutDone to return a status different from SRM_INTERNAL_ERROR!",
						ResultType.TRANSFER);
