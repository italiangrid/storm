...
public class ThirdPartGridFTPTransferClient implements GridFTPTransferClient {
	...
	private static Logger log = LoggerFactory
		.getLogger(ThirdPartGridFTPTransferClient.class);
	 ...
	 */
	public void putFile(GridUserInterface gu, TTURL source, TTURL destination)
		throws GridFTPTransferClientException {
		...
		if (!firstPart || !secondPart) {
			throw new GridFTPTransferClientException(
				"The third part gridFTP trasfer requires both TURL in GSIFTP protocol! (first ="
					+ firstPart + ") second =" + secondPart + ")");
		}
		...
		try {
			...
			GSSCredential cred = new GlobusGSSCredentialImpl(new GlobusCredential(
			...
			sourceClient.extendedTransfer(fullSourceFile, destClient,
				fullDestinationFile, listener);
			...
			throw new GridFTPTransferClientException(
				"Unexpected runtime error in ThirdPartGridFTPTransferClient! " + e);
		}
	}
