 ...
 * of extended features of GridFTP such as parallel streams and transfers
 * between third parties. Moreover, it does not share a data channel for
 * multiple file transfers. The performance, therefore, can be greatly improved
 * upon!
 * 
...
public class NaiveGridFTPTransferClient implements GridFTPTransferClient {
	...
	private static final Logger log = LoggerFactory
		.getLogger(NaiveGridFTPTransferClient.class);

	/**
	 * Implementation of inherited put method: it executes a put of a file as per
	 * contract specified in the interface. In particular
	 * GridFTPTransferClientException is thrown if: the local TURL does not have
	 * file as protocol, or the remote one does not have gsiftp as protocol; no
	 * credentials (proxy) were found, there were problems with the proxy itself,
	 * or there were problems with the security framework that handles the proxy;
	 * a generic input/output problem occured; the GridFTP server reported some
	 * problems; there were client side problems.
	 */
	public void putFile(GridUserInterface gu, TTURL local, TTURL remote)
		...
		if (!localIsFile || !remoteIsGSIFTP) {
			throw new GridFTPTransferClientException(
				"Unsupported local/remote protocol: local-is-file=" + localIsFile
					+ ", remote-is-GSIFTP=" + remoteIsGSIFTP);
		}
		...
		try {
			...
			GridFTPClient client = new GridFTPClient(remote.tfn().machine()
				.getValue(), remotePort);
			client.authenticate(new GlobusGSSCredentialImpl(new GlobusCredential(
			...
			client.setClientWaitParams(Configuration.getInstance()
				.getGridFTPTimeOut(), Session.DEFAULT_WAIT_DELAY);
			...
			throw new GridFTPTransferClientException(
				"Unexpected runtime error in NaiveGridFTPTransferClient! " + e);
		}
	}
}
