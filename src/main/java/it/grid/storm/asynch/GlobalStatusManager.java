...
public class GlobalStatusManager {
  ...
  public synchronized void addChunk(ChunkData c) {
    ...
    if (finished) {
      log.warn(
          "GlobalStatusManager: chunkData NOT added because finishedAdding "
    ...
    if (chunks.contains(Long.valueOf(c.getIdentifier()))) {
      log.warn(
          "GlobalStatusManager: chunkData NOT added because of another "
  ...
  public synchronized void successfulChunk(ChunkData c) {
    ...
    if (c == null) {
      log.error(
          "ERROR IN GLOBAL STATUS EVALUATION! There was an attempt to "
    ...
    if (finished && (chunks.isEmpty())) {
      ...
      log.debug(
          "GlobalStatusManager: still not finished adding chunks for "
      ...
      log.error(
          "ERROR IN GLOBAL STATUS EVALUATION! An impossible logic "
  ...
  private void saveRequestState() {
    ...
    if (internal.equals(InternalState.ERROR)) {
      ...
      retstat =
          new TReturnStatus(
              TStatusCode.SRM_PARTIAL_SUCCESS, "Some chunks were successful while others failed!");
      ...
      retstat =
          new TReturnStatus(
              TStatusCode.SRM_SPACE_LIFETIME_EXPIRED, "Supplied SpaceToken has expired lifetime!");
      ...
      retstat =
          new TReturnStatus(
              ...
              "Global status cannot be evaluated: single file status must be checked.");
      log.error(
          "ERROR IN GLOBAL STATUS EVALUATION! {} was attempted to be "
              + "written into persistence, but it is not a final state!",
          internal);
      log.error("Request: {}", rt);
    }
  ...
  private static class InternalState {
    ...
    public static InternalState IN_PROGRESS =
        new InternalState() {
    ...
    public static InternalState INTERMEDIATE_SUCCESS =
        new InternalState() {
    ...
    public static InternalState INTERMEDIATE_FAIL =
        new InternalState() {
    ...
    public static InternalState ERROR =
        new InternalState() {
    ...
    public static InternalState FAIL =
        new InternalState() {
    ...
    public static InternalState SPACEFAIL =
        new InternalState() {
    ...
    public static InternalState SUCCESS =
        new InternalState() {
    ...
    public static InternalState PARTIAL =
        new InternalState() {
