 ...
 * supplied ChunkData: it encloses the logic for computing such global state. It
 * must be intialised with the TRequestToken of the request; any ChunkData that
 * must be considered in the computation must be added with the addChunk method;
 * when all ChunkData have been added, finishedAdding method must be called.
 * Chunks then must invoke either successfulChunk or failedChunk passing their
 * ChunkData as arguments, in order for the OverallRequestStatus to be updated
 * accordingly. When all chunks complete, the state is updated into persistence
 * automatically. Finally, chunks may also invoke expiredSpaceLifetimeChunk to
 * signal that the associated space token has its lifetime expired. This is a
 * special state: it is not possible to have a request with some chunks in this
 * situation while others are not. All chunks of the same request should invoke
 * this method: it is an anomaly if it does not occur; it gets signalled
 * properly.
 * 
...
public class GlobalStatusManager {
	...
	private static Logger log = LoggerFactory
		.getLogger(GlobalStatusManager.class);
	...
	public GlobalStatusManager(TRequestToken rt)
		throws InvalidOverallRequestAttributeException {
	...
	/**
	 * Method that tells this OverallRequest to consider the state of the supplied
	 * ChunkData, when computing the global state. If finishedAdding method has
	 * already been invoked, this method has no effect.
	 */
	...
	/**
	 * Method used to communicate that the given ChunkData c has transited to a
	 * final successful state. If it is null, there are no Chunks being kept track
	 * of, or that specific ChunkData is not being tracked, an error message gets
	 ...
	 * failed state. If it is null, there are no Chunks being kept track of, or
	 * that specific Chunk is not being tracked, an error message gets written to
	 * the logs and the global state transits to ERROR.
	 */
	 ...
	 * there are no Chunks being kept track of, or that specific Chunk is not
	 * being tracked, an error message gets written to the logs and the global
	...
	/**
	 * Auxiliary private class that keeps track of internal state of request, with
	 * respect to all the chunks.
	 * 
	 * @author EGRID - ICTP Trieste
	 * @version 2.0
	 * @date September, 2006
