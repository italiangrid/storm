...
public class TaskResource {
	...
	/*
	 * Method called by storm Frontend when it find a ptg or bol marked as in
	 * progress on the db (for boot tape enabled and disk only SA)
	 */
	@PUT
	@Path("/")
	@Consumes("text/plain")
	public Response putTaskStatus(InputStream input) {
		...
		PutTapeRecallStatusValidator validator = new PutTapeRecallStatusValidator(
			inputString);
		...
		try {
			...
			log
				.error("Error serving request. TapeRecallException: " + e.getMessage());
			response = Response.status(
				javax.ws.rs.core.Response.Status.INTERNAL_SERVER_ERROR).build();
		}
	...
	public void putNewTaskStatusOrRetryValue(
		@PathParam("groupTaskId") UUID groupTaskId, InputStream input)
		throws TapeRecallException {

		log.debug("Requested to change recall table value for taskId "
			+ groupTaskId);
		...
		try {
			if (!rtCat.existsGroupTask(groupTaskId)) {
				log
					.info("Received a tape recall status update but no Recall Group Task found with ID = '"
						+ groupTaskId + "'");
				throw new TapeRecallException("No Recall Group Task found with ID = '"
					+ groupTaskId + "'");
			}
		} catch (DataAccessException e) {
			log.error("Unable to retrieve Recall Group Task with ID = '"
				+ groupTaskId + "' " + e.getMessage());
			throw new TapeRecallException(
				"Unable to retrieve Recall Group Task with ID = '" + groupTaskId + "' "
					+ e.getMessage());
		}
		...
		if (key.equals(keyRetryValue)) { // **** Set the Retry value
			log.debug("Changing retry attempt of task " + groupTaskId + " to "
				+ intValue);
