...
public class TasksResource {
  ...
  @Consumes("text/plain")
  @Produces("text/plain")
  public Response putTakeoverTasks(InputStream input) {
    ...
    if (eqIndex > 0) {
      ...
      if (key.equals(keyTakeover)) {
        ...
        try {
          ...
          String message = format("Unable to understand the number value = '%s'", value);
          log.error(message, e);
    ...
    log.debug("Number of tasks recalled : <{}> over <{}> tasks requested", groupTasks.size(),
        tasks.size());
  ...
  private TapeRecallTO makeOne(List<TapeRecallTO> recallTasks) {
    ...
    for (TapeRecallTO recallTask : recallTasks) {
      ...
      if (!recallTask.getTaskId().equals(taskId)) {
        ...
        log.error("Received a list of not omogeneous tasks, the taskid '{}' is not matched by : {}",
            taskId, recallTask);
    ...
    for (TapeRecallTO recallTask : recallTasks) {
      ...
      Date currentPinExpirationInstant = new Date(
          recallTask.getDeferredRecallInstant().getTime() + (recallTask.getPinLifetime() * 1000));
