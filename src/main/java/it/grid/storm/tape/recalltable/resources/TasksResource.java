...
public class TasksResource {
	...
	public String putTakeoverTasks(InputStream input) throws TapeRecallException {
		...
		for (ArrayList<TapeRecallTO> groupTaskList : groupTaskMap.values()) {
			try {
				...
				log
					.error("Unable to makeOne the task list . IllegalArgumentException : "
						+ e.getMessage());
				log.error("Erroneous task list (long output): "
					+ groupTaskList.toString());
				log
					.error("Skip the erroneous task list and go on...Please contact StoRM support");
			}
		}
	 ...
	 * @return
	 */
	private HashMap<UUID, ArrayList<TapeRecallTO>> buildGroupTaskMap(
		ArrayList<TapeRecallTO> tasks) {

		HashMap<UUID, ArrayList<TapeRecallTO>> groupTaskMap = new HashMap<UUID, ArrayList<TapeRecallTO>>();
		for (TapeRecallTO task : tasks) {
			ArrayList<TapeRecallTO> taskList = groupTaskMap
				.get(task.getGroupTaskId());
			if (taskList == null) {
	...
	private TapeRecallTO makeOne(ArrayList<TapeRecallTO> recallTasks)
		...
		for (TapeRecallTO recallTask : recallTasks) {
			...
			if (minDeferredRecallInstant == null
				|| recallTask.getDeferredRecallInstant().before(
					minDeferredRecallInstant)) {
				minDeferredRecallInstant = recallTask.getDeferredRecallInstant();
			}
			Date currentPinExpirationInstant = new Date(recallTask
		...
		int pinLifeTime = (int) (maxPinExpirationInstant.getTime() - minDeferredRecallInstant
			.getTime()) / 1000;
