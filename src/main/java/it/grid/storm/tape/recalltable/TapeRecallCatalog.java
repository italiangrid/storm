...
public class TapeRecallCatalog {
    ...
    public List<TapeRecallTO> takeoverTasks(int numberOfTaks, String voName) {
        ...
        try {
            ...
            log.error("Unable to takeover " + numberOfTaks + " tasks for vo " + voName, e);
        }
        return taskList;
    }
    ...
    public UUID insertNewTask(TapeRecallTO task) throws DataAccessException {
        ...
        /* if no tasks are in status queued or in_progress for this taskId (the file to be recalled) then add a new task row and
        * assign if the provided group task id
        * If there are, insert the row setting the status and the group id to the one of the found row
        */
        UUID groupTaskId = tapeRecallDAO.insertCloneTask(task, new int[] {TapeRecallStatus.QUEUED.getStatusId(), TapeRecallStatus.IN_PROGRESS.getStatusId()}, newGroupTaskId);   
    ...
    public boolean changeGroupTaskStatus(UUID groupTaskId, TapeRecallStatus recallTaskStatus, Date timestamp) throws DataAccessException {
        ...
        synchronized (this)
        {
            ...
            if(updated)
            {
                ...
                {
                    log.warn("Setting the status to IN_PROGRESS or QUEUED using setGroupTaskStatus() is not "
                             + "a legal operation, doing it anyway. groupTaskId=" + groupTaskId);
                }
                else
                {
