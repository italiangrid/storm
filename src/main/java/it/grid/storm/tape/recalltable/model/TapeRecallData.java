...
public class TapeRecallData {
	 ...
	 */
	public static TapeRecallData buildFromString(String inputString)
		throws TapeRecallException {
	 ...
	 * @param voName
	 */
	public TapeRecallData(String filename, String dn, String[] fqans,
		String voName) {
	 ...
	 * @return
	 */
	private String parseDN(String taskDataText, int fnPos, int dnPos,
		int fqansPos, int voNamePos) {
		...
		if (result.contains("CN=")) {
			...
			int endIndex = result.contains(",") ? result.indexOf(',') : result
				.length();
	 ...
	 * @return
	 */
	private String parseFN(String taskDataText, int fnPos, int dnPos,
		int fqansPos, int voNamePos) {
	 ...
	 * @return
	 */
	private ArrayList<FQAN> parseFQANs(String taskDataText, int fnPos, int dnPos,
		int fqansPos, int voNamePos) {
		...
		if (fqansPos > 0) {
			...
			fqansSt = fqansSt.substring(TapeRecallBuilder.fqansArrayStart.length(),
				pos);
			log.debug(fqansSt);
	 ...
	 * @return
	 */
	private String parseVOName(String taskDataText, int fnPos, int dnPos,
		int fqansPos, int voNamePos) {
	...
	public String toString() {
		...
		String result = "";
		result += TapeRecallBuilder.fnPrefix + TapeRecallBuilder.equalChar
			+ fileName + "\t";
		result += TapeRecallBuilder.userIdPrefix + TapeRecallBuilder.equalChar
			+ userId + "\t";
		result += TapeRecallBuilder.voNamePrefix + TapeRecallBuilder.equalChar
			+ voName;
		return result;
	}
}
