...
public class DNPattern implements SubjectPattern {
	...
	public DNPattern(String dnPatternString, boolean checkValidity)
	...
	private void generatePattern() {
		...
		if ((this.dnPatternString == null) || (this.dnPatternString.equals("*"))) {
			...
			if (rules != null) {
				...
				for (int i = 0; i < length; i++) {
					...
					if (rules[i].startsWith("O=")) {
						organizationPatternString = rules[i]
							.substring(2, rules[i].length());
					}
					if (rules[i].startsWith("OU=")) {
						organizationalUnitPatternString = rules[i].substring(3,
							rules[i].length());
					}
					...
					if (rules[i].startsWith("DC=")) {
						domainComponentPatternString = rules[i].substring(3,
							rules[i].length());
					}
			...
			init(countryPatternString, organizationPatternString,
				organizationalUnitPatternString, localityPatternString,
				commonNamePatternString, domainComponentPatternString);
		}
	}
	...
	protected void init(String countryPatternString,
		...
		if (organizationalUnitPatternString != null) {
			...
			organizationalUnitPattern = Pattern
				.compile(this.organizationalUnitPatternString);
		} else {
			organizationalUnitPatternString = ADMIT_ALL;
		...
		if (domainComponentPatternString != null) {
			...
			domainComponentPattern = Pattern
				.compile(this.domainComponentPatternString);
		} else {
			domainComponentPatternString = ADMIT_ALL;
	...
	public boolean match(SubjectAttribute subjectAttribute) {
		...
		if (subjectAttribute instanceof DistinguishedName) {
			...
			if (organizationName != null) {
				CharSequence organization = organizationName.subSequence(0,
			...
			if (localityName != null) {
				CharSequence locality = localityName.subSequence(0,
					localityName.length());
			...
			// OU ArrayList
			ArrayList<String> organizationalUnitNames = dn
				.getOrganizationalUnitNames();
			if ((organizationalUnitNames != null)
				&& (!(organizationalUnitNames.isEmpty()))) {
				CharSequence organizationalUnit = null;
				String nameStr = null;
				Matcher organizationalUnitMatcher = null;
				for (Iterator<String> name = organizationalUnitNames.iterator(); name
					.hasNext();) {
					nameStr = name.next();
					organizationalUnit = nameStr.subSequence(0, nameStr.length());
					organizationalUnitMatcher = organizationalUnitPattern
						.matcher(organizationalUnit);
				...
				organizationalUnitMatch = organizationalUnitPatternString
					.equals(ADMIT_ALL);
			}
			if (!(organizationalUnitMatch)) {
				return false;
			...
			if ((domainComponents != null) && (!(domainComponents.isEmpty()))) {
				...
				for (Iterator<String> scanDC = domainComponents.iterator(); scanDC
					.hasNext();) {
					domainComponentStr = scanDC.next();
					domainComponent = domainComponentStr.subSequence(0,
						domainComponentStr.length());
					domainComponentMatcher = domainComponentPattern
						.matcher(domainComponent);
