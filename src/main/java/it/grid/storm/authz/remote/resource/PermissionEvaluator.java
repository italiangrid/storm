...
class PermissionEvaluator {
  ...
  static Boolean evaluateVomsGridUserPermission(
      String DNDecoded, String FQANSDecoded, String filePathDecoded, PathOperation operation) {
    ...
    try {
      ...
      log.error(
          "Unable to determine a VFS that maps the requested file "
              + "path '{}'. NamespaceException: {}",
          filePathDecoded,
          e.getMessage());
    ...
    if (!fileVFS.getCapabilities().getAllManagedProtocols().contains(Protocol.HTTPS)) {
      log.debug(
          "User '{}' is not authorized to access the requeste file '{}' via " + "HTTPS",
          gu,
          filePathDecoded);
    ...
    log.info(
        "Authorization decision for user '{}{}' requesting {} on {} is " + "[{}]",
        DNDecoded,
        FQANSDecoded == null ? "" : " - " + FQANSDecoded,
        operation,
        filePathDecoded,
        decision);
   ...
   */
  static Boolean evaluateVomsGridUserPermission(
      String DNDecoded, String FQANSDecoded, String filePathDecoded, SRMFileRequest request)
      throws WebApplicationException {
    ...
    if (!fileVFS.isApproachableByUser(gu)) {
      log.debug(
          "User '{}' is not authorized to approach the requeste Storage " + "Area '{}'",
          gu,
          fileVFS.getAliasName());
      return Boolean.valueOf(false);
    }
  ...
  static Boolean evaluateAnonymousPermission(String filePathDecoded, PathOperation request) {
    ...
    if (!fileVFS.getCapabilities().getAllManagedProtocols().contains(Protocol.HTTP)) {
      log.debug(
          "The requeste Storage Area '{}' is not approachable via " + "HTTPS",
          fileVFS.getAliasName());
      return Boolean.FALSE;
    }
    log.info(
        "Authorization decision for Anonymous user requesting {} on {} " + "is [{}]",
        request,
        filePathDecoded,
  ...
  static Boolean evaluateAnonymousPermission(String filePathDecoded, SRMFileRequest request) {
    ...
    if (!fileVFS.isApproachableByAnonymous()
      ...
      log.debug(
          "The requeste Storage Area '{}' is not approachable by " + "anonymous users",
          fileVFS.getAliasName());
      return Boolean.valueOf(false);
    }
    StFN fileStFN = buildStFN(filePathDecoded, fileVFS);
    ...
    if (decision.equals(AuthzDecision.PERMIT)) {
      ...
      if (decision.equals(AuthzDecision.DENY)) {
        ...
        if (decision.equals(AuthzDecision.INDETERMINATE)) {
          log.warn(
              "Authorization decision is INDETERMINATE! Unable to "
                  + "determine authorization of the user to perform requested "
                  + "operation on the resource");
          return Boolean.valueOf(false);
        } else {
          log.warn(
              "Authorization decision has an unknown value '{}'! "
  ...
  private static StFN buildStFN(String filePathDecoded, VirtualFS fileVFS)
    ...
    try {
      if (fileVFS != null) {
        ...
        if (VFSRootPath == null) {
          log.error(
              "Unable to build StFN for path '{}'. VFS: {} has null " + "RootPath",
              filePathDecoded,
              fileVFS.getAliasName());
          throw new WebApplicationException(
        ...
        if (VFSMappingRules != null && VFSMappingRules.size() > 0) {
          ...
          if (VFSStFNRoot == null) {
            log.error(
                "Unable to build StFN for path '{}'. VFS: {} has null " + "StFNRoot",
                filePathDecoded,
                fileVFS.getAliasName());
            throw new WebApplicationException(
                Response.status(INTERNAL_SERVER_ERROR)
          ...
          log.error(
              "Unable to determine the StFNRoot for file path's VFS. "
        ...
        log.error(
            "None of the VFS maps the requested file path '{}'. " + "fileVFS is null!",
            filePathDecoded);
        throw new WebApplicationException(
            Response.status(NOT_FOUND)
                .entity("Unable to determine file path\'s associated virtual file system")
    ...
    try {
      ...
      log.error(
          "Unable to build StFN for path '{}'. " + "InvalidStFNAttributeException: {}",
          fileStFNpath,
          e.getMessage());
      throw new WebApplicationException(
          Response.status(INTERNAL_SERVER_ERROR)
  ...
  private static GridUserInterface buildGridUser(String DNDecoded, String[] FQANSArray) {
    ...
    try {
      ...
      log.error(
          ...
          DNDecoded,
          Arrays.toString(FQANSArray),
          e.getMessage());
      throw new WebApplicationException(
          Response.status(BAD_REQUEST)
              .entity(
                  "Unable to build a GridUser for DN \'"
                      + DNDecoded
                      + "\' and FQANS \'"
                      + Arrays.toString(FQANSArray)
                      + "\'. Missing argument(s)")
  ...
  private static GridUserInterface loadVomsGridUser(String dn, String[] fqansStringVector)
    ...
    try {
      ...
      log.error(
          "Unexpected error on voms grid user creation. Contact "
              + "StoRM Support : IllegalArgumentException {}",
          e.getMessage(),
          e);
    }
    return gridUser;
