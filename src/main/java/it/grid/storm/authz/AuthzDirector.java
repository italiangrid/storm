...
public class AuthzDirector {
  ...
  private static Map<TSpaceToken, SpaceAuthzInterface> buildSpaceAuthzsMAP() {
    ...
    HashMap<TSpaceToken, SpaceAuthzInterface> spaceAuthzMap =
        new HashMap<TSpaceToken, SpaceAuthzInterface>();
  ...
  public static SpaceAuthzInterface getSpaceAuthz(TSpaceToken token) {
    ...
    if ((spaceAuthzs != null) && (spaceAuthzs.containsKey(token))) {
      ...
      log.debug(
          "Space Authz related to S.Token ='{}' is '{}'", token, spaceAuthz.getSpaceAuthzID());
    } else {
      log.debug(
          "Space Authz related to S.Token ='{}' does not exists. " + "Use the MOCK one.", token);
    }
    return spaceAuthz;
  }
