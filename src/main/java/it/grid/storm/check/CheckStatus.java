...
public enum CheckStatus {
   ...
   * @param status
   * @param bol
   * @return a CheckStatus that is successful only if bol is true and status is successful
   */
   ...
   * @param status
   * @param otherStatus
   * @return a successful CheckStatus if the provided status and otherStatus are successful, a
   *     failed CheckStatus if at least one of status and otherStatus is failed, a notApplicable
   *     status if status and otherStatus are notApplicable, an indeterminate status otherwise
   */
  public static CheckStatus and(CheckStatus status, CheckStatus otherStatus) {
    ...
    if ((FAILURE.equals(status) && FAILURE.equals(otherStatus))
        || ((FAILURE.equals(status) || FAILURE.equals(otherStatus))
            && (SUCCESS.equals(status) || SUCCESS.equals(otherStatus)))) {
      return FAILURE;
    }
