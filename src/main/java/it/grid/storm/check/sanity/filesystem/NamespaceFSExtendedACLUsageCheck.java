...
public class NamespaceFSExtendedACLUsageCheck implements Check {
	...
	private static final Logger log = LoggerFactory
		.getLogger(NamespaceFSExtendedACLUsageCheck.class);
	...
	private static final GridUserInterface TEST_USER = new FakeGridUser(
		"/C=IT/O=INFN/L=CNAF/CN=Fake User");
	...
	private boolean checkEACL(File file, FilesystemIF filesystem) {
		...
		currentPermission = filesystem.getGroupPermission(TEST_LOCAL_USER,
			file.getAbsolutePath());
		if (currentPermission == null) {
			currentPermission = FilesystemPermission.None;
		}
		...
		oldPermisssion = filesystem.getUserPermission(TEST_LOCAL_USER,
			file.getAbsolutePath());
		if (oldPermisssion == null) {
			oldPermisssion = FilesystemPermission.None;
		}
