...
public class NamespaceFSExtendedAttributeDeclarationCheck implements Check
{
                ...
                else
                {
                    ...
                    for (MtabRow row : rows)
                    {
                        if (fsRootPath.startsWith(row.getMountPoint()))
                        {
                            ...
                            // given the file system specified in the row check if the appropriate flag
                            // enabling EA is set
                            ...
                            switch (fsType)
                            {
                                ...
                                default:
                                {
                                    log.error("Unable to switch on the provided SupportedFSType (unknown) : "
                                            + fsType);
                                    throw new GenericCheckException("Unable to switch on the provided SupportedFSType (unknown) : "
                                            + fsType);
                                }
                            }
    ...
    private CheckStatus checkGPFS(List<String> fsOptions)
    {
        log.debug("Checking gpfs file system estended attribute options against \'" + fsOptions.toString()
                + "\'");
        /*
         * According to Vladimir for GPFS the EA are enabled by default and their status doesn't have any info
         * in mtab
         */
