...
public class NamespaceFSAssociationCheck implements Check {
  ...
  private static final String CHECK_DESCRIPTION =
      "This check verifies that the file system type declared in namespace.xml and phisical filesystem type matches";
   ...
   */
  public NamespaceFSAssociationCheck(Map<String, String> mountPoints, Collection<VirtualFS> vfsSet)
      throws IllegalArgumentException {

    this();
    if (mountPoints == null || vfsSet == null) {
      log.error(
          "Unable to create NamespaceFSAssociationCheck received null "
              + "arguments: mountPoints={} vfsSet={}",
          mountPoints,
          vfsSet);
  ...
  public CheckResponse execute() throws GenericCheckException {
    ...
    for (VirtualFS vfs : vfsSet) {
      ...
      if (!currentResponse) {
        log.error(
            "Check on VFS {} failed. FSType={}, rootPath={}",
            vfs.getAliasName(),
            vfs.getFSType(),
            vfs.getRootPath());
        errorMessage +=
  ...
  private boolean verifyPosixDeclaredFS(String FSType) throws IllegalArgumentException {
    ...
    if (FSType == null) {
      log.error(
          "Unable to check posix filesystem declaration received null " + "argument: FSType={}",
          FSType);
      throw new IllegalArgumentException(
  ...
  private boolean check(String fsRootPath, String fsType, Map<String, String> mountPoints) {
    ...
    try {
      ...
      log.error(
          "unable to build the canonical path for root '{}'. " + "IOException: {}",
          fsRootPath,
          e.getMessage());
      return false;
    }
    ...
    if (mountPointFSType != null) {
      ...
      if (fsType.equals(mountPointFSType)) {
        ...
        log.warn(
            "Mount point File System type {} differs from the declared. " + "Check failed",
            mountPointFSType,
            fsType);
      }
    } else {
