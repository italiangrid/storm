  ...
  public CheckResponse lauchChecks() {
    ...
    for (Check check : checkSchedule) {
      ...
      try {
        ...
        getLogger()
            .warn(
                "Received a GenericCheckException during {} check " + "execution : {}",
                check.getName(),
                e.getMessage());
        response =
            new CheckResponse(
                CheckStatus.INDETERMINATE,
                "Received a GenericCheckException during "
                    + check.getName()
                    + " check execution : "
                    + e.getMessage());
      }
      getLogger().info("Check '{}' response is : {}", check.getName(), response.toString());
