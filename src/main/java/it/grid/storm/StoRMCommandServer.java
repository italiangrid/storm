 ...
 * 
 * If it receives the string START, the picker, the spaceReservationServer and
 * the xmlrpcServer get started; if it receives the string STOP, the picker
 * stops; any other string interrupts the connection to this command server
 * leaving StoRM in whatever state is was on.
 * 
...
public class StoRMCommandServer {
	...
	private enum StormStatus {
		BOOTSTRAPPING("BOOTSTRAPPING"), RUNNING("RUNNING"), STOPPED("STOPPED"), STARTING(
			"STARTING"), STOPPING("STOPPING"), SHUTTING_DOWN("SHUTTING_DOWN"), UNKNOW(
			"UNKNOW");
	...
	/**
	 * Private class that represents a thread that gets started when a new
	 * connection to this CommandServer is made: each client connecting to this
	 * CommandServer gets its own thread for processing the commands sent to StoRM
	 */
	private class CommandExecuterThread extends Thread {
		...
		private Socket socket; // socket receiveing the communication with the
														// client!
		 ...
		 * @param socket
		 */
		private void sendOutputAndClose(String response, BufferedWriter out,
			BufferedReader in, Socket socket) {
	...
	/**
	 * Method that automatically starts a CommandServer listening on the port
	 ...
	 * /home/storm/backend/etc/storm.properties, and refresh rate for checking
	 * changes in the configuration file of 5 seconds. A value of 0 disables
	 * refresh.
	 * 
	 * If no command line parameters are specified, the behaviour is dictated by
	 * the StoRM Class. Please refer there for further information.
	 */
