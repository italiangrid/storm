...
public class RestServer {
  ...
  public RestServer(
      int restServicePort, int maxThreads, int maxQueueSize, boolean isTokenEnabled, String token) {
  ...
  private void configure() {
    ...
    if (isTokenEnabled) {
      ...
      servletContextHandler.addFilter(
          filterHolder, "/metadata/*", EnumSet.of(DispatcherType.REQUEST));
      servletContextHandler.addFilter(
          filterHolder, "/recalltable/*", EnumSet.of(DispatcherType.REQUEST));
    }
    ...
    LOG.info(
        "RESTful services threadpool configured: maxThreads={}, maxQueueSize={}",
        maxThreads,
        maxQueueSize);

    InstrumentedHandler ih =
        new InstrumentedHandler(
            METRIC_REGISTRY.getRegistry(), servletContextHandler, "rest-handler");
