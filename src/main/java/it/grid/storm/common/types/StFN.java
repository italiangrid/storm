...
public class StFN {
	...
	private StFN(ArrayList<String> name, /* boolean startingSlash, */
		boolean empty, boolean dir) {
	...
	public static StFN make(String name) throws InvalidStFNAttributeException {
		...
		return new StFN(normalize(name),/* name.charAt(0)=='/', */false,
			checkDirectory(name));
	}
	...
	public Collection<StFN> getParents() {
		...
		for (int i = 1; i < size; i++) {
			aux.add(new StFN(new ArrayList<String>(name.subList(0, i))/*
																																 * ,this.
																																 * startingSlash
																																 */, false,
				true)); // recall
	...
	public StFN getParent() {
		...
		return new StFN(new ArrayList<String>(name.subList(0, size - 1))/*
																																		 * ,this.
																																		 * startingSlash
																																		 */, false,
			true);
	...
	public String toString() {
		...
		int size = name.size();
		if (size == 0) {
			return "/"; // This is not an Empty StFN because empty==false; yet its
									// elements List is empty: it can only
	 ...
	 * +". getValue(): "+p1.getValue()+". isEmpty should be false: "
	 ...
	 * +p3.hashCode()+". getValue(): "+p3.getValue()+
	 * ". isEmpty should be false: "+p3.isEmpty());
