...
public class SFN {
  ...
  public static SFN makeEmpty() {
    ...
    return new SFN(
        Machine.makeEmpty(), Port.makeEmpty(), EndPoint.makeEmpty(), StFN.makeEmpty(), true);
  }
  ...
  public static SFN makeFromString(String surlString)
    ...
    if (colon > 0) {
      if (question < 0) {
        /*
         * Supplied string does not contain a colon, and does not contain
         * question mark! Treat it as optional port specification, _in_ simple
         * form!
         */
        if ((slash == -1) || (slash == 0)) {
        ...
        /*
         * Supplied string does not contain a colon! Treat it as optional port
         * specification, _in_ query form!
         */
        if ((slash == -1) || (slash == 0) || (slash > question)) {
          /*
           * Slash not found or right at the beginning! Or, slash follows
           * question!
           */
          throw new ParsingSFNAttributesException(
              surlString,
      ...
      if (question < 0) {
        /*
         * Supplied string contains a colon! Treat it as if port _is_ specified,
         * and _not_ in query form!
         */

        if ((colon == 0) || (colon > slash)) {
          /*
           * Solon or slash not found or right at the beginning! or, colon
           * follows slash!
           */
          throw new ParsingSFNAttributesException(
              surlString,
        ...
        if ((colon == 0) || (colon > slash) || (slash > question)) {
          /*
           * Colon or slash not found or right at the beginning! Or, colon
           * follows slash! Or slash follows question!
           */
          throw new ParsingSFNAttributesException(
              surlString,
  ...
  private static SFN makeFromQueryForm(String surlString, int colon, int slash, int question)
    ...
    try {
      ...
      log.warn(
          "SFN: Unable to build -machine- attribute from {}. {}", machineString, e.getMessage());
    }
    if ((colon + 1) == slash) {
    ...
    try {
      ...
      log.warn(
          "SFN: Unable to build -endpoint- attribute from {}. {}", endpointString, e.getMessage());
    }
    ...
    if (question + 5 >= surlString.length()) {
      throw new ParsingSFNAttributesException(
          surlString,
          "String interpreted as omitting the optional port specification, and as referring to query form; but theere is nothing left after the question mark!"); // nothing
      // left
      // after
      // question!!!
    }
  ...
  private static SFN makeFromQueryFormNoPort(String surlString, int question, int slash)
    ...
    try {
      ...
      log.warn("SFN: Unable to build -machine- attribute from {}. {}", machine, e.getMessage());
    }
    // EndPoint
    ...
    if (question + 5 >= surlString.length()) {
      throw new ParsingSFNAttributesException(
          surlString,
          "String interpreted as omitting the optional port specification, and as referring to query form; but nothing left after the question mark!"); // nothing
      // left
      // after
      // question!!!
    }
   ...
   *
   * <p>Original SFN: storage.egrid.it:8444/EGRID/original/data/nyse/file.txt
   *
   * <p>Parent SFNs: storage.egrid.it:8444/EGRID/original/data/nyse
   * storage.egrid.it:8444/EGRID/original/data storage.egrid.it:8444/EGRID/original
   * storage.egrid.it:8444/EGRID
   *
   * <p>An empty collection is returned if any error occurs during creation of parent SFNs. Likewise
   * if This is an EmptySFN.
   */
   ...
   * <p>Original SFN: storage.egrid.it:8444/EGRID/original/data/nyse/file.txt
   *
   * <p>Parent SFN: storage.egrid.it:8444/EGRID/original/data/nyse
   *
   * <p>An empty SFN is returned if any error occurs during creation. Likewise if This is an
   * EmptySFN.
   */
