...
public class ReserveSpaceCommand extends SpaceCommand implements Command {
	...
	private String formatLogMessage(boolean success, GridUserInterface user,
		TSizeInBytes desSize, TSizeInBytes guarSize, TLifeTimeInSeconds lifetime,
		TRetentionPolicyInfo rpinfo, TStatusCode code, String expl) {
	...
	public OutputData execute(InputData indata) {
		...
		try {
			...
			log.error(formatLogMessage(FAILURE, data.getUser(),
				data.getDesiredSize(), data.getGuaranteedSize(),
				data.getSpaceLifetime(), data.getRetentionPolicyInfo(), statusCode,
				explanation));
			return manageError(statusCode, explanation);
		}
		...
		log.debug("Reserve Space File Size :"
			+ spaceSize.getDesiderataSpaceSize().toString());
		try {
			/*
			 * here the file is created, (note that since there is no chance to space
			 * is reserved) reclaim allocated space from gpfs, space is never
			 * allocated!) errors from here on must revert this operation
			 */
		...
		try {
			...
			log.info(formatLogMessage(SUCCESS, data.getUser(), data.getDesiredSize(),
	...
	private boolean checkParameters(IdentityReserveSpaceInputData data) {
		...
		if (data.getUser() == null) {
			...
			log.error(formatLogMessage(FAILURE, null, data.getDesiredSize(),
				data.getGuaranteedSize(), data.getSpaceLifetime(),
		...
		if (data.getRetentionPolicyInfo() == null) {
			log
				.debug("SpaceRes: Invalid request, null TRetentionPolicyInfo specified");
	...
	private void setSpaceFilePermissions(StoRI spaceStori, GridUserInterface user)
		...
		if (localFile == null || localUser == null) {
			log.warn("Unable to setting up the ACL. Null value/s : LocalFile "
				+ localFile + " , localUser " + localUser);
			statusCode = TStatusCode.SRM_INTERNAL_ERROR;
			explanation = "Unable to setting up the ACL ";
			throw new Exception(explanation);
		}
		if (spaceStori.hasJustInTimeACLs()) {
			...
			log.debug("<SpaceResManager>:reserveSpace AddACL for FIle: "
				+ spaceStori.getPFN() + "  " + "USER RW");
			try {
				manager.grantUserPermission(localFile, localUser, fp);
			} catch (IllegalArgumentException e) {
				log
					.error("Unable to grant user permission on space file. IllegalArgumentException: "
						+ e.getMessage());
				statusCode = TStatusCode.SRM_INTERNAL_ERROR;
			...
			// AoT Case
			log.debug("<SpaceResManager>:reserveSpace AddACL for FIle: "
				+ spaceStori.getPFN() + "  " + "GROUP RW");
			try {
				manager.grantGroupPermission(localFile, localUser, fp);
			} catch (IllegalArgumentException e) {
				log
					.error("Unable to grant group permission on the space file. IllegalArgumentException: "
						+ e.getMessage());
				statusCode = TStatusCode.SRM_INTERNAL_ERROR;
				explanation = "Unable to setting up the ACL ";
				throw new Exception(explanation);
	...
	public TReturnStatus resetReservation(TSpaceToken token) {
		...
		if (authorize) {
			...
			if (hasJiTACL) {
				// JiT Case
				log.debug("<SpaceResManager>:reserveSpace AddACL for FIle: " + spacePFN
					+ "  " + "USER RW");
				try {
					AclManager manager = AclManagerFSAndHTTPS.getInstance();
					// TODO ACL manager
					...
					if (localFile == null || localUser == null) {
						...
						try {
							...
							log
								.error("Unable to grant group permission on space file. IllegalArgumentException: "
									+ e.getMessage());
							statusCode = TStatusCode.SRM_INTERNAL_ERROR;
							explanation = "Unable to grant group permission on space file ";
							return manageErrorStatus(statusCode, explanation);
		...
		try {
			...
			log
				.error("Unable to persist the space reservation into the DB. DataAccessException: "
					+ e.getMessage());
			statusCode = TStatusCode.SRM_INTERNAL_ERROR;
	...
	public TReturnStatus updateReservation(TSpaceToken token,
		...
		/*
		 * TODO Update the StorageFile information related to the removed chunk. The
		 * SURL has been associated during the Put operation to the SpaceFile, due
		 * to an abort operation the SURL must be removed from the StorageFile
		 * relation, and the size of the spaceFile have to be updated.
		 */
		//
		try {
			...
			log
				.error("Unable to compute new available space size. InvalidTSizeAttributesException: "
					+ e.getMessage());
			revertOldSpaceFileDeletion(localFile);
			statusCode = TStatusCode.SRM_INTERNAL_ERROR;
