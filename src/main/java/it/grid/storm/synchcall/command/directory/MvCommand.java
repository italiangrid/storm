 ...
 * ICTP/EGRID project This class implements the SrmMv Command.
 *
 * @author lucamag
 * @date May 28, 2008
 */
public class MvCommand extends DirectoryCommand implements Command {
  ...
  public OutputData execute(InputData data) {
    ...
    try {
      if (inputData instanceof IdentityInputData) {
        try {
          ...
          log.info(
              "srmMv: Unable to build a stori for surl {} for user {}. {}",
              fromSURL,
          ...
          log.info(
              ...
              e.getMessage());
          outputData.setStatus(
              CommandHelper.buildStatus(TStatusCode.SRM_INVALID_PATH, e.getMessage()));
          printRequestOutcome(outputData.getStatus(), inputData);
        }
      } else {
        try {
    ...
    try {
      ...
      outputData.setStatus(
          CommandHelper.buildStatus(
              TStatusCode.SRM_INTERNAL_ERROR, "Unable to build StoRI by destination SURL"));
      printRequestOutcome(outputData.getStatus(), inputData);
      return outputData;
    }
    ...
    if (fromStori.getLocalFile().getPath().compareTo(toStori.getLocalFile().getPath()) == 0) {
      outputData.setStatus(
          CommandHelper.buildStatus(
              TStatusCode.SRM_SUCCESS, "Source SURL and target SURL are the same file."));
      printRequestOutcome(outputData.getStatus(), inputData);
      return outputData;
    }
    ...
    if (toStori.getLocalFile().exists()) {
      if (toStori.getLocalFile().isDirectory()) {
        ...
        outputData.setStatus(
            CommandHelper.buildStatus(
                TStatusCode.SRM_DUPLICATION_ERROR, "destination SURL already exists!"));
        printRequestOutcome(outputData.getStatus(), inputData);
        return outputData;
      }
    }
    ...
    if ((sourceDecision.equals(AuthzDecision.PERMIT))
      ...
      log.debug(
          "SrmMv: Mv authorized for user {}. Source: {}. Target: {}",
          DataHelper.getRequestor(inputData),
          fromStori.getPFN(),
          toStori.getPFN());
      ...
      if (returnStatus.isSRM_SUCCESS()) {
        ...
        log.warn(
            "srmMv: <{}> Request for [fromSURL={}; toSURL={}] failed with [status: {}]",
            DataHelper.getRequestor(inputData),
            fromSURL,
            toSURL,
            returnStatus);
  ...
  private void setAcl(StoRI oldFileStoRI, StoRI newFileStoRI, LocalUser localUser) {
    ...
    if (newFileStoRI.hasJustInTimeACLs()) {
      ...
      try {
        AclManagerFS.getInstance()
            .grantHttpsUserPermission(
                newFileStoRI.getLocalFile(), localUser, FilesystemPermission.ReadWrite);
      } catch (IllegalArgumentException e) {
    ...
    if (inputData != null) {
      if (inputData.getFromSURL() != null && inputData.getToSURL() != null) {
        CommandHelper.printRequestOutcome(
            ...
            Arrays.asList(
                new String[] {
                  inputData.getFromSURL().toString(), inputData.getFromSURL().toString()
                }));
      } else {
