 ...
 * @date = Oct 10, 2008
 */
public class ExtendFileLifeTimeCommand extends DataTransferCommand implements
	Command {

	private static final Logger log = LoggerFactory
		.getLogger(ExtendFileLifeTimeCommand.class);
	...
	public OutputData execute(InputData data) {
		...
		if (data instanceof IdentityInputData) {
			...
			outputData.setReturnStatus(CommandHelper.buildStatus(
			...
			printRequestOutcome(outputData.getReturnStatus(),
				(ExtendFileLifeTimeInputData) data);
			return outputData;
		}
		...
		try {
			if (inputData.getArrayOfSURLs() == null) {
				...
				&& !(inputData.getNewFileLifetime().isEmpty())
			...
			ExtendFileLifeTimeCommand.log
				.warn("dataTransferManger: Error creating returnStatus " + e);
		}
		if (globalStatus != null) {
			ExtendFileLifeTimeCommand.log.debug(funcName
				+ globalStatus.getExplanation());
		...
		if ((inputData.getRequestToken() == null)
			&& (inputData.getNewPinLifetime().isEmpty())) {
			...
			try {
				globalStatus = manageExtendPinLifetime(inputData.getRequestToken(),
					inputData.getNewPinLifetime(), inputData.getArrayOfSURLs(), user,
					arrayOfFileStatus);
	...
	/**
	 * Extend the lifetime of a SURL. The parameter details is filled by this
	 * method and contains file level information on the execution of the request.
	 * 
	 * @param newLifetime
	 *          TLifeTimeInSeconds.
	 * @param arrayOfSURLS
	 ...
	 * @param guser
	 *          VomsGridUser.
	 * @param arrayOfFileLifetimeStatus
	 *          . ArrayOfTSURLLifetimeReturnStatus The returned file level
	 *          information.
	 * @return TReturnStatus. The request status.
	 */
	private TReturnStatus manageExtendSURLLifetime(
		TLifeTimeInSeconds newLifetime, ArrayOfSURLs arrayOfSURLS,
		GridUserInterface guser, ArrayOfTSURLLifetimeReturnStatus details,
		...
		for (int i = 0; i < arrayOfSURLS.size(); i++) {
			...
			try {
				try {
					...
					ExtendFileLifeTimeCommand.log.error(
						"Unable to build StoRI by SURL and user", e);
				...
				if (stori != null) {
					...
					if (localFile.exists()) {
						...
						if (volatileInfo.isEmpty()) {
							...
							catalog.trackVolatile(stori.getPFN(),
								(Calendar) volatileInfo.get(0), newLifetime);
					...
					if (fileStatus.getStatusCode().equals(TStatusCode.SRM_SUCCESS)) {
						ExtendFileLifeTimeCommand.log.info("srmExtendFileLifeTime: <"
							+ guser + "> Request for [token:" + requestToken + "] for [SURL:"
							+ surl + "] with [lifetime:" + newLifetime
							+ " ] successfully done with: [status:" + fileStatus + "]");
					} else {
						ExtendFileLifeTimeCommand.log.error("srmExtendFileLifeTime: <"
							+ guser + "> Request for [token:" + requestToken + "] for [SURL:"
							+ surl + "] with [lifetime:" + newLifetime
							+ "] failed with: [status:" + fileStatus + "]");
					}
					TSURLLifetimeReturnStatus lifetimeReturnStatus = new TSURLLifetimeReturnStatus(
						surl, fileStatus, newLifetime, null);
				...
				ExtendFileLifeTimeCommand.log
					.debug("Thrown InvalidTReturnStatusAttributeException");
	...
	/**
	 * Extend the PIN lifetime of a SURL. The parameter details is filled by this
	 * method and contains file level information on the execution of the request.
	 * 
	 * @param requestToken
	 *          TRequestToken.
	...
	private TReturnStatus manageExtendPinLifetime(TRequestToken requestToken,
		TLifeTimeInSeconds newPINLifetime, ArrayOfSURLs arrayOfSURLS,
		GridUserInterface guser, ArrayOfTSURLLifetimeReturnStatus details)
		throws IllegalArgumentException {
			...
			try {
				if (surlFound) {
					...
					if (stori != null) {
						LocalFile localFile = stori.getLocalFile();
						if (localFile.exists()) {
							VolatileAndJiTCatalog catalog = VolatileAndJiTCatalog
								.getInstance();
							List volatileInfo = catalog.volatileInfoOn(stori.getPFN());
					...
					ExtendFileLifeTimeCommand.log.debug("SURL: " + surl.getSURLString()
						+ " NOT FOUND!");
					fileStatusCode = TStatusCode.SRM_INVALID_PATH;
				...
				if (fileStatus.getStatusCode().equals(TStatusCode.SRM_SUCCESS)) {
					ExtendFileLifeTimeCommand.log.info("srmExtendFileLifeTime: <" + guser
						...
						+ "] with [pinlifetime: " + newPINLifetime
						+ "] successfully done with: [status:" + fileStatus.toString()
						+ "]");
				} else {
					ExtendFileLifeTimeCommand.log.error("srmExtendFileLifeTime: <"
						+ guser + "> Request for [token:" + requestToken + "] for [SURL:"
						+ surl + "] with [pinlifetime: " + newPINLifetime
		...
		try {
			...
			// Nothing to do, it will never be thrown
			ExtendFileLifeTimeCommand.log
				.debug("BUG: Unexpected TReturnStatus exception" + e);
		}
		return globalStatus;
	}

	/**
	 * Returns the list of SURLs and statuses (a List of SURLData) belonging to
	 * the request identified by the requestToken.
	 * 
	 * @param requestToken
	 *          TRequestToken
