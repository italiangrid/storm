...
public class ExtendFileLifeTimeCommand extends DataTransferCommand implements Command {
  ...
  public OutputData execute(InputData data) {
    ...
    if (inputData.getArrayOfSURLs() == null) {
      globalStatus =
          new TReturnStatus(
              TStatusCode.SRM_INVALID_REQUEST, "Missing mandatory parameter 'arrayOfSURLs'");
    } else if (inputData.getArrayOfSURLs().size() < 1) {
      globalStatus =
          new TReturnStatus(
              TStatusCode.SRM_INVALID_REQUEST, "Parameter 'arrayOfSURLs': invalid size");
    } else if (!(inputData.getNewPinLifetime().isEmpty())
        && !(inputData.getNewFileLifetime().isEmpty())
        && (inputData.getRequestToken() != null)) {
      globalStatus =
          new TReturnStatus(
              TStatusCode.SRM_INVALID_REQUEST, "Cannot update both FileLifetime and PinLifetime");
    } else if (inputData.getNewPinLifetime().isEmpty()
        && !(inputData.getNewFileLifetime().isEmpty())
        && (inputData.getRequestToken() != null)) {
      globalStatus =
    ...
    if (user == null) {
      ...
      outputData.setReturnStatus(
          CommandHelper.buildStatus(
              TStatusCode.SRM_AUTHENTICATION_FAILURE, "Unable to get user credential!"));
      printRequestOutcome(outputData.getReturnStatus(), inputData);
      outputData.setArrayOfFileStatuses(null);
      return outputData;
    }
    ...
    if ((inputData.getRequestToken() == null) && (inputData.getNewPinLifetime().isEmpty())) {
      ...
      globalStatus =
          manageExtendSURLLifetime(
              inputData.getNewFileLifetime(),
  ...
  private TReturnStatus manageExtendSURLLifetime(
      ...
      try {
        ...
        if (stori != null) {
          ...
          if (fileStatus.getStatusCode().equals(TStatusCode.SRM_SUCCESS)) {
            ExtendFileLifeTimeCommand.log.info(
        ...
        ExtendFileLifeTimeCommand.log.debug(
            "Thrown InvalidTSURLLifetimeReturnStatusAttributeException");
      }
    }
    TReturnStatus globalStatus = null;
    // Set global status
    if (requestFailure) {
      ...
      globalStatus =
          new TReturnStatus(
              TStatusCode.SRM_SUCCESS, "All file requests are successfully completed");
    } else {
      globalStatus =
   ...
   */
  private TReturnStatus manageExtendPinLifetime(
      TRequestToken requestToken,
          ...
          try {
            ...
            log.info(
                String.format(
                    ...
                    surl, guser, e.getClass().getCanonicalName(), e.getMessage()));
          }
          if (stori != null) {
            LocalFile localFile = stori.getLocalFile();
            if (localFile.exists()) {
   ...
   */
  private List<SURLData> getListOfSURLsInTheRequest(
      GridUserInterface user, TRequestToken requestToken)
    ...
    if (!(surlStatusMap == null || surlStatusMap.isEmpty())) {
      for (Entry<TSURL, TReturnStatus> surlStatus : surlStatusMap.entrySet()) {
        listOfSURLsInfo.add(
            new SURLData(surlStatus.getKey(), surlStatus.getValue().getStatusCode()));
      }
    }
    ...
    if (inputData != null) {
      if (inputData.getArrayOfSURLs() != null) {
        if (inputData.getRequestToken() != null) {
          ...
          CommandHelper.printRequestOutcome(
              SRM_COMMAND, log, status, inputData, inputData.getArrayOfSURLs().asStringList());
        }

      } else {
        if (inputData.getRequestToken() != null) {
          CommandHelper.printRequestOutcome(
              SRM_COMMAND, log, status, inputData, inputData.getRequestToken());
        } else {
