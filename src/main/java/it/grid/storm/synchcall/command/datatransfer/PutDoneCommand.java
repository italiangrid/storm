...
public class PutDoneCommand extends DataTransferCommand implements Command {
  ...
  private ManageFileTransferRequestFilesInputData inputDataSanityCheck(InputData inputData)
    ...
    try {
      ...
      log.error(
          "PutDone: Invalid input parameters specified [{}: {}]",
          e.getClass().getName(),
          e.getMessage());
  ...
  private static void printRequestOutcome(
      TReturnStatus status, ManageFileTransferRequestFilesInputData inputData) {
  ...
  private ArrayOfTSURLReturnStatus loadSURLsStatus(
      GridUserInterface user, TRequestToken requestToken, List<TSURL> inputSURLs)
  ...
  public static boolean executePutDone(TSURL surl, GridUserInterface user)
    ...
    try {
      ...
      log.error(
          String.format(
              "User %s is unable to build a stori for surl %s, %s: %s",
