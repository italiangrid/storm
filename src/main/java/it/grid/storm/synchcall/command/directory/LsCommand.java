  ...
  public static final Logger log = LoggerFactory.getLogger(LsCommand.class);
	...
	public OutputData execute(InputData data) {
		...
		if (inputData.getOffset() == null) {
			...
			if (offset < 0) {
				globalStatus = CommandHelper.buildStatus(
					TStatusCode.SRM_INVALID_REQUEST, "Parameter 'offset' is negative");
				printRequestOutcome(globalStatus, inputData);
				outputData.setStatus(globalStatus);
				return outputData;
			}
		...
		for (int j = 0; j < surlArray.size(); j++) {
			...
			if (!surl.isEmpty()) {
				...
				printRequestOutcome(
					CommandHelper.buildStatus(fileLevelStatusCode, fileLevelExplanation),
					inputData);
			}
		...
		if (numberOfReturnedEntries.intValue() >= maxEntries) {
			if (maxEntries < count) {
				globalStatus = CommandHelper.buildStatus(
					TStatusCode.SRM_TOO_MANY_RESULTS, "Max returned entries is: "
	...
	/**
	 * Recursive function for visiting Directory an TMetaDataPath Creation.
	 * Returns the number of file statuses different than SRM_SUCCESS.
	 * 
	 * @param guser
	 * @param stori
	 * @param rootArray
	...
	private int manageAuthorizedLS(LSInputData inputData, StoRI stori,
		ArrayOfTMetaDataPathDetail rootArray, boolean allLevelRecursive,
		int numOfLevels, boolean fullDetailedList, int errorCount,
		int count_maxEntries, int offset, MutableInt numberOfResults,
		int currentLevel, MutableInt numberOfIterations) {

		/**
		 * @todo In this version the FileStorageType field is not managed even if it
		 *       is specified.
		 */
		...
		/**
		 * The recursive idea is: - if the StoRI is a directory, fill up with
		 * details, calculate the first level children and for each recurse on. - it
		 * the StoRI is a file, fill up with details and return. Please note that
		 * for each level the same ArrayOfTMetaData is passed as parameter, in order
		 * to collect results. this Array is referenced in the currentTMetaData
		 * element.
		 */
			...
			if (localElement.isDirectory()) {
				...
				if (checkAnotherLevel(allLevelRecursive, numOfLevels, currentLevel)) {
					...
					for (StoRI item : childrenArray) {
						...
						manageAuthorizedLS(inputData, item, currentMetaDataArray,
							allLevelRecursive, numOfLevels, fullDetailedList, errorCount,
							count_maxEntries, offset, numberOfResults, currentLevel + 1,
							numberOfIterations);
					} // for
				}
	 ...
	 *          TMetaDataPathDetail
	 */
	private void populateDetailFromFS(StoRI element,
	 ...
	 * SRM_SPACE_AVAILABLE, false otherwise. This method queries the DB, therefore
	...
	private void fullDetail(LSInputData inputData, StoRI stori,
		...
		if (inputData instanceof IdentityInputData) {
			fullDetail(stori, ((IdentityInputData) inputData).getUser(),
	...
	private void fullDetail(StoRI element, GridUserInterface guser,
		TMetaDataPathDetail elementDetail) throws FSException {
		...
		try {
			...
			if (element.hasJustInTimeACLs()) {
				permission = element.getLocalFile().getUserPermission(
					guser.getLocalUser());
			} else {
				permission = element.getLocalFile().getGroupPermission(
					guser.getLocalUser());
			}
			if (permission != null) {
				userPermission = new TUserPermission(new TUserID(guser.getLocalUser()
					.getLocalUserName()), TPermissionMode.getTPermissionMode(permission));
				groupPermission = new TGroupPermission(new TGroupID(guser
					.getLocalUser().getLocalUserName()),
	...
	/**
	 * Set full details into "elementDetail". Information details set by the
	 * function populateDetailFromFS() are not considered.
	 * 
	 * @param element
	 *          StoRI
	 * @param localElement
	 ...
	 *          TMetaDataPathDetail
	 */
	private void fullDetail(StoRI element, TMetaDataPathDetail elementDetail)
		throws FSException {
		...
		if (localElement.isDirectory()) {
			...
			boolean isTapeEnabled = element.getVirtualFileSystem()
				.getStorageClassType().isTapeEnabled();
			if (isTapeEnabled) {
			...
			if (element.getFileStartTime() != null) {
				elementDetail.setLifetimeLeft(element.getFileLifeTime().timeLeft(
					element.getFileStartTime()));
			} else {
			...
			if (!(checksums.isEmpty())) {
				...
				try {
					ChecksumAlgorithm chkType = ChecksumAlgorithm
						.getChecksumAlgorithm(cksmAlg);
	...
	private Map<String, String> retrieveChecksum(LocalFile localFile,
		boolean tapeEnabled, boolean fileOnDisk) {

		HashMap<String, String> checksums = new HashMap<String, String>();
		if (localFile.hasChecksum()) {
			String checksum = localFile.getDefaultChecksum();
			if (checksum != null) {
				checksums.put(ChecksumManager.getInstance().getDefaultAlgorithm(),
					checksum);
			}
	...
	private void populateFileDetailsFromPersistence(StoRI element,
		...
		// TFileStorageType
		boolean isVolatile = VolatileAndJiTCatalog.getInstance().exists(
			element.getPFN());
		if (isVolatile) {
	 ...
	 * @return boolean
	 */
	private boolean checkAnotherLevel(boolean allLevelRecursive, int numOfLevels,
		int currentLevel) {
