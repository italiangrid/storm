...
public class PrepareToPutRequestStatusCommand extends FileTransferRequestStatusCommand
    implements Command {
  ...
  protected TReturnStatus computeRequestStatus(ArrayOfTSURLReturnStatus arrayOfFileStatuses) {
    ...
    if (((atLeastOneSuccessOrSpaceAvailable || atLeastOneAborted || atLeastOneFailed)
            && atLeastOneQueued)
    ...
    if (atLeastOneSuccessOrSpaceAvailable
      ...
      return CommandHelper.buildStatus(
          TStatusCode.SRM_PARTIAL_SUCCESS, "Partial success, some surls are failed");
    }
