...
public class RmdirCommand extends DirectoryCommand implements Command {
  ...
  private RmdirOutputData doRmdir(RmdirInputData data) {
    ...
    try {
      ...
      log.debug("srmRmdir: rmdir authorized for {}. Dir={}. Recursive={}", userToString(user),
          stori.getPFN(), recursion);
  ...
  private StoRI resolveStoRI(TSURL surl, GridUserInterface user) throws RmdirException {
    ...
    try {
      ...
      throw new RmdirException(TStatusCode.SRM_AUTHORIZATION_FAILURE, e.getMessage());
    } catch (NamespaceException e) {
      log.error(formatStr, surl, userToString(user), e.getMessage());
  ...
  private void checkUserAuthorization(StoRI stori, GridUserInterface user) throws RmdirException {
    ...
    if (!isSpaceAuthorized) {
      log.debug(
          "srmRmdir: User not authorized to perform srmRmdir request " + "on the storage area: {}",
          token);
      throw new RmdirException(TStatusCode.SRM_AUTHORIZATION_FAILURE,
          "User is not authorized to remove the directory on the storage area " + token);
    }
    ...
    if (isAnonymous(user)) {
      ...
      decision = AuthzDirector.getPathAuthz().authorize(user, SRMFileRequest.RMD, stori);
    }
  ...
  private TSURL getSURL(RmdirInputData data) throws RmdirException {
    ...
    if (surl == null) {
      throw new RmdirException(TStatusCode.SRM_FAILURE, "SURL specified is NULL");
    }
    if (surl.isEmpty()) {
      throw new RmdirException(TStatusCode.SRM_FAILURE, "SURL specified is empty");
    }
    return surl;
  }
  ...
  private void decreaseUsedSpace(LocalFile localFile, long sizeToRemove) throws NamespaceException {
    ...
    Namespace.getInstance().resolveVFSbyLocalFile(localFile).decreaseUsedSpace(sizeToRemove);
  }
  ...
  private TReturnStatus removeFolder(LocalFile dir, boolean recursive, TSize size)
    ...
    /*
     * Check if dir exists and is a directory, if recursion is enabled when directory is not empty,
     * etc...
     */

    if (!dir.exists()) {
      return new TReturnStatus(TStatusCode.SRM_INVALID_PATH, "Directory does not exists");
    }
    if (!dir.isDirectory()) {
      return new TReturnStatus(TStatusCode.SRM_INVALID_PATH, "Not a directory");
    }
    if (!recursive && (dir.listFiles().length > 0)) {
      return new TReturnStatus(TStatusCode.SRM_NON_EMPTY_DIRECTORY, "Directory is not empty");
    }
  ...
  private void removeEmptyDirectory(LocalFile directory, TSize size) throws RmdirException {
