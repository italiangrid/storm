...
public class RmdirCommand extends DirectoryCommand implements Command {
  ...
  private RmdirOutputData doRmdir(RmdirInputData data) {
    ...
    try {
      ...
      log.debug(
          "srmRmdir: rmdir authorized for {}. Dir={}. Recursive={}",
          userToString(user),
          stori.getPFN(),
          recursion);
  ...
  private void checkUserAuthorization(StoRI stori, GridUserInterface user) throws RmdirException {
    ...
    if (!decision.equals(AuthzDecision.PERMIT)) {
      ...
      throw new RmdirException(
          TStatusCode.SRM_AUTHORIZATION_FAILURE, "User is not authorized to remove the directory");
    }
    return;
  }
  ...
  private void removeFile(LocalFile file, TSize size) throws RmdirException {
    ...
    if (!file.delete()) {
      ...
      throw new RmdirException(
          TStatusCode.SRM_FAILURE, "Unable to delete " + file.getAbsolutePath());
    }
    size.add(fileSize);
  }
