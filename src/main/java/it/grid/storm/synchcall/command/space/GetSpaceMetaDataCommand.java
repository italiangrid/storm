 ...
 * ICTP/EGRID project
 *
 * <p>This class represents the GetSpaceMetaDataManager Class. This class hava a reseveSpace method
...
public class GetSpaceMetaDataCommand extends SpaceCommand implements Command {
  ...
  public OutputData execute(InputData indata) {
    ...
    for (TSpaceToken token : data.getSpaceTokenArray().getTSpaceTokenArray()) {
      ...
      try {
        ...
        metadata =
            createFailureMetadata(
                ...
                "Error building space data from row DB data",
                data.getUser());
        errorCount++;
      ...
      if (spaceData != null) {
        ...
        metadata =
            createFailureMetadata(
                token, TStatusCode.SRM_INVALID_REQUEST, "Space Token not found", data.getUser());
        errorCount++;
      }
      arrayData.addTMetaDataSpace(metadata);
    }
    ...
    if (requestSuccess) {
      ...
      if (requestFailure) {
        ...
        globalStatus =
            new TReturnStatus(
                TStatusCode.SRM_PARTIAL_SUCCESS, "Check space tokens statuses for details");
  ...
  private TMetaDataSpace createFailureMetadata(
      TSpaceToken token, TStatusCode statusCode, String message, GridUserInterface user) {
