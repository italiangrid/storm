...
public class PingCommand extends DiscoveryCommand implements Command {
		...
		try {
			...
			if (tasks != null) {
				...
				for (TapeRecallTO tapeRecallTO : tasks) {
					otherInfo = new TExtraInfo(tapeRecallTO.getTaskId().toString(),
						tapeRecallTO.toString());
					arrayResult.addTExtraInfo(otherInfo);
				}
			}
		} catch (InvalidTExtraInfoAttributeException e) {
	...
	private enum SpecialKey {
		...
		ALL("all", "return all the pair <key,value> defined in properties"), BE_OS_PLATFORM(
			...
			"returns the operating system kernel release"), TEST_TAKEOVER(
			"take-over", "testing the take-over method"), TEST_POST_NEW_TASK(
			"new-task", "testing the take-over method"), TEST_PUT_NEW_STATUS(
			"new-status", "testing the take-over method"), TEST_PUT_RETRY_VALUE(
			"retry-value", "testing the take-over method"), UNKNOWN("unknown",
