...
public class SurlStatusManager {
	...
	private static final Logger log = LoggerFactory
		.getLogger(SurlStatusManager.class);
	...
	public static void checkAndUpdateStatus(TRequestToken requestToken,
		...
		TRequestType requestType = isPersisted(requestToken);
		if (!requestType.isEmpty()) {
			checkAndUpdatePersistentStatus(requestType, requestToken,
				expectedStatusCode, newStatusCode, explanation);
		} else {
			SurlStatusStore.getInstance().checkAndUpdate(requestToken,
				expectedStatusCode, newStatusCode, explanation);
		}
	}

	public static void checkAndUpdateStatus(TRequestToken requestToken,
		List<TSURL> surls, TStatusCode expectedStatusCode, TStatusCode newStatusCode)
		throws IllegalArgumentException, UnknownTokenException,
		...
		TRequestType requestType = isPersisted(requestToken);
		if (!requestType.isEmpty()) {
			checkAndUpdatePersistentStatus(requestType, requestToken, surls,
				expectedStatusCode, newStatusCode);
		} else {
			SurlStatusStore.getInstance().checkAndUpdate(requestToken, surls,
				expectedStatusCode, newStatusCode);
		}
	}
	...
	public static void checkAndUpdateStatus(TRequestType requestType, TSURL surl,
		TStatusCode expectedStatusCode, TStatusCode newStatusCode,
		String explanation) throws IllegalArgumentException, UnknownSurlException {
		...
		if (!requestType.isEmpty()) {
			checkAndUpdatePersistentStatus(requestType, surl, expectedStatusCode,
				newStatusCode, explanation);
		} else {
			SurlStatusStore.getInstance().checkAndUpdate(surl, expectedStatusCode,
				newStatusCode, explanation);
		}
	}

	public static void updateStatus(TRequestToken requestToken, TSURL surl,
		TStatusCode statusCode, String explanation)
		throws IllegalArgumentException, UnknownTokenException,
		ExpiredTokenException, UnknownSurlException {

		if (requestToken == null || surl == null || statusCode == null
			|| explanation == null) {
			throw new IllegalArgumentException(
				"unable to check and update the statuses, "
		...
		TRequestType requestType = isPersisted(requestToken);
		if (!requestType.isEmpty()) {
			updatePersistentStatus(requestType, requestToken, surl, statusCode,
				explanation);
		} else {
			SurlStatusStore.getInstance().update(requestToken, surl, statusCode,
				explanation);
		}
	}

	public static void updateStatus(TRequestType requestType, TSURL surl,
		TStatusCode statusCode, String explanation) throws UnknownSurlException {

		if (requestType == null || surl == null || statusCode == null
			|| explanation == null) {
			throw new IllegalArgumentException(
				"unable to check and update the statuses, "
					+ "null arguments: requestType=" + requestType + " surl=" + surl
	...
	public static Map<TSURL, TReturnStatus> getSurlsStatus(
		TRequestToken requestToken, ArrayList<TSURL> surls, TRequestType requestType)
		throws IllegalArgumentException, UnknownTokenException,
		ExpiredTokenException {
	...
	public static Map<TRequestToken, TReturnStatus> getSurlCurrentStatuses(
		TSURL surl, GridUserInterface user) {
		...
		Map<TRequestToken, TReturnStatus> persistentTokensStatusMap = getSurlPerPersistentTokenStatuses(
			surl, user);
		try {
			// TODO requestType should be stored together with the token
			persistentTokensStatusMap.putAll(SurlStatusStore.getInstance()
				.getSurlPerTokenStatuses(surl, user));
		} catch (UnknownSurlException e) {
			log.debug("Unable to get surl statuses. UnknownTokenException: "
				+ e.getMessage());
		} catch (IllegalArgumentException e) {
			log
				.error("Unexpected IllegalArgumentException during surl statuses retrieving: "
					+ e);
	...
	private static HashMap<TRequestToken, TReturnStatus> getSurlCurrentStatuses(
		TSURL surl, GridUserInterface user, TRequestType requestType) {

		if (surl == null || user == null || requestType == null) {
			throw new IllegalArgumentException(
				"Unable to get the statuses, null arguments: surl=" + surl + " user="
					+ surl + " requestType=" + requestType);
		}
		Map<TRequestToken, TReturnStatus> persistentTokensStatusMap = getSurlPerPersistentTokenStatuses(
			surl, user, requestType);
		try {
			// TODO requestType should be stored together with the token
			persistentTokensStatusMap.putAll(SurlStatusStore.getInstance()
				.getSurlPerTokenStatuses(surl, user));
	...
	public static Map<TRequestToken, TReturnStatus> getSurlCurrentStatuses(
		TSURL surl) throws IllegalArgumentException {
	...
	private static Map<TRequestToken, TReturnStatus> getSurlCurrentStatuses(
		TSURL surl, TRequestType requestType) {
	...
	public static TReturnStatus getSurlsStatus(TSURL surl, GridUserInterface user)
		throws UnknownSurlException, IllegalArgumentException {

		if (surl == null || user == null) {
			throw new IllegalArgumentException(
				"Unable to get the status, null arguments: surl=" + surl + " user="
					+ surl);
		}

		Collection<TReturnStatus> statuses = getSurlCurrentStatuses(surl, user)
			.values();
		if (statuses.isEmpty()) {
		...
		try {
			...
			log.warn("Inconsistent status for surl " + surl + " . Statuses are: "
				+ statuses);
			return extractMostRecentStatus(statuses);
		}
	}

	public static TReturnStatus getSurlStatus(TSURL surl, GridUserInterface user,
		TRequestType requestType) throws UnknownSurlException,
	...
	public static TReturnStatus getSurlStatus(TSURL surl)
		throws IllegalArgumentException, UnknownSurlException {

		if (surl == null) {
			throw new IllegalArgumentException(
				"Unable to get the status, null arguments: surl=" + surl);
		}
	...
	public static TReturnStatus getSurlsStatus(TSURL surl,
		...
		Collection<TReturnStatus> statuses = getSurlCurrentStatuses(surl,
			requestType).values();
		if (statuses.isEmpty()) {
			throw new UnknownSurlException("The surl is not stored");
		}
	...
	private static TReturnStatus extractSurlStatus(
		Collection<TReturnStatus> statuses) throws IllegalArgumentException {
	...
	private static Map<TSURL, TReturnStatus> getPersistentSurlsStatuses(
		TRequestType requestType, TRequestToken requestToken) {

		Map<TSURL, TReturnStatus> surlsStatuses;
		switch (requestType) {
		case PREPARE_TO_GET:
			Collection<ReducedChunkData> ptgChunksData = PtGChunkCatalog
				.getInstance().lookupReducedPtGChunkData(requestToken);
		...
		case PREPARE_TO_PUT:
			Collection<ReducedPtPChunkData> ptpChunksData = PtPChunkCatalog
				.getInstance().lookupReducedPtPChunkData(requestToken);
			...
			return surlsStatuses;
		case COPY:
			Collection<CopyPersistentChunkData> copyChunksData = CopyChunkCatalog
				.getInstance().lookup(requestToken);
			...
			for (CopyPersistentChunkData chunkData : copyChunksData) {
				...
				surlsStatuses
					.put(chunkData.getDestinationSURL(), chunkData.getStatus());
			}
			return surlsStatuses;
		case BRING_ON_LINE:
			Collection<ReducedBoLChunkData> bolChunksData = BoLChunkCatalog
				.getInstance().lookupReducedBoLChunkData(requestToken);
		...
		default:
			throw new IllegalArgumentException("Received unknown TRequestType: "
				+ requestType);
		}
	}

	private static void updatePersistentStatus(TRequestType requestType,
		TRequestToken requestToken, TSURL surl, TStatusCode statusCode,
		String explanation) {

		switch (requestType) {
		case PREPARE_TO_GET:
			PtGChunkCatalog.getInstance().updateStatus(requestToken, surl,
				statusCode, explanation);
			break;
		case PREPARE_TO_PUT:
			PtPChunkCatalog.getInstance().updateStatus(requestToken, surl,
				statusCode, explanation);
			break;
		case COPY:
			// TODO if needed do it
			// CopyChunkCatalog.getInstance().updateStatus(requestToken, surl,
			// statusCode, explanation);
			throw new IllegalStateException(
				"Not implemented, contact storm developers: "
					+ "CopyChunkCatalog.getInstance().updateStatus(requestToken, surl, statusCode, explanation);");
		case BRING_ON_LINE:
			// TODO if needed do it
			// BoLChunkCatalog.getInstance().updateStatus(requestToken, surl,
			// statusCode, explanation);
			throw new IllegalStateException(
				"Not implemented, contact storm developers: "
					+ "BoLChunkCatalog.getInstance().updateStatus(requestToken, surl, statusCode, explanation);");
		case EMPTY:
			throw new IllegalArgumentException("Received Empty TRequestType: "
				+ requestType);
		default:
			throw new IllegalArgumentException("Received unknown TRequestType: "
				+ requestType);
	...
	private static void updatePersistentStatus(TRequestType requestType,
		...
		case PREPARE_TO_GET:
			// TODO if needed do it
			// PtGChunkCatalog.getInstance().updateStatus(surl, StatusCode,
			// explanation);
			throw new IllegalStateException(
				"Not implemented, contact storm developers: "
	...
	private static void checkAndUpdatePersistentStatus(TRequestType requestType,
		TSURL surl, TStatusCode expectedStatusCode, TStatusCode newStatusCode,
		String explanation) {
		...
		case PREPARE_TO_GET:
			...
			PtGChunkCatalog.getInstance().updateFromPreviousStatus(surl,
				expectedStatusCode, newStatusCode, explanation);
			break;
			
		case PREPARE_TO_PUT:
			PtPChunkCatalog.getInstance().updateFromPreviousStatus(surl,
				expectedStatusCode, newStatusCode, explanation);
			break;
	...
	private static void checkAndUpdatePersistentStatus(TRequestType requestType,
		...
		switch (requestType) {
		case PREPARE_TO_GET:
			PtGChunkCatalog.getInstance().updateFromPreviousStatus(requestToken,
				expectedStatusCode, newStatusCode, explanation);
			break;
		case PREPARE_TO_PUT:
			PtPChunkCatalog.getInstance().updateFromPreviousStatus(requestToken,
				expectedStatusCode, newStatusCode, explanation);
			break;
		case COPY:
			CopyChunkCatalog.getInstance().updateFromPreviousStatus(requestToken,
				expectedStatusCode, newStatusCode, explanation);
			break;
		case BRING_ON_LINE:
			BoLChunkCatalog.getInstance().updateFromPreviousStatus(requestToken,
				expectedStatusCode, newStatusCode, explanation);
			break;
		case EMPTY:
			break;
	...
	private static void checkAndUpdatePersistentStatus(TRequestType requestType,
		TRequestToken requestToken, List<TSURL> surls,
		TStatusCode expectedStatusCode, TStatusCode newStatusCode)
		...
		case PREPARE_TO_GET:
			PtGChunkCatalog.getInstance().updateFromPreviousStatus(requestToken,
				surls, expectedStatusCode, newStatusCode);
			break;
		case PREPARE_TO_PUT:
			PtPChunkCatalog.getInstance().updateFromPreviousStatus(requestToken,
				surls, expectedStatusCode, newStatusCode);
	...
	private static Map<TSURL, TReturnStatus> getPersistentSurlsStatuses(
		TRequestType requestType, TRequestToken requestToken,
		Collection<TSURL> surls) throws IllegalArgumentException {

		Map<TSURL, TReturnStatus> surlsStatuses;
		switch (requestType) {
		case PREPARE_TO_GET:
			Collection<ReducedChunkData> ptgChunksData = PtGChunkCatalog
				.getInstance().lookupReducedPtGChunkData(requestToken, surls);
			surlsStatuses = new HashMap<TSURL, TReturnStatus>(ptgChunksData.size());
			for (ReducedChunkData chunkData : ptgChunksData) {
				surlsStatuses.put(chunkData.fromSURL(), chunkData.status());
			}
		...
		case PREPARE_TO_PUT:
			Collection<ReducedPtPChunkData> ptpChunksData = PtPChunkCatalog
				.getInstance().lookupReducedPtPChunkData(requestToken, surls);
			surlsStatuses = new HashMap<TSURL, TReturnStatus>(ptpChunksData.size());
			for (ReducedPtPChunkData chunkData : ptpChunksData) {
				surlsStatuses.put(chunkData.toSURL(), chunkData.status());
			}
		...
		case COPY:
			Collection<CopyPersistentChunkData> copyChunksData = CopyChunkCatalog
				.getInstance().lookupCopyChunkData(requestToken, surls);
			surlsStatuses = new HashMap<TSURL, TReturnStatus>(copyChunksData.size());
			for (CopyPersistentChunkData chunkData : copyChunksData) {
				surlsStatuses.put(chunkData.getSURL(), chunkData.getStatus());
				surlsStatuses
		...
		case BRING_ON_LINE:
			Collection<ReducedBoLChunkData> bolChunksData = BoLChunkCatalog
				.getInstance().lookupReducedBoLChunkData(requestToken, surls);
			surlsStatuses = new HashMap<TSURL, TReturnStatus>(bolChunksData.size());
			for (ReducedChunkData chunkData : bolChunksData) {
				surlsStatuses.put(chunkData.fromSURL(), chunkData.status());
			}
	...
	private static Map<TRequestToken, TReturnStatus> getSurlPerPersistentTokenStatuses(
		TSURL surl, GridUserInterface user) {

		HashMap<TRequestToken, TReturnStatus> tokenStatusMap = new HashMap<TRequestToken, TReturnStatus>();
		tokenStatusMap.putAll(buildPtGTokenStatusMap(PtGChunkCatalog.getInstance()
			.lookupPtGChunkData(surl, user)));
		tokenStatusMap.putAll(buildPtPTokenStatusMap(PtPChunkCatalog.getInstance()
			.lookupPtPChunkData(surl, user)));
		tokenStatusMap.putAll(buildCopyTokenStatusMap(CopyChunkCatalog
			.getInstance().lookupCopyChunkData(surl, user)));
		tokenStatusMap.putAll(buildBoLTokenStatusMap(BoLChunkCatalog.getInstance()
			.lookupBoLChunkData(surl, user)));
	...
	private static Map<TRequestToken, TReturnStatus> getSurlPerPersistentTokenStatuses(
		...
		switch (requestType) {
		case PREPARE_TO_GET:
			return buildPtGTokenStatusMap(PtGChunkCatalog.getInstance()
				.lookupPtGChunkData(surl, user));
		case PREPARE_TO_PUT:

			return buildPtPTokenStatusMap(PtPChunkCatalog.getInstance()
				.lookupPtPChunkData(surl, user));
		case COPY:
			return buildCopyTokenStatusMap(CopyChunkCatalog.getInstance()
				.lookupCopyChunkData(surl, user));
		case BRING_ON_LINE:
			return buildBoLTokenStatusMap(BoLChunkCatalog.getInstance()
				.lookupBoLChunkData(surl, user));
		default:
	...
	private static Map<TRequestToken, TReturnStatus> getSurlPerPersistentTokenStatuses(
		...
		HashMap<TRequestToken, TReturnStatus> tokenStatusMap = new HashMap<TRequestToken, TReturnStatus>();
		tokenStatusMap.putAll(buildPtGTokenStatusMap(PtGChunkCatalog.getInstance()
			.lookupPtGChunkData(surl)));
		tokenStatusMap.putAll(buildPtPTokenStatusMap(PtPChunkCatalog.getInstance()
			.lookupPtPChunkData(surl)));
		tokenStatusMap.putAll(buildCopyTokenStatusMap(CopyChunkCatalog
			.getInstance().lookupCopyChunkData(surl)));
		tokenStatusMap.putAll(buildBoLTokenStatusMap(BoLChunkCatalog.getInstance()
			.lookupBoLChunkData(surl)));
		return tokenStatusMap;
	}
	...
	private static LinkedList<TReturnStatus> extractNonFinalStatuses(
		Collection<TReturnStatus> statuses) {
	...
	private static HashMap<TRequestToken, TReturnStatus> filterOutFinalStatuses(
		Map<TRequestToken, TReturnStatus> statuses) {
	...
	private static TReturnStatus extractMostRecentStatus(
		Collection<TReturnStatus> statuses) {
	...
	public static TRequestType isPersisted(TRequestToken requestToken)
		throws IllegalArgumentException {
