...
public class SurlStatusStore {
	...
	private static final Logger log = LoggerFactory
		.getLogger(SurlStatusStore.class);
	...
	public void store(TRequestToken requestToken,
		...
		if (requestToken == null || surlStatuses == null || surlStatuses.isEmpty()) {
			throw new IllegalArgumentException(
				"unable to store the request, null arguments: requestToken="
		...
		try {
			...
			if (writeLock == null) {
				if (verifyMissingToken(requestToken)) {
					throw new IllegalStateException("Token " + requestToken
						+ " has not a lock but is registerd in the store");
				}
				throw new TokenDuplicationException("Token " + requestToken
					+ " is already stored");
			}
			if (tokenSurlStatusStore.containsKey(requestToken)) {
				log.warn("Token \'" + requestToken + "\' is already stored");
				throw new IllegalArgumentException("Provided token \'" + requestToken
					+ "\'is already stored");
			}
			...
			for (Entry<TSURL, TReturnStatus> surlStatus : surlStatuses.entrySet()) {
				if (surlStatus.getKey() == null || surlStatus.getValue() == null) {
					...
					throw new IllegalArgumentException(
						"Received invalid surl-status entry surl=" + surlStatus.getKey()
							+ " status=" + surlStatus.getValue());
				}
				...
				try {
					...
					removeLock(requestToken);
					throw new IllegalArgumentException(
						"Received invalid status for surl " + surlStatus.getKey()
							+ " InvalidTReturnStatusAttributeException: " + e.getMessage());
				}
				surlStatusMap.put(surlStatus.getKey(), status);
			}
			for (Entry<TSURL, ModifiableReturnStatus> surlStatus : surlStatusMap
				.entrySet()) {
				log.debug("Storing surl " + surlStatus.getKey() + " with status "
					+ surlStatus.getValue() + " for token " + requestToken);
				WriteLock surlWriteLock = null;
				try {
					surlWriteLock = writeLock(surlStatus.getKey());
					Map<TRequestToken, ModifiableReturnStatus> map = surlTokenStatusStore
						.get(surlStatus.getKey());
					if (map == null) {
	...
	public void store(TRequestToken requestToken, GridUserInterface user,
		...
		if (user == null) {
			throw new IllegalArgumentException(
				"unable to store the request, null arguments: user=" + user);
		}
	...
	public void update(TRequestToken requestToken, List<TSURL> surls,
		...
		if (requestToken == null || surls == null || surls.isEmpty()
			...
			throw new IllegalArgumentException(
				"unable to check and update the statuses, "
					+ "null arguments: requestToken=" + requestToken + " surls=" + surls
					+ " newStatusCode=" + newStatusCode + " explanation=" + explanation);
		}
		log.debug("Updating token " + requestToken + " for surls " + surls
			+ " with status " + newStatusCode + " " + explanation);
		ReadLock readLock = null;
		try {
			readLock = verifyAndReadLock(requestToken);
			checkToken(requestToken);
			Map<TSURL, ModifiableReturnStatus> surlStatusMap = this.tokenSurlStatusStore
				.get(requestToken);
			for (TSURL surl : surls) {
				if (surl == null) {
					log.warn("Unexpected null element in input surls list : " + surls);
					throw new IllegalArgumentException(
						"Received invalid null surl in surls list!");
				}
				ModifiableReturnStatus status = surlStatusMap.get(surl);
				if (status == null) {
					log.warn("Surl \'" + surl + "\' is not associated to token \'"
						+ requestToken + "\'");
					throw new UnknownSurlException("Surl " + surl
						+ " is not associated to " + requestToken);
				} else {
	...
	public void checkAndUpdate(TRequestToken requestToken, TSURL surl,
	...
	public void checkAndUpdate(TRequestToken requestToken, TSURL surl,
		...
		ArrayList<TSURL> surlList = new ArrayList<TSURL>(1);
		surlList.add(surl);
		checkAndUpdate(requestToken, surlList, expectedStatusCode, newStatusCode,
			explanation);
	}
	...
	public void update(TSURL surl, TStatusCode statusCode, String explanation)
		...
		if (surl == null) {
			throw new IllegalArgumentException("unable to update the status, "
				+ "null arguments: surl=" + surl);
		}
		ArrayList<TSURL> surlList = new ArrayList<TSURL>(1);
		surlList.add(surl);
	...
	private void update(ArrayList<TSURL> surls, TStatusCode statusCode,
		...
		if (surls == null || surls.isEmpty() || statusCode == null
			|| explanation == null) {
			throw new IllegalArgumentException("unable to update the statuses, "
				+ "null arguments: surls=" + surls + " statusCode=" + statusCode
				+ " explanation=" + explanation);
		}
		log.debug("Updating surls " + surls + " with status " + statusCode + " "
			+ explanation);
		for (TSURL surl : surls) {
			if (surl == null) {
				log.warn("Unexpected null element in input surls list : " + surls);
			...
			try {
				...
				Map<TRequestToken, ModifiableReturnStatus> tokenStatusMap = this.surlTokenStatusStore
					.get(surl);
	...
	public void checkAndUpdate(TSURL surl, TStatusCode expectedStatusCode,
		TStatusCode newStatusCode, String explanation)
	...
	private void checkAndUpdate(ArrayList<TSURL> surls,
		TStatusCode expectedStatusCode, TStatusCode newStatusCode,
		String explanation) throws UnknownSurlException {
		...
		if (surls == null || surls.isEmpty() || expectedStatusCode == null
			|| newStatusCode == null || explanation == null) {
			throw new IllegalArgumentException(
				"unable to check and update the statuses, " + "null arguments: surls="
					+ surls + " expectedStatusCode=" + expectedStatusCode
					+ " newStatusCode=" + newStatusCode + " explanation=" + explanation);
		}
		log.debug("Checking and updating surls " + surls + " where status is "
			+ expectedStatusCode + " with status " + newStatusCode + " "
			+ explanation);
		for (TSURL surl : surls) {
			if (surl == null) {
				log.warn("Unexpected null element in input surls list : " + surls);
			...
			try {
				...
				for (ModifiableReturnStatus status : tokenStatusMap.values()) {
					status.testAndSetStatusCodeExplanation(expectedStatusCode,
	...
	public void checkAndUpdate(TRequestToken requestToken, List<TSURL> surls,
		TStatusCode expectedStatusCode, TStatusCode newStatusCode)
		throws IllegalArgumentException, UnknownTokenException,
		ExpiredTokenException, UnknownSurlException {
	...
	public void checkAndUpdate(TRequestToken requestToken, List<TSURL> surls,
		...
		if (requestToken == null || surls == null || surls.isEmpty()
			|| expectedStatusCode == null || newStatusCode == null
			|| explanation == null) {
			throw new IllegalArgumentException(
				"unable to check and update the statuses, "
					+ "null arguments: requestToken=" + requestToken + " surls=" + surls
					+ " expectedStatusCode=" + expectedStatusCode + " newStatusCode="
					+ newStatusCode + " explanation=" + explanation);
		}
		log.debug("Checking and updating token " + requestToken + " for surls "
			+ surls + " where status is " + expectedStatusCode + " with status "
			+ newStatusCode + " " + explanation);
		ReadLock readLock = null;
		try {
			readLock = verifyAndReadLock(requestToken);
			...
			for (TSURL surl : surls) {
				...
				if (status == null) {
					log.warn("Surl \'" + surl + "\' is not associated to token \'"
						+ requestToken + "\'");
					throw new UnknownSurlException("Surl " + surl
						+ " is not associated to requestToken " + requestToken);
				} else {
					if (!status.testAndSetStatusCodeExplanation(expectedStatusCode,
						newStatusCode, explanation)) {
						log.debug("Surl status not updated, current status \'"
							+ status.getStatusCode() + "\' doesn't match expected status \'"
							+ expectedStatusCode + "\'");
					}
				}
			}
	...
	public void checkAndUpdate(TRequestToken requestToken,
		TStatusCode expectedStatusCode, TStatusCode newStatusCode,
		String explanation) throws UnknownTokenException, ExpiredTokenException {
		...
		log.debug("Checking and updating token " + requestToken
			+ " for surls where status is " + expectedStatusCode + " with status "
			+ newStatusCode + " " + explanation);
		ReadLock readLock = null;
		try {
			readLock = verifyAndReadLock(requestToken);
			...
			for (Entry<TSURL, ModifiableReturnStatus> surlStatus : surlStatusMap
				.entrySet()) {
				if (surlStatus == null) {
					log.warn("Unexpected null element surlStatus for token : "
						+ requestToken);
					continue;
				}
				if (!surlStatus.getValue().testAndSetStatusCodeExplanation(
					expectedStatusCode, newStatusCode, explanation)) {
					log
						.debug("Surl status not updated, current status \'"
	...
	public Map<TSURL, TReturnStatus> getSurlsStatus(TRequestToken requestToken)
		...
		try {
			readLock = verifyAndReadLock(requestToken);
			checkToken(requestToken);
			return (Map<TSURL, TReturnStatus>) ((HashMap<TSURL, ModifiableReturnStatus>) this.tokenSurlStatusStore
				.get(requestToken)).clone();
		} finally {
			if (readLock != null) {
				unlockToken(readLock);
			}
	...
	public Map<TSURL, TReturnStatus> getSurlsStatus(TRequestToken requestToken,
		TSURL surl) throws UnknownTokenException, IllegalArgumentException,
		ExpiredTokenException {

		if (surl == null || surl.isEmpty()) {
			throw new IllegalArgumentException(
				"unable to get the statuses, null arguments: surl=" + surl);
		}
		return getSurlsStatus(requestToken, Arrays.asList(new TSURL[] { surl }));
	}

	public Map<TSURL, TReturnStatus> getSurlsStatus(TRequestToken requestToken,
		Collection<TSURL> surls) throws UnknownTokenException,
		...
		log.debug("Retrieving statuses of surls " + surls + " for token "
			+ requestToken);
		ReadLock readLock = null;
		try {
			readLock = verifyAndReadLock(requestToken);
			checkToken(requestToken);
			HashMap<TSURL, TReturnStatus> builtSurlStatusMap = new HashMap<TSURL, TReturnStatus>(
				surls.size());
			Map<TSURL, ModifiableReturnStatus> surlStatusMap = this.tokenSurlStatusStore
				.get(requestToken);
			for (TSURL surl : surls) {
	...
	public Collection<TReturnStatus> getSurlStatuses(TSURL surl)
		...
		return new ArrayList<TReturnStatus>(new HashSet<TReturnStatus>(
			tokensStatusMap.values()));
	}

	public Map<TRequestToken, ? extends TReturnStatus> getSurlPerTokenStatuses(
		TSURL surl, GridUserInterface user) throws UnknownSurlException {

		if (surl == null || user == null) {
			throw new IllegalArgumentException(
				"Unable to get the statuses, null arguments: surl=" + surl + " user="
					+ user);
		}
		...
		try {
			readLock = readLock(surl);
			Map<TRequestToken, ModifiableReturnStatus> tokensStatusMap = surlTokenStatusStore
				.get(surl);
			if (tokensStatusMap == null) {
				throw new UnknownSurlException("Surl " + surl + " is not stored");
			}
			...
			try {
				lockTokenUserStore();
				for (Entry<TRequestToken, ModifiableReturnStatus> tokenStatus : tokensStatusMap
					.entrySet()) {
					if (!tokenUserStore.containsKey(tokenStatus.getKey())) {
						userTokensStatusMap.put(tokenStatus.getKey(),
							tokenStatus.getValue());
					} else {
						if (tokenUserStore.get(tokenStatus.getKey()).equals(user)) {
							...
							log
								.debug("Surl " + surl
									+ " has not been stored with a token belonging to user "
									+ user);
						}
					}
				}
	...
	public Map<TRequestToken, TReturnStatus> getSurlPerTokenStatuses(TSURL surl)
		...
		try {
			...
			return (Map<TRequestToken, TReturnStatus>) ((HashMap) tokensStatusMap)
				.clone();
		} finally {
			unlockSurl(readLock);
		}
	...
	public List<TRequestToken> getExpiredTokens() {
		...
		lockExpirationMap();
		try {
			return new ArrayList<TRequestToken>(expirationTokenRegistry.headMap(
				Calendar.getInstance().getTimeInMillis(), true).values());
		} finally {
			unlockExpirationMap();
		}
	...
	private void checkToken(TRequestToken requestToken)
		throws UnknownTokenException, ExpiredTokenException {

		if (!tokenSurlStatusStore.containsKey(requestToken)) {
			log.warn("Token \'" + requestToken + "\' is not stored");
			throw new UnknownTokenException("Provided token \'" + requestToken
				+ "\'is not stored");
		}
		if (requestToken.hasExpirationDate() && requestToken.isExpired()) {
			log.warn("Token \'" + requestToken + "\' is expired");
			throw new ExpiredTokenException("Provided token \'" + requestToken
				+ "\'is expired");
		}
	}

	private void cleanToken(TRequestToken requestToken)
		throws UnknownTokenException {
		...
		try {
			...
			Map<TSURL, ModifiableReturnStatus> surlStatuses = this.tokenSurlStatusStore
				.remove(requestToken);
			for (TSURL surl : surlStatuses.keySet()) {
				WriteLock writeLockSurl = null;
				try {
					writeLockSurl = writeLock(surl);
					Map<TRequestToken, ModifiableReturnStatus> tokenStatuses = this.surlTokenStatusStore
						.get(surl);
					if (tokenStatuses == null) {
						throw new IllegalStateException(
							"Inconsistent Surl Status Store. Surl uid " + surl
	...
	private void verifyToken(TRequestToken requestToken)
		throws UnknownTokenException {
		...
		if (!tokenLockMap.containsKey(requestToken)) {
			...
			throw new IllegalStateException("Provided token \'" + requestToken
				+ "\'is not stored in tokenLockMap");
		}
		try {
			if (!expirationTokenRegistry
				.containsKey(getStoredTokenExpiration(requestToken))) {
				log.warn("Token \'" + requestToken
					+ "\' is not stored in expirationTokenRegistry");
				throw new IllegalStateException("Provided token \'" + requestToken
					+ "\'is not stored in expirationTokenRegistry");
			}
		} catch (UnknownTokenException e) {
			log.warn("Token \'" + requestToken
				+ "\' is not retrieved from getStoredTokenExpiration");
			throw new IllegalStateException("Provided token \'" + requestToken
				+ "\'is not retrieved from getStoredTokenExpiration");
		}
	}

	private boolean verifyMissingToken(TRequestToken requestToken)
		throws IllegalArgumentException {

		if (requestToken == null) {
			throw new IllegalArgumentException(
				"Unable to verify the token, null arguments: requestToken="
					+ requestToken);
		}
		return (!tokenSurlStatusStore.containsKey(requestToken)
			&& !tokenLockMap.containsKey(requestToken)
			&& (!requestToken.hasExpirationDate() || !requestToken
				.equals(expirationTokenRegistry.get(requestToken.getExpiration()))) && !tokenUserStore
				.containsKey(requestToken));
	}

	private long getStoredTokenExpiration(TRequestToken requestToken)
		throws UnknownTokenException {
		...
		try {
			...
			if (index >= 0) {
				...
				if (!storedToken.hasExpirationDate()) {
					throw new IllegalStateException(
						"Unexpected stored token with no expiration date");
				}
				return storedToken.getExpiration().getTimeInMillis();
			} else {
				throw new UnknownTokenException("Token " + requestToken
					+ " not stored in tokenSurlStatusStore");
			}
		} finally {
			if (readLock != null) {
				unlockToken(readLock);
	...
	public void checkIntegrity() throws Exception {
		...
		for (Entry<TSURL, Map<TRequestToken, ModifiableReturnStatus>> surlTokenStatus : surlTokenStatusStore
			.entrySet()) {
			if (surlTokenStatus.getValue() == null
				|| surlTokenStatus.getValue().isEmpty()) {
				throw new Exception(
					"Integrity violation: empty tokenStatusMap in surlTokenStatusStore for surl "
						+ surlTokenStatus.getKey());
			}
			for (Entry<TRequestToken, ModifiableReturnStatus> tokenSurlStatus : surlTokenStatus
				.getValue().entrySet()) {
				if (tokenSurlStatus.getValue() == null) {
					throw new Exception(
						"Integrity violation: Null status in surlTokenStatusStore for surl "
							+ surlTokenStatus.getKey() + " token " + tokenSurlStatus.getKey());
				}
				Map<TSURL, ModifiableReturnStatus> surlStatus = tokenSurlStatusStore
					.get(tokenSurlStatus.getKey());
				if (surlStatus == null) {
					throw new Exception(
						"Integrity violation: Null surlStatus map in tokenSurlStatusStore for token "
							+ tokenSurlStatus.getKey() + " surl "
							+ surlTokenStatus.getValue());
				}
				if (surlStatus.get(surlTokenStatus.getKey()) == null) {
					throw new Exception(
						"Integrity violation: Null Status in tokenSurlStatusStore for token "
							+ tokenSurlStatus.getKey() + " surl "
							+ surlTokenStatus.getValue());
				}
				if (!tokenSurlStatus.getValue().equals(
				...
				if (!(tokenSurlStatus.getValue() == surlStatus.get(surlTokenStatus
					.getKey()))) {
					throw new Exception(
						"Integrity violation: object mismatch for status in tokenSurlStatusStore "
							+ tokenSurlStatus.getValue() + " and surlTokenStatusStore "
							+ surlStatus.get(surlTokenStatus.getKey()));
				}
			}
		...
		for (Entry<TRequestToken, Map<TSURL, ModifiableReturnStatus>> tokenSurlStatus : tokenSurlStatusStore
			.entrySet()) {
			if (tokenSurlStatus.getValue() == null
				|| tokenSurlStatus.getValue().isEmpty()) {
				throw new Exception(
					"Integrity violation: empty surlStatus map in tokenSurlStatusStore for token "
						+ tokenSurlStatus.getKey());
			}
			if (tokenSurlStatus.getKey().hasExpirationDate()) {
				TRequestToken expirationToken = expirationTokenRegistry
					.get(tokenSurlStatus.getKey().getExpiration().getTimeInMillis());
				if (expirationToken == null) {
					throw new Exception(
						"Integrity violation: empty expirationToken for token "
							+ tokenSurlStatus.getKey());
				}
				if (!expirationToken.equals(tokenSurlStatus.getKey())) {
					throw new Exception(
						"Integrity violation: value mismatch for expirationToken "
							+ expirationToken + " and token " + tokenSurlStatus.getKey());
				}
			}
			for (Entry<TSURL, ModifiableReturnStatus> surlTokenStatus : tokenSurlStatus
				.getValue().entrySet()) {
				if (surlTokenStatus.getValue() == null) {
					throw new Exception(
						"Integrity violation: Null status in tokenSurlStatusStore for token "
				...
				Map<TRequestToken, ModifiableReturnStatus> tokenStatus = surlTokenStatusStore
					.get(surlTokenStatus.getKey());
				if (tokenStatus == null) {
					throw new Exception(
						"Integrity violation: Null tokenStatus map in surlTokenStatusStore for surl "
							+ surlTokenStatus.getKey() + " token "
							+ tokenSurlStatus.getValue());
				}
				if (tokenStatus.get(tokenSurlStatus.getKey()) == null) {
					throw new Exception(
						"Integrity violation: Null Status in surlTokenStatusStore for surl "
							+ surlTokenStatus.getKey() + " surl "
							+ tokenSurlStatus.getValue());
				}
				if (!surlTokenStatus.getValue().equals(
				...
				if (!(surlTokenStatus.getValue() == tokenStatus.get(tokenSurlStatus
					.getKey()))) {
					throw new Exception(
		...
		for (Entry<Long, TRequestToken> surlexpirationToken : expirationTokenRegistry
			.entrySet()) {
			if (!surlexpirationToken.getValue().hasExpirationDate()) {
				throw new Exception("Integrity violation: stored token "
					+ surlexpirationToken.getValue()
					+ " in expirationTokenRegistry has no expiration ");
			}
			if (surlexpirationToken.getKey() != surlexpirationToken.getValue()
				.getExpiration().getTimeInMillis()) {
				throw new Exception(
					"Integrity violation: value mismatch between expiration "
			...
			if (tokenSurlStatusStore.get(surlexpirationToken.getValue()) == null) {
				throw new Exception(
					"Integrity violation: Null tokenSurlStatus map for expirationToken "
						+ surlexpirationToken.getValue());
			}
		}
		for (TRequestToken token : tokenUserStore.keySet()) {
			if (tokenSurlStatusStore.get(token) == null) {
				throw new Exception(
					"Integrity violation: Null tokenSurlStatus map for tokenUserStore "
						+ token.getValue());
			}
		}
		for (Entry<TRequestToken, ReentrantReadWriteLock> tokenLock : tokenLockMap
			.entrySet()) {
			if (tokenSurlStatusStore.get(tokenLock.getKey()) == null) {
				throw new Exception(
					"Integrity violation: Null tokenSurlStatus map for tokenLock "
						+ tokenLock.getKey());
			}
		}
	...
	private WriteLock writeLockIfNew(TRequestToken requestToken) {
		...
		ReentrantReadWriteLock storedLock = tokenLockMap.putIfAbsent(requestToken,
			lock);
		if (storedLock != null) {
			return null;
		}
	...
	private WriteLock verifyAndWriteLock(TRequestToken requestToken)
		throws UnknownTokenException {
	...
	private void unlockToken(WriteLock writeLock) throws IllegalArgumentException {
		...
		if (writeLock == null) {
			throw new IllegalArgumentException(
				"The provided argument is null: writeLock=" + writeLock);
		}
		writeLock.unlock();
		log.trace("SurlStatusStore.writeUnlock(TOKEN)");
	}

	private ReadLock verifyAndReadLock(TRequestToken requestToken)
		throws UnknownTokenException {
	...
	private void unlockToken(ReadLock readLock) throws IllegalArgumentException {
		...
		if (readLock == null) {
			throw new IllegalArgumentException(
				"The provided argument is null: readLock=" + readLock);
		}
		readLock.unlock();
	...
	private void removeLock(TRequestToken requestToken)
		throws IllegalArgumentException {

		if (tokenLockMap.remove(requestToken) == null) {
			throw new IllegalArgumentException(
				"Unable to remove the provided token lock, it doesn't exists");
		}
	}
