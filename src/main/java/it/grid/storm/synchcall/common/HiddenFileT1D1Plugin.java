...
public class HiddenFileT1D1Plugin implements T1D1PluginInterface {
	...
	private static final Logger log = LoggerFactory
		.getLogger(HiddenFileT1D1Plugin.class);

	public int startMigration(StoRI stori, String prefix) {

		if (stori != null) {
			log.debug("HiddenFileT1D1PLugin: SURL filename" + stori.getFilename());
			String localPathWithoutFilename = NamespaceUtil.consumeFileName(stori
				.getLocalFile().getPath());
			log.debug("HiddenFileT1D1PLugin: local path without filename "
				+ localPathWithoutFilename);
			String hiddenFileName = localPathWithoutFilename + "." + prefix
				+ stori.getFilename();
