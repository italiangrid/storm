 ...
 *     <p>The it.grid.storm.filesystem.FilesystemPermission class.
 */
/*
 ...
 * directory).
 *
 * <p>Note: this class is an interface to the fs_acl::permission_t type; if the low-level
 * fs_acl::permission_t type ever gets modified, then the {@link toFsAclPermission()} method should
 * be modified also.
 *
 * <p>To all effects, instances of this class are <em>immutable</em>. Permissions are read off or
...
public class FilesystemPermission implements java.io.Serializable {
  ...
  static final int EXECUTE = fs_acl.permission_flags.PERM_EXECUTE;

  /** Permission to write file contents. */
  static final int WRITE_DATA = fs_acl.permission_flags.PERM_WRITE_DATA;

  /** Permission to read file contents. */
  static final int READ_DATA = fs_acl.permission_flags.PERM_READ_DATA;

  /** Permission to change file extended ACL (that is, beyond normal UNIX permission bits). */
  static final int WRITE_ACL = fs_acl.permission_flags.PERM_WRITE_ACL;

  /** Permission to read file extended ACL (that is, beyond normal UNIX permission bits). */
  static final int READ_ACL = fs_acl.permission_flags.PERM_READ_ACL;

  /** Permission to delete a filesystem entry (file or directory). */
  static final int DELETE = fs_acl.permission_flags.PERM_DELETE;

  /** Permission to descend to children directories of a directory. */
  static final int TRAVERSE_DIRECTORY = fs_acl.permission_flags.PERM_TRAVERSE_DIRECTORY;

  /** Permission to list directory contents. */
  static final int LIST_DIRECTORY = fs_acl.permission_flags.PERM_LIST_DIRECTORY;

  /** Permission to create a child subdirectory. */
  static final int CREATE_SUBDIRECTORY = fs_acl.permission_flags.PERM_CREATE_SUBDIRECTORY;

  /** Permission to create a new file. */
  static final int CREATE_FILE = fs_acl.permission_flags.PERM_CREATE_FILE;

  /** Permission to delete a file or directory within a directory. */
  static final int DELETE_CHILD = fs_acl.permission_flags.PERM_DELETE_CHILD;

  /** No permission at all. */
  static final int NONE = fs_acl.permission_flags.PERM_NONE;

  /** All permission bits set. */
  static final int ALL = fs_acl.permission_flags.PERM_ALL;

  // --- public constant instances --- //

  /** Permission to read a file. */
  public static final FilesystemPermission Read = new FilesystemPermission(READ_DATA);

  /** Permission to read and write to a file. */
  public static final FilesystemPermission ReadWrite =
      new FilesystemPermission(READ_DATA | WRITE_DATA);

  /** Permission to list directory contents. */
  public static final FilesystemPermission List = new FilesystemPermission(LIST_DIRECTORY);

  /** Permission to traverse directory (descend path where directory is an intermediate step). */
  public static final FilesystemPermission Traverse = new FilesystemPermission(TRAVERSE_DIRECTORY);

  /** Permission to list and traverse directory. */
  public static final FilesystemPermission ListTraverse =
      new FilesystemPermission(LIST_DIRECTORY | TRAVERSE_DIRECTORY);

  /** Permission to list, traverse and write directory. */
  public static final FilesystemPermission ListTraverseWrite =
      new FilesystemPermission(LIST_DIRECTORY | TRAVERSE_DIRECTORY | WRITE_DATA);

  /** No permission at all. */
  public static final FilesystemPermission None =
      new FilesystemPermission(FilesystemPermission.NONE);

  /** Permission to write. */
  public static final FilesystemPermission Write =
      new FilesystemPermission(FilesystemPermission.WRITE_DATA);

  /** Permission to create file */
  public static final FilesystemPermission Create =
      new FilesystemPermission(FilesystemPermission.CREATE_FILE);

  /** Permission to create subdirectory */
  public static final FilesystemPermission CreateSubdirectory =
      new FilesystemPermission(FilesystemPermission.CREATE_SUBDIRECTORY);

  /** Permission to delete file or directory */
  public static final FilesystemPermission Delete =
      new FilesystemPermission(FilesystemPermission.DELETE);

  /** */
  public static final FilesystemPermission ListDirectory =
   ...
   * it.grid.storm.filesyste.FilesystemPermission} interface and creates an instance of this class
   * granting <em>exactly</em> the same permissions.
   */
   ...
   * fs_acl::permission_t type, or could be constructed by bitwise-OR'ing the <code>READ_DATA</code>
   * , <code>WRITE_DATA</code>, ... constants defined elsewhere in this class. For any bit that is
   * set in the <code>bitfield</code> argument, the corresponding permission will be granted from
   * this object.
   *
   * <p>Example usage:
   *
   ...
   *
   * <p>This constructor's intended usage is in conjunction with the permission manipulation
  ...
  /**
   * Change instance status so that all subsequent <code>can...</code> calls will return <code>false
   * </code>.
   *
   * <p>Returns the instance itself, so that calls to the permission manipulation functions can be
   * chained:
   *
   * <pre>
  ...
  /**
   * Change instance status so that all subsequent <code>can...</code> calls will return <code>true
   * </code>. <em>Dangerous</em>, use with caution.
   *
   * <p>Returns the instance itself, so that calls to the permission manipulation functions can be
   * chained:
   *
