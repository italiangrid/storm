 ...
 * rather manipulate the filesystem in StoRM through the {@link File} and {@link
 * Space} interfaces: this class is just a thin wrapper around low-level
 * filesystem calls.
 * 
...
public class Filesystem implements FilesystemIF {
   ...
   * 
   * Methods here come in two variants: the "exact" one which tries to return
   * up-to-date information, and the "standard" one, which uses common POSIX
   * system calls to get the required info.
   * 
   * The "exact" variant may force a cluster-wide metadata update, so it may
   * result in performance degradation if used frerquently.
   * 
   * Motivation for this comes from the GPFS filesystem: GPFS nodes perform file
   * metadata and attribute caching, so they may return out-of-date results to
   * the standard system calls. The GPFS API provides the corresponding "exact"
   * calls.
   * 
   * In the generic POSIX filesystem implementation, there's no difference
   * between the "exact" and the "standard" call. @{
   **/
  ...
  public long getLastModifiedTime(final String fileOrDirectory) {
     ...
     * solution will not work in situation (as Amazon s3) for which a native JVM
   ...
   * 
   * <strong>This interface is a draft!</strong> Due to the unsettled state of
   * the SRM spec regarding to reserved space semantics, and the differences
   * between SRM space reservation and GPFS preallocation, it's better not use
   * this interface directly; rather, use wrapper objects (like {@link
    ...
    public FilesystemPermission get(final fs_acl a, final LocalUser u) {
      ...
      if (a.has_group_perm(u.getPrimaryGid())) {
        return new FilesystemPermission(
          a.get_group_effective_perm(u.getPrimaryGid()));
      } else {
        return null;
      }
    }
  ...
  private class RemoveUserPermissionMethod implements RemovePermissionMethod {
    ...
    public FilesystemPermission remove(final fs_acl a, final LocalUser u) {
      ...
      assert (a.has_user_perm(
        u.getUid())) : "Filesystem: removing permission for user " + u.getUid()
  ...
  private class RemoveGroupPermissionMethod implements RemovePermissionMethod {
    ...
    public FilesystemPermission remove(final fs_acl a, final LocalUser u) {
      ...
      assert (a.has_user_perm(
        u.getUid())) : "Filesystem: removing permission for group " + u.getUid()
          + "that has no permission associated!";
      return new FilesystemPermission(
        a.remove_group_perm_not_owner(u.getPrimaryGid()));
    }
  }
  ...
  private class RevokeUserPermissionMethod implements SetPermissionMethod {
    ...
    public FilesystemPermission apply(final fs_acl a, final LocalUser u,
      ...
      if (a.has_user_perm(u.getUid())) {
        return new FilesystemPermission(
          a.revoke_user_perm(u.getUid(), p.toFsAclPermission()));
      } else {
        return null;
      }
    }
