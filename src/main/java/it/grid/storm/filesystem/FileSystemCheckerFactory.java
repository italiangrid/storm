...
public class FileSystemCheckerFactory
{
    ...
    public static FileSystemCheckerFactory getInstance()
    {
        if (instance == null)
        {
            log.info("FileSystemCheckerFactory not explicitaly initialized, using default checker type : "
                    + defaultType);
            init(defaultType);
        }
        return instance;
    ...
    /**
     * Initializes the class by creating the singleton instance if not already done, does nothing if it
     *  already exists and has the chosenType is the same as the provided FileSystemCheckerType throws an 
     *  IllegalStateException otherwise
     *  
     * @param type
     * @throws IllegalArgumentException if type is null
     * @throws IllegalStateException if class already initialized with a different FileSystemCheckerType
     */
    public static void init(FileSystemCheckerType type) throws IllegalArgumentException, IllegalStateException
    {
        ...
        else
        {
            ...
            {
                log.warn("Asked to initialize the already initialized FileSystemCheckerFactory "
                        + "with FileSystemCheckerType " + type + ". Current FileSystemCheckerType is "
                        + instance.chosenType);
                throw new IllegalStateException("Asked to initialize the already initialized FileSystemCheckerFactory "
                        + "with FileSystemCheckerType " + type + ". Current FileSystemCheckerType is "
                        + instance.chosenType);
            }
            else
    ...
    /**
     * Creates the proper FileSystemChecker implementation object using the chosenType available attribute
     * 
     * @return
     */
    public FileSystemChecker createFileSystemChecker() throws IllegalStateException , FileSystemCheckerException
    {
        switch (this.chosenType)
        {
            ...
            default:
                log.error("No correct FileSystemChecker setted : " + this.chosenType
                ...
                throw new IllegalStateException("No correct FileSystemCheckerType setted : "
                        + this.chosenType + ". Available FileSystemCheckerType : "
                        + FileSystemCheckerFactory.FileSystemCheckerType.values()
                        + " Please contact StoRM developers");
        }
    }
