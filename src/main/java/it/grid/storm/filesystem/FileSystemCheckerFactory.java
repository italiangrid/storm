...
public class FileSystemCheckerFactory {
   ...
   *
   * @return the class instance already created by a call to the init method, creates a new one
   *     using the defaultType otherwise
   */
  public static FileSystemCheckerFactory getInstance() {

    if (instance == null) {
      log.info(
          "FileSystemCheckerFactory not explicitaly initialized, "
              + "using default checker type :{}",
          defaultType);
      init(defaultType);
    }
    return instance;
  ...
  /**
   * Initializes the class by creating the singleton instance if not already done, does nothing if
   * it already exists and has the chosenType is the same as the provided FileSystemCheckerType
   * throws an IllegalStateException otherwise
   *
   * @param type
   * @throws IllegalArgumentException if type is null
   * @throws IllegalStateException if class already initialized with a different
  ...
  public static void init(FileSystemCheckerType type)
    ...
    if (type == null) {
      log.error(
          "Unable to init FileSystemCheckerFactory. Received null FileSystemCheckerType parameter!");
    ...
    if (instance == null) {
      ...
      if (!instance.chosenType.equals(type)) {
        log.warn(
            "FileSystemCheckerFactory already initialized for {}. "
        ...
        throw new IllegalStateException(
            "Asked to initialize the already initialized FileSystemCheckerFactory "
                ...
                + type
                + ". Current FileSystemCheckerType is "
                + instance.chosenType);
      } else {
        log.info(
            "Asked to re-initialize the already initialized FileSystemCheckerFactory, nothing to do");
      }
    }
  }

  /**
   * Creates the proper FileSystemChecker implementation object using the chosenType available
   * attribute
   *
   * @return
   */
  public FileSystemChecker createFileSystemChecker()
    ...
    switch (this.chosenType) {
      ...
      default:
        log.error(
            "No correct FileSystemChecker setted : "
                + this.chosenType
        ...
        throw new IllegalStateException(
            "No correct FileSystemCheckerType setted : "
                + this.chosenType
