...
public class MtabRow
{
    ...
    {
        if (device == null || device.trim().equals("") || mountPoint == null || mountPoint.trim().equals("")
                || fileSystem == null || fileSystem.trim().equals(""))
        {
            throw new IllegalArgumentException("Received invalid arguments : device = " + device
     ...
     * 
     * @param elementsList [device , mountPoint, fileSystem, mountOptions, dump, fsckOrderPosition]
    ...
    public MtabRow(List<String> elementsList) throws IllegalArgumentException
    {
        ...
        for(int i = 0 ; i < 5 ; i++)
        {
            /* all the arguments must be strings not empty apart from the one at index 4 (mount options) */
            if((elementsList.get(i) == null || elementsList.get(i).trim().length() == 0) && i != 3)
            {
                throw new IllegalArgumentException("Received an argument list where element at index " + i
        ...
        {
            throw new IllegalArgumentException("Received an illegal argument at index 4. "
                    + "It as to be an integer (the dump value), received value : " + elementsList.get(4)
        ...
        {
            throw new IllegalArgumentException("Received an illegal argument at index 5. "
                    + "It as to be an integer (the fsck order position value), received value : "
                    + elementsList.get(4) + " . NumberFormatException : " + e.getMessage());
        }
    }
    ...
    {
        return "MtabRow [device=" + device + ", mountPoint=" + mountPoint + ", fileSystem=" + fileSystem
                + ", mountOptions=" + mountOptions + ", dump=" + dump + ", fsckOrderPosition="
                + fsckOrderPosition + "]";
    }    
}
