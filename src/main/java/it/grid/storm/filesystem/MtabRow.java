...
public class MtabRow {
  ...
  public MtabRow(
      ...
      boolean dump,
      int fsckOrderPosition) {

    if (device == null
        || device.trim().equals("")
        || mountPoint == null
        || mountPoint.trim().equals("")
        || fileSystem == null
        || fileSystem.trim().equals("")) {
      throw new IllegalArgumentException(
          "Received invalid arguments : device = "
              + device
              + " mountPoint = "
              + mountPoint
              + " fileSystem = "
  ...
  public MtabRow(List<String> elementsList) throws IllegalArgumentException {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      if ((elementsList.get(i) == null || elementsList.get(i).trim().length() == 0) && i != 3) {
        throw new IllegalArgumentException(
            "Received an argument list where element at index "
                + i
                + " is invalid : \'"
                + elementsList.get(i)
                + "\'");
    ...
    if (dumpValue != 1 && dumpValue != 0) {
      throw new IllegalArgumentException(
          "Received an illegal argument at index 4. "
              + "It can be only 0 or 1, received value : "
              + elementsList.get(4));
    }
