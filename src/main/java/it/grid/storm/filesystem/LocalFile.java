 ...
 *     <p>The it.grid.storm.filesystem.File class
 */
/*
...
/**
 * Façade for operations on a filesystem entry (file or directory). All operations on the filesystem
 * should be performed by creating an instance of the {@link it.grid.storm.filesystem.File} class,
 * and using its methods to create or modify a filesystem entry. From {@link java.io.File} the
 * following methods are inherited: {@link java.io.File#delete() delete}, {@link
 * java.io.File#exists() exists}, {@link java.io.File#getAbsolutePath() getAbsolutePath}, {@link
 * java.io.File#getName() getName}, {@link java.io.File#isDirectory() isDirectory}, {@link
 * java.io.File#isFile() isFile}, {@link java.io.File#list() list}, {@link java.io.File#mkdir()
 * mkdir} , {@link java.io.File#mkdirs() mkdirs}; the methods {@link java.io.File#getParentFile()
 * getParentFile} and {@link java.io.File#listFiles() listFiles} have been overridden to return
 * {@link it.grid.storm.filesystem.File} objects instead of {@link java.io.File} ones. Additional
 * methods are provided to manipulate this file's ACL in a filesystem-indepenent way; all ACL
 * operations take a LocalUser identity (see {@link it.grid.storm.griduser.LocalUser}) and a {@link
 * FilesystemPermission} permission representation. A pathname need not exist for an associated
 * it.grid.storm.filesystem.File object to be created; indeed, you should create an instance
 * associated to a non existing pathname and then invoke {@link #create()} or {@link
 * #makeDirectory()} to create a file or a directory, respectively. The constructor for this class
 * requires a pathname and a {@link Filesystem} object. The passed {@link Filesystem} object need to
 * match the filesystem-type of the filesystem the passed pathname resides on. Therefore, {@link
 * it.grid.storm.filesystem.File} objects can only be created from a factory method that has all the
 * required pieces of information; {@link it.grid.storm.namespace.StoRI} looks like the right class
 * for that.
 *
 * @see java.io.File
...
public class LocalFile {
   ...
   * @param fs The {@link it.grid.storm.filesystem.Filesystem} object to use for operations on this
   *     file.
  ...
  public LocalFile(final LocalFile parent, final String name, final FilesystemIF fs)
    ...
    assert (localFile.isAbsolute())
        : "Non-absolute path in constructor File(File,String,Filesystem)";
   ...
   *
   * @param pathname The pathname wrapped by this PFN; <em>must</em> be absolute, or code will fail
   *     in an assertion.
   * @param fs The {@link it.grid.storm.filesystem.Filesystem} object to use for operations on this
   *     file.
   * @see it.grid.storm.filesystem.Filesystem
  ...
  /**
   * Constructor, taking parent pathname (as a <code>String</code>), child name (as a <code>String
   * </code>) and the hosting filesystem object. From <i>parent</i> and <i>child</i> an absolute
   * pathname must result, following the rules in {@link java.io.File#File(String,String)}.
   *
  ...
  public LocalFile(final String parent, final String name, final FilesystemIF fs)
    ...
    assert (localFile.isAbsolute())
        : "Non-absolute path in constructor File(String,String,Filesystem)";
   ...
   * operate on the specified <code>fileOrDirectory</code> in the mode given by <code>accessMode
   * </code>, according to the permissions set on the filesystem.
   */
   ...
   * given {@link LocalUser} instance <i>u</i>. If no ACE for that group is found, return {@link
   * Filesystem#NONE}.
   *
   ...
   *     LocalUser <i>u</i> in this file ACL, or <code>null</code> if no ACL entry for that user was
   *     found.
   */
   ...
   * return the permission associated with the local account primary group of the given {@link
   * LocalUser} instance <i>u</i>. If no ACE for that group is found, return {@link
   * Filesystem#NONE}.
   *
   * @param u the LocalUser whose local account primary GID's permissions are to be retrieved.
  ...
  /**
   * Return (possibly cached) file last modification time, as a UNIX epoch. Same as {@link
   * #lastModified()}, although this uses the host Filesystem native <code>stat()</code> -like
   * calls. May return inaccurate results, as some filesystems (notably GPFS) provide the choice
   * between filesystem-specific calls for accurate reporting (slower, as it may imply synchronizing
   * the metadata cache) or standard libc calls that report possibly outdated information.
   *
   * @return time (seconds since the epoch) this file was last modified.
   * @see #lastModified()
  ...
  /**
   * Return the (possibly outdated) size in bytes of this file. Same as {@link #length()}, although
   * this uses the host Filesystem native <code>stat()</code>-like calls. May return inaccurate
   * results, as some filesystems (notably GPFS) provide the choice between filesystem-specific
   * calls for accurate reporting (slower, as it may imply synchronizing the metadata cache) or
   * standard libc calls that report possibly outdated information.
   *
   * @return size (in bytes) of this file
   * @see #length()
   * @see #getExactSize()
   ...
   *
   * <p>Adds the specified permission to the ones that the primary group of the given LocalUser
   ...
   *
   * <p>If no ACE is present for the specified group, then one is created and its permission value
   ...
   *     <i>u</i> in this file ACL, or {@link Filesystem#NONE} if no ACE for that group was found.
   */
  public FilesystemPermission grantGroupPermission(
      final LocalUser u, final FilesystemPermission permission) {
   ...
   *
   * <p>Adds the specified permissions to the ones that the local account UID of the given LocalUser
   * <i>u</i> already holds on this file or directory: all permission bits that are set in
   * <i>permission</i> will be set in the appropriate user ACE in the file ACL.
   *
   * <p>If no ACE is present for the specified user, then one is created and its permission value is
   ...
   *
   * <p>Removes the ACE (if any) of the primary group of the given LocalUser <i>u</i> from this file
   * or directory ACL. Returns the permission formerly associated with that group.
   *
   * <p>If the given group is the file owning group, then its ACE is set to {@link Filesystem#NONE},
   * rather than removed.
   *
   * @param u the LocalUser whose local account primary GID's ACE is to be altered.
   * @return permission formerly associated to the local account primary GID of the given LocalUser
   ...
   *
   * <p>Removes the ACE (if any) of the primary user of the given LocalUser <i>u</i> from this file
   * or directory ACL. Returns the permission formerly associated with that user.
   *
   * <p>If the given user is the file owner, then its ACE is set to {@link Filesystem#NONE}, rather
   * than removed.
   *
   * @param u the LocalUser whose local account UID's ACE is to be altered.
   * @return permission formerly associated to the local account UID of the given LocalUser <i>u</i>
   ...
   *
   * <p>Removes the specified permission from the ones that the primary group of the given LocalUser
   ...
   *
   * <p>If no ACE is present for the specified group, then one is created and its permission value
   * is set to {@link Filesystem#NONE}.
   *
   * @param u the LocalUser whose local account primary GID's ACE is to be altered.
   ...
   *
   * <p>Removes the specified permission from the ones that the primary user of the given LocalUser
   * <i>u</i> local account already holds on this file or directory: all permission bits that are
   ...
   *
   * <p>Sets the ACE of the primary group of the given LocalUser <i>u</i> to the given
   ...
   * permission.
   *
   * <p>Sets the ACE of the primary user of the given LocalUser <i>u</i> to the given
