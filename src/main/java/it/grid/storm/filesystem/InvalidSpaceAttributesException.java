...
public class InvalidSpaceAttributesException extends Exception {
  ...
  */
  ...
  // boolean true if garanteedSize is null
  private boolean nullGuarSize = false;

  // boolean true if totalSize is null
  private boolean nullTotSize = false;

  // boolean true if spaceFile is null
  private boolean nullSpaFil = false;

  // boolean true if SpaceSystem is null
  private boolean nullSS = false;

  // boolean true if totalSize is Empty
  private boolean emptyTotSize = false;

  // boolean true if guaranteedSize and totalSize are not null, not
  // empty, and when interpreted as double of size BYTE
  // it is _guaranteedSize_ that is GREATER
  // than _totalSize_
  private boolean greater = false;

  // double that gets set only if (greater) is
  ...
  public InvalidSpaceAttributesException(TSizeInBytes guaranteedSize, TSizeInBytes totalSize,
      LocalFile spaceFile, SpaceSystem ss) {
    ...
    greater =
        (!nullGuarSize) && (!nullTotSize) && (!guaranteedSize.isEmpty()) && (!totalSize.isEmpty())
