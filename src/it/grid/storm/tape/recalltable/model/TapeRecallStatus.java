...
public enum TapeRecallStatus {
    ...
    public static TapeRecallStatus getRecallTaskStatus(String status) {
        if (status.toLowerCase().equals(TapeRecallStatus.ABORTED.toString().toLowerCase())) {
            return TapeRecallStatus.ABORTED;
        }
        if (status.toLowerCase().equals(TapeRecallStatus.SUCCESS.toString().toLowerCase())) {
            return TapeRecallStatus.SUCCESS;
        }
        if (status.toLowerCase().equals(TapeRecallStatus.IN_PROGRESS.toString().toLowerCase())) {
            return TapeRecallStatus.IN_PROGRESS;
        }
        if (status.toLowerCase().equals(TapeRecallStatus.ERROR.toString().toLowerCase())) {
            return TapeRecallStatus.ERROR;
        }
        if (status.toLowerCase().equals(TapeRecallStatus.QUEUED.toString().toLowerCase())) {
    ...
    /**
     * Returns true if the provided status id refers to a status that does not allows 
     * state transitions
     * 
     * @param statusId
     * @return
    ...
    /**
     * Determines if there is a sequence of transitions that can
     * bring from this status to the given status parameter 
     * 
     * NOTE: valid transitions are : queued -> inProgress
     *                               inProgress -> \<any final status\>
     * @param otherStatus
     * @return
     */
