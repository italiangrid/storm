...
//    private TransferResult transferResult = null; //Results of the transfer operation
    ...
    public void doIt()
    {
        ...
        if (getResult.successful() && getResult.status().getStatusCode() == TStatusCode.SRM_FILE_PINNED)
        {
            ...
            /*
             * ATTENTION! the following check for SRM_SUCCESS is done to keep compatibility between StoRM
             * servers which use a hack thereby switching to state SRM_SUCCESS instead of transiting to
             * SRM_SPACE_AVAILABLE!!! But for any other SRM2.2 compliant server, SRM_SPACE_AVAILABLE
             * suffices!!!
             */
            if (putResult.successful()
                    && ((putResult.status().getStatusCode().equals(TStatusCode.SRM_SPACE_AVAILABLE)) || (putResult.status()
            ...
            else
            {
                ...
                log.error(message);
                requestData.changeStatusSRM_FAILURE("PUT part of srmCopy failed! " + message);
                this.failure = true;
            }
        }
        else
        ...
        log.info("Finished handling Copy chunk for user DN: " + this.gu.getDn() + "; fromSURL: "
                + this.requestData.getSURL() + "; toSURL: " + this.requestData.getDestinationSURL()
                + " result is: " + this.requestData.getStatus());
    }
    ...
    public String getName() {
        return "Copy for SURL " + requestData.getSURL() + " to SURL " + requestData.getDestinationSURL();
    }
    ...
    public enum ResultType{ PUT, GET, TRANSFER}
    /**
    ...
    protected class GetOperationResult implements Result
    {
        ...
        {
            return "GetOperationResult: successful=" + successful + "; status=" + status + "; getTURL="
                    + getTURL + "; filesize=" + filesize + "; requestToken=" + rt;
        }
    }
    ...
    protected class PutOperationResult implements Result
    {
        ...
        public String toString()
        {
            return "PutOperationResult: successful=" + successful + "; status=" + status + "; putTURL="
                    + putTURL + "; failureExplanation=" + failureExplanation;
        }
    }
}
