...
public class PushCopyPutVisitor implements CopyVisitor
{
    ...
    public Result visit(VisitableCopy copy)
    {
        try
        {
            TLifeTimeInSeconds retryTime = TLifeTimeInSeconds.make(Configuration.getInstance()
                                                                                .getSRMClientPutTotalRetryTime(),
                                                                   TimeUnit.SECONDS);
            try
            {
                ...
                SRMPrepareToPutReply reply = srmClient.prepareToPut(copy.getGu(), copy.getRequestData().getDestinationSURL(),
                                                                    copy.getRequestData().getLifetime(),
                                                                    copy.getRequestData().getFileStorageType(),
                                                                    copy.getRequestData().getSpaceToken(), getFileSize,
                                                                    TransferProtocol.GSIFTP,
                                                                    "StoRM Remote PtP for (push) srmCopy",
                                                                    copy.getRequestData().getOverwriteOption(), retryTime);
                copy.getLog().debug("... got it! Reply was: " + reply);
                // Polling...
                long timeOut = new Date().getTime() + Configuration.getInstance().getSRMClientPutTimeOut()
                        * 1000; 
                ...
                try
                {
                    do
                    {
                        ...
                        statusOfPutRequestReply = srmClient.statusOfPutRequest(reply.requestToken(), copy.getGu(),
                                                                               copy.getRequestData().getDestinationSURL());
                        replyCode = statusOfPutRequestReply.returnStatus().getStatusCode();
                        timedOut = (new Date().getTime() > timeOut);
                        copy.getLog().debug("PUSH COPY CHUNK: reply was " + statusOfPutRequestReply
                                + "; the reply code was: " + replyCode + "; timedOut is:" + timedOut);
                ...
                } catch(SRMClientException e2)
                {
                    ...
                    copy.getLog().error("ERROR IN PushCopyChunk! PutOperation failed: SRMClient could not do an srmStatusOfPutRequest! "
                            + e2);
                    ...
                    return copy.buildOperationResult(
                                                  "SRMClient failure! Could not do an srmStatusOfPutRequest! "
                                                          + e2, ResultType.PUT);
                }
                ...
                if (timedOut)
                {
                    ...
                    return copy.buildOperationResult("PutOperation timed out!", ResultType.PUT);
                }
            ...
            } catch(SRMClientException e1)
            {
                ...
                copy.getLog().error("ERROR IN PushCopyChunk! PutOperation failed: SRMClient could not do an srmPrepareToPut! "
                        + e1);
                StringBuffer sb = new StringBuffer();
                sb.append("Parameters passed to client: ");
                sb.append("GridUser:");
                ...
                sb.append(".");
                copy.getLog().debug(sb.toString());
                return copy.buildOperationResult("SRMClient failure! Could not do an srmPrepareToPut! " + e1, ResultType.PUT);
            } catch(NoSRMClientFoundException e1)
            {
                copy.getLog().error("ERROR IN PushCopyChunk! Cannot call remote SRM server because no SRM client could be loaded! "
                        + e1, ResultType.PUT);
                return copy.buildOperationResult(
