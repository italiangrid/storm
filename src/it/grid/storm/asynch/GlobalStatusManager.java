 ...
 * supplied ChunkData: it encloses the logic for computing such global state.
 * It must be intialised with the TRequestToken of the request; any ChunkData
 * that must be considered in the computation must be added with the addChunk
 * method; when all ChunkData have been added, finishedAdding method must be
 * called. Chunks then must invoke either successfulChunk or failedChunk passing
 * their ChunkData as arguments, in order for the OverallRequestStatus to be
 * updated accordingly. When all chunks complete, the state is updated into
 * persistence automatically.
 *
 * Finally, chunks may also invoke expiredSpaceLifetimeChunk to signal that the
 * associated space token has its lifetime expired. This is a special state: it
 * is not possible to have a request with some chunks in this situation while
 * others are not. All chunks of the same request should invoke this method: it
 * is an anomaly if it does not occur; it gets signalled properly.
 *
...
public class GlobalStatusManager {
    ...
    /**
     * Method that tells this OverallRequest to consider the state of
     * the supplied ChunkData, when computing the global state. If finishedAdding
     * method has already been invoked, this method has no effect.
     */
    ...
    /**
     * Method used to indicate that no other ChunkDAta will be considered for the
     * computation of the global state. After invoking this method, all subsequent
     * calls to addChunk will be ignored.
     */
    ...
    /**
     * Method used to communicate that the given ChunkData c has transited to a final
     * successful state. If it is null, there are no Chunks being kept track of,
     * or that specific ChunkData is not being tracked, an error message gets written
     * to the logs and the global state transits to ERROR.
     */
    synchronized public void successfulChunk(ChunkData c) {
        ...
        if ((c!=null) && (!chunks.isEmpty()) && (chunks.remove(new Long(c.primaryKey()))!=null)) {
            ...
            if (finished && (chunks.isEmpty())) {
                ...
                log.error("ERROR IN GLOBAL STATUS EVALUATION! An impossible logic codition has materialised: it may be due to multithreading issues! Request is "+rt);
    ...
    /**
     * Method used to communicate that the given Chunk c has transited to a final
     * failed state. If it is null, there are no Chunks being kept track of,
     * or that specific Chunk is not being tracked, an error message gets written
     * to the logs and the global state transits to ERROR.
     */
    ...
    /**
     * Method used to communicate that the given Chunk c has transited to a final
     * failed state specifically due to an expired SpaceLifetime. If it is null,
     * there are no Chunks being kept track of, or that specific Chunk is not being
     * tracked, an error message gets written to the logs and the global state
     * transits to ERROR.
     */
    ...
    private void saveRequestState() {
        ...
        try {
            ...
            if (internal==InternalState.ERROR) {
                ...
                retstat = new TReturnStatus(TStatusCode.SRM_PARTIAL_SUCCESS,"Global status cannot be evaluated: single file status must be checked.");
                log.error("ERROR IN GLOBAL STATUS EVALUATION! "+internal+" was attempted to be written into persistence, but it is not a final state!");
                log.error("Request: "+rt);
            }
    ...
    /**
     * Auxiliary private class that keeps track of internal state of request,
     * with respect to all the chunks.
     *
     * @author  EGRID - ICTP Trieste
     * @version 2.0
     * @date    September, 2006
