...
public class AdvancedPicker {
    ...
    private long delay = Configuration.getInstance().getPickingInitialDelay()*1000;   //Delay time before starting retriever thread! Set to 5 seconds
    private long period = Configuration.getInstance().getPickingTimeInterval()*1000;  //Period of execution of retrieving! Set to 1 minute

    private boolean abort = false; //boolean that indicates there is a token to abort!
    private Collection abortSURLS = null; //Collection with chunks to abort!
    private TRequestToken abortToken = null; //TRequestToken of request to abort!
     ...
     * TRequestType; the global status of each request then changes to SRM_SUCCESS,
     * appropriate Feeders get created and forwarded to the crusher scheduler.
     *
     ...
     * to SRM_FAILURE, as well as that of each chunk. Under anomalous circumstances
     * it could be that it is not possible to update the status of each chunk, in
     * which case the chunk status remains SRM_REQUEST_QUEUED. This last case is
     * particularly pernicious, so a FATAL log is signalled: it means the code was
     * not updated!
     */
    public void retrieve() {
        ...
        //executed)
        if (abort) {
            abortToken = null; //BE CAREFUL!!! FIRST set abortToken and abortSURLS to null, and THEN set the flag to false!
            abortSURLS = null;
            abort = false;
        }
    }
