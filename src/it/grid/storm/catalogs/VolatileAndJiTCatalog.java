 ...
 * files will remain in the system: upon expiry those files are removed.
 *
 * In particular the srmPrepareToPut analyses the request and if the specified
 * file is set to Volatile, then it calls on the catalogue to add the
 * corresponding entry for the given fileLifetime. If StoRM is configured for
 * JiT, another method is invoked to add an entry to keep track of the ACLs for
 * the desired pinLifetime.
 * 
 * For srmPrepareToGet, only if StoRM is configured for JiT ACLs then a method
 * is invoked to add the corresponding entry for the given pinLifetime.
 *
 * Repeatedly putting the same Volatile file, will overwrite existing
 * fileLifetime only if the overwrite option allows file overwriting. If JiT is
 * enabled and it is a new user that is putting again the same file in, a new
 * pinLifetime entry is added; but if it is the same user, the pinLifetime WILL
 * be changed provided the new expiry exceeds the current one!
 *
 ...
 * pinLifetime provided the new expiry exceeds the current one!
 *
 * In case the pinLifetime exceeds the fileLifetime, the fileLifetime is used
 * as ceiling. This may occur when a file is Put and defined Volatile, but with
 * a pinLifetime that is longer than that of the pin. Or if _subsequent_ calls
 * to PtG specify a pinLifetime that lasts longer. To be more precise, the
 ...
 * the files are erased, also cleaning up the catalogue.
 * 
 * As a last note, the catalogue checks periodically its entries for any expired
 * ones, and then proceeds with purging; this frequency of cleaning is specified
 * in a configuration parameter, and the net effect is that the pinning/volatile
 * may actually last longer (but never less) because the self cleaning mechanism
 * is active only at those predetermined times.
 *
...
public class VolatileAndJiTCatalog {
    ...
    private final Timer cleaner = new Timer(); //Timer object in charge of cleaning periodically the Catalog!
    private final long delay = Configuration.getInstance().getCleaningInitialDelay()*1000;  //Delay time before starting cleaning thread! Set to 1 minute
    private final long period = Configuration.getInstance().getCleaningTimeInterval()*1000; //Period of execution of cleaning! Set to 1 hour
     ...
     * to be used by code handling srmAbort command.
     *
     * If the entry does not exist, then nothing happens and a warning gets written
     * in the logs; otherwise the entry gets its start time set to now, and its
     * lifetime set to zero; in case more than one matching entry is found, a
     * message gets written to the logs, and the updating continues anyway as
     * explained.
     *
     * At this point, when the garbage colelctor wakes up the entry is cleanly
     * handled (physical ACL is removed, catalog entry removed, etc.); or an
     * earlier cleaning can be forced by invoking directly the purge mehod.
     *
     * The method returns FALSE in case no entry was found or the supplied
    ...
    synchronized public boolean expireJiT(PFN pfn, LocalUser localUser, FilesystemPermission acl) {
        ...
        if (!ok) {
            ...
            //is not yet reached!
            long pinStart = (Calendar.getInstance().getTimeInMillis() / 1000) - 10; //seconds needed and not milliseconds!
     ...
     * code handling srmAbort command, and by srmPutDone.
     *
     * Notice that the Traverse on the parents is NOT removed! This is to
     * accomodate for the use case of a user that has run many PtP on different
     * SURLs but that are all contained in the same directory tree! In practice,
     * this method removes R and W permissions.
     *
     * If any entry does not exist, then nothing happens and a warning gets
     * written in the logs; otherwise entries get their start time set to now,
     * and the lifetime set to zero; in case more than one matching entry is
    ...
    /**
     * Method that purges the catalog, removing expired ACLs and deleting
     * expired Volatile files.
     *
     * When Volatile entries expire, any realted JiT will automatically expire
     * too, regardless of the specified pinLifetime: that is, fileLifetime wins
     * over pinLifetime.
     *
     * WARNING! Notice that the catalogue DOES get cleaned up even if
     * the physical removal of the ACL or erasing of the file fails.
     */
    ...
    /**
     * Private method used to return a String representation of the
