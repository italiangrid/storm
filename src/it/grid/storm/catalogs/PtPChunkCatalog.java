...
public class PtPChunkCatalog {
    ...
    private final long delay = Configuration.getInstance().getTransitInitialDelay()*1000;  //Delay time before starting cleaning thread! Set to 1 minute
    private final long period = Configuration.getInstance().getTransitTimeInterval()*1000; //Period of execution of cleaning! Set to 1 hour
     ...
     *
     * Moreover, the physical file corresponding to the SURL gets removed; then
        ...
        TimerTask transitTask = new TimerTask() {
            public void run() {
                ...
                if (!ids.isEmpty()) {
                    ...
                    StoRI auxStoRI = null;
                    PFN auxPFN = null;
                    LocalFile auxFile = null;
                    Calendar start = Calendar.getInstance();
                    for (Iterator i = reduced.iterator(); i.hasNext(); ) {
                        try {
                            auxReduced = (ReducedPtPChunkData) i.next();
                            auxStoRI = NamespaceDirector.getNamespace().resolveStoRIbySURL(auxReduced.toSURL());
                            auxPFN = auxStoRI.getPFN();
                            //
                            //remove file
                            try {
     ...
     * the request is dropped, gets logged and an attempt is made to write in the
     * DB that the chunk was malformed; the processing continues with the next
     * part. Only the valid chunks get returned.
     *
     * If there are no chunks to process then an empty Collection is returned,
     * and a messagge gets logged.
     *
     * NOTE! Chunks in SRM_ABORTED status are NOT returned! This is imporant
     * because this method is intended to be used by the Feeders to fetch all
     ...
     * does not allow a ReducedPtPChunkData Object to be created, then that chunk
     ...
     * All valid chunks get returned: the others get dropped.
     *
     * WARNING! If there are no chunks associated to any of the given IDs, no
     * messagge gets written in the logs!
     */
    synchronized public List fetchReducedPtPChunkDataFor(List volids) {
        Collection cl = dao.fetchReduced(volids);
    ...
    /**
     * Method that synchronises the supplied PtPChunkData with the information
     * present in Persistence. BE WARNED: a new object is returned, and the
     * original PtPChunkData is left untouched! null is returned in case of
     * any error.
     */
    ...
    /**
     * Private method used to create a PtPChunkData object, from a
     * PtPChunkDataTO and TRequestToken. If a chunk cannot be created,
     * an error messagge gets logged and an attempt is made to signal
     * in the DB that the chunk is malformed.
     */
    private PtPChunkData makeOne(PtPChunkDataTO auxTO, TRequestToken rt) {
        ...
        try {
            pinLifetime = TLifeTimeInSeconds.make(PinLifetimeConverter.getInstance().toStoRM(auxTO.pinLifetime()), TimeUnit.SECONDS);
        } catch (InvalidTLifeTimeAttributeException e) {
            sb.append("\n");
            sb.append(e);
        }
        ...
        try {
            aux = new PtPChunkData(rt, toSURL, pinLifetime, fileLifetime, fileStorageType, spaceToken, expectedFileSize, transferProtocols, overwriteOption, status, transferURL);
            aux.setPrimaryKey(auxTO.primaryKey());
     ...
     * on the supplied SURL, and whose state is SRM_SPACE_AVAILABLE, in which
     * case true is returned. In case none are found or there is any problem, false
     * is returned.
     */
     ...
     * not been changed (a user forgot to run srmPutDone)!
     *
     * The method returns a List containing all ids of transited chunks that are
     * also Volatile.
     */
    synchronized public List transitExpiredSRM_SPACE_AVAILABLE() {
        return dao.transitExpiredSRM_SPACE_AVAILABLE();
    }

    /**
     * Method used to transit the specified Collection of ReducedPtPChunkData of
     * the request identified by the supplied TRequestToken, from SRM_SPACE_AVAILABLE
     ...
     * status. The global status of the request is not changed.
     *
