...
/**
 * Class that handles DB representation of a TLifetimeInSeconds, in particular it
 * takes care of protocol specification:
 *
 * 0/null/negative are translated as default StoRM configurable values.
 * StoRMs Empty TLifeTimeInSeconds is translated as 0.
 *
 * @author  EGRID ICTP
 * @version 1.0
 * @date    March 2007
...
public class PinLifetimeConverter {
    ...
    private static PinLifetimeConverter stc = new PinLifetimeConverter(); //only instance
    ...
    public long toStoRM(int s) {
        if (s<=0) return Configuration.getInstance().getPinLifetimeMinimum();
