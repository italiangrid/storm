...
public class BackgroundDU {
    ...
    /**
     * CompletionService for running submitted tasks. All tasks are submitted through this CompletionService to provide
     * blocking, queued access to completion information.
     */
    private final CompletionService<DUResult> completionService = new ExecutorCompletionService<DUResult>(exec);
    ...
    /**
     * @param interrupt <tt>true</tt> if the threads executing tasks task should be interrupted; otherwise, in-progress
     *            tasks are allowed to complete normally.
     * @param waitMillis maximum amount of time to wait for tasks to complete.
     * @return <tt>true</tt> if this all the running tasks terminated and <tt>false</tt> if the some running task did
     *         not terminate.
