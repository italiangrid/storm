...
public class PrepareToGetRequestStatusCommand extends FileTransferRequestStatusCommand implements Command
{
    ...
    protected TReturnStatus computeRequestStatus(ArrayOfTSURLReturnStatus arrayOfFileStatuses)
    {
        ...
        if(atLeastOneSuccessOrPinned && !(atLeastOneQueued || atLeastOneInProgress || atLeastOneAborted || atLeastOneFailed))
        {
            return CommandHelper.buildStatus(TStatusCode.SRM_SUCCESS, "Request executed successfully");
        }
        if(((atLeastOneSuccessOrPinned || atLeastOneAborted || atLeastOneFailed) && atLeastOneQueued) || atLeastOneInProgress)
        {
            return CommandHelper.buildStatus(TStatusCode.SRM_REQUEST_INPROGRESS, "Request in progress");
        }
        if(atLeastOneSuccessOrPinned && atLeastOneFailed && !(atLeastOneQueued || atLeastOneInProgress))
        {
            return CommandHelper.buildStatus(TStatusCode.SRM_PARTIAL_SUCCESS, "Partial success, some surls are failed");
        }
        if(atLeastOneAborted && !(atLeastOneQueued || atLeastOneInProgress || atLeastOneSuccessOrPinned || atLeastOneFailed))
        {
