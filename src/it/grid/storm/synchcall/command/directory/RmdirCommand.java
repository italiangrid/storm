...
public class RmdirCommand extends DirectoryCommand implements Command {
    ...
    public OutputData execute(InputData data) {
        ...
        if (!surl.isEmpty())
        {
            ...
            {
                log.error("srmRmdir: Unable to build StoRI by surl and user. IllegalArgumentException: " + e.getMessage());
        ...
        if (inputData instanceof IdentityInputData)
        {
            isSpaceAuthorized = spaceAuth.authorize(((IdentityInputData) inputData).getUser(), SRMSpaceRequest.RMD);
        }
        else
        {
        ...
        if (inputData instanceof IdentityInputData)
        {
            decision = AuthzDirector.getPathAuthz().authorize(((IdentityInputData) inputData).getUser(), SRMFileRequest.RMD, stori);
        }
        else
        {
            decision = AuthzDirector.getPathAuthz().authorizeAnonymous(SRMFileRequest.RMD, stori.getStFN());
        }
        if (decision.equals(AuthzDecision.PERMIT))
        {
            log.debug("RMDIR is authorized for " + DataHelper.getRequestor(inputData)
                    + " and the directory = " + stori.getPFN() + " with recursove opt = " + recursive);
    ...
    private TReturnStatus manageAuthorizedRMDIR(LocalFile directory, boolean recursive)
    {
        ...
        if ((directory.exists()) && (directory.isDirectory()))
        {
            if (recursive)
            {
                ...
                log.debug(SRM_COMMAND + ": Recursive deletion. Removing dir with all files included! ");
            ...
            {
                returnStatus = CommandHelper.buildStatus(TStatusCode.SRM_NON_EMPTY_DIRECTORY,
    ...
    private boolean removeFile(LocalFile file)
    {
        ...
        { 
            if (file.isDirectory())
            {
                ...
                { 
                    result = false;
                    log.info(SRM_COMMAND + ": Unable to delete the target file '" + file + "' . It is a not-empty directory.");
                }
                else
                { 
                    result = file.delete();
