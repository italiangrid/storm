...
public class SurlStatusCommandHelper
{
    ...
    public static Map<TSURL, TReturnStatus> getSurlsStatus(InputData inputData, TRequestType requestType) throws IllegalArgumentException, RequestUnknownException, UnknownTokenException, ExpiredTokenException
    {
        ...
        {
            return getSurlsStatus(((ManageFileTransferRequestFilesInputData)inputData).getRequestToken(), ((ManageFileTransferRequestFilesInputData)inputData).getArrayOfSURLs(), requestType);
        }
        else
        {
            if(inputData instanceof ManageFileTransferFilesInputData)
            {
                ...
                {
                    return getSurlsStatus(((ManageFileTransferFilesInputData)inputData).getArrayOfSURLs(), ((IdentityInputData)inputData).getUser(), true, requestType, true);
                }  
                else
                {
                    return getSurlsStatus(((ManageFileTransferFilesInputData)inputData).getArrayOfSURLs(), null, false, requestType, true);
                }
            }
            else
            {
                ...
                {
                    return getSurlsStatus(((ManageFileTransferRequestInputData)inputData).getRequestToken(), requestType);
                }
                else
                {
    ...
    public static Map<TSURL, TReturnStatus> getSurlsStatus(InputData inputData) throws IllegalArgumentException, RequestUnknownException, UnknownTokenException, ExpiredTokenException
    {
        ...
        else
        {
            if(inputData instanceof ManageFileTransferFilesInputData)
            {
                if(inputData instanceof IdentityInputData)
                {
                    return getSurlsStatus(((ManageFileTransferFilesInputData)inputData).getArrayOfSURLs(), ((IdentityInputData)inputData).getUser(), true, null, false);
                }  
                else
                {
                    return getSurlsStatus(((ManageFileTransferFilesInputData)inputData).getArrayOfSURLs(), null, false, null, false);
                }
            }
            else
            {
    ...
    public static Map<TSURL, TReturnStatus> getSurlsStatus(TRequestToken requestToken, TRequestType requestType)
            ...
            ExpiredTokenException
    {
        ...
        Map<TSURL, TReturnStatus> surlsStatuses = SurlStatusManager.getSurlsStatus(requestToken, requestType);
    ...
    public static Map<TSURL, TReturnStatus> getSurlsStatus(TRequestToken requestToken) throws RequestUnknownException, IllegalArgumentException,
    ...
    private static Map<TSURL, TReturnStatus> getSurlsStatus(ArrayOfSURLs arrayOfSURLs, GridUserInterface user,
            boolean withUser, TRequestType requestType, boolean withRequestType) throws RequestUnknownException, IllegalArgumentException
    {
        if (arrayOfSURLs == null || (withUser && user == null) || (withRequestType && requestType == null))
        {
            throw new IllegalArgumentException("unable to get the statuses, invalid arguments: arrayOfSURLs="
                    + arrayOfSURLs + " hasUser=" + withUser + " user=" + user + " withRequestType=" + withRequestType + " requestType=" +requestType);
        }
        ...
        for (TSURL surl : arrayOfSURLs.getArrayList())
        {
            try
            {
                ...
                else
                {
                    if(withRequestType)
                    {
                        surlsStatuses.put(surl, SurlStatusManager.getSurlsStatus(surl, requestType));
                    }
                    else
                    {
    ...
    public static Map<TSURL, TReturnStatus> getSurlsStatus(TRequestToken requestToken, ArrayOfSURLs arrayOfSURLs, TRequestType requestType)
            throws RequestUnknownException, IllegalArgumentException, UnknownTokenException,
            ExpiredTokenException
    {
        ...
        Map<TSURL, TReturnStatus> surlsStatuses = SurlStatusManager.getSurlsStatus(requestToken,
                                                                                   arrayOfSURLs.getArrayList(), requestType);
        if (surlsStatuses.isEmpty())
        {
            log.info("No one of the requested surls found for the provided token");
            throw new RequestUnknownException("No one of the requested surls found for the provided token");
    ...
    public static Map<TSURL, TReturnStatus> getSurlsStatus(TRequestToken requestToken,
            ArrayOfSURLs arrayOfSURLs) throws RequestUnknownException,
    ...
    public static ArrayOfTSURLReturnStatus prepareSurlsReturnStatus(Map<TSURL, TReturnStatus> surlStastuses)
    {
        ...
        {
            surlReturnStatuses.addTSurlReturnStatus(CommandHelper.buildStatus(surlStatus.getKey(), surlStatus.getValue()));
        }
            ...
            ArrayOfSURLs arrayOfUserSURLs)
    {
        ...
        for(TSURL surl : arrayOfUserSURLs.getArrayList())
        {
            ...
            if (status == null)
            {
                status = CommandHelper.buildStatus(TStatusCode.SRM_INVALID_PATH, "Invalid SURL");
            }
            surlReturnStatuses.addTSurlReturnStatus(CommandHelper.buildStatus(surl, status));
        }
        return surlReturnStatuses;
    }
    
    public static void printRequestOutcome(TReturnStatus status, InputData inputData, String srmCommand)
    {
        if (inputData != null)
        {
            if (inputData instanceof ManageFileTransferRequestFilesInputData)
