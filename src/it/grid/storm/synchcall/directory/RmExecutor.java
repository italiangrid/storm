...
class RmExecutor
{
     ...
     */
    public RmOutputData doit(RmInputData inputData)
    {
        ...
        /** 
         *  Check if GridUser in RMInputData is not null, 
            ...
            for (int i = 0; i < numberOfFiles; i++) {
                ...
                if (surl.isEmpty()) {
                    ...
                    try {
                    	...
                    	log.error("srmRm: <"+user+"> Removing SURL "+i+" of "+numberOfFiles+" [SURL:] failed. Error creating returnStatus " + ex1);
                    }
                    failure = true;
                } else {
                    //Creation of StoRI
                ...
                if (!failure) {
                    ...
                    if ((deleteAuth != null) && (deleteAuth.isPermit())) {
                        ...
                        if (returnStatus.getStatusCode() == TStatusCode.SRM_SUCCESS) {
                        	globalFailure = false;
                        	log.info("srmRm: <"+user+"> Removing SURL "+i+" of "+numberOfFiles+" [SURL:] successfully done with [status: "+ returnStatus.toString()+"]");
                        } else
                        	partialSuccess = true;
                        
                        fileStatus.setStatus(returnStatus);
                        
                        log.debug("srmRm:  of (" + stori.getPFN() + ") authorized was done. File Status is : " + returnStatus);
                        failure = false;
                    } else {
                        failure = true;
        ...
        try {
            ...
            if(globalStatus.getStatusCode().equals(TStatusCode.SRM_SUCCESS))
            	log.info("srmRm: <"+user+"> Request for SURLs [SURL:"+surlArray+"] successfully done with [status: "+ globalStatus.toString()+"]");
            else
            	log.error("srmRm: <"+user+"> Request for SURLs [SURL:"+surlArray+"] failed with [status: "+ globalStatus.toString()+"]");
        } catch (InvalidTReturnStatusAttributeException e) {
        	log.error("srmRm: <"+user+"> Request for SURLs [SURL:"+surlArray+"] failed. Error creating returnStatus " + e);
        }
    ...
    private TReturnStatus manageAuthorizedRM(LocalUser lUser, TSURL surl, StoRI stori)
    {
        ...
        if (!(file.exists())) {
            ...
            /**
             * If there are SrmPrepareToPut active on the SURL specified
             * change the SRM_STATUS from SRM_SPACE_AVAILABLE to SRM_ABORTED
             */
            putcatalog.transitSRM_SPACE_AVAILABLEtoSRM_ABORTED(surl, "File Removed by a SrmRm()");
            
            /**
             * If there are SrmPrepareToGet active on the SURL specified
             * change the SRM_STATUS from SRM_FILE_PINNED to SRM_ABORTED
             */
            //getcatalog.transitSRM_FILE_PINNEDtoSRM_ABORTED(surl,"File Removed by a SrmRm()");
            ...
            if (!(fileRemoved)) {
                ...
                /**
                 * @todo: Remove file entry from Persistence
                 * Check if the specified SURL is associated to a certain space Token, 
    ...
    private boolean removeTarget(LocalFile file, LocalUser lUser)
    {
        ...
        /** Same situation in Rmdir
         * This check is really needed here? 
         * The check can not be done at this level.
         * In Jit no ACE on file are setted!!!
         * In any case add check for null permission.
         */
