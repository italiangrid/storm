/**
 * This class represents the GetSpaceMetaDataManager Class.
 * This class hava a reseveSpace method that perform all operation nedded to satisfy
 * a SRM space release request.
 *
 * @author  Magnoni Luca
...
public class GetSpaceMetaDataExecutor {
  ...
  /**
   * Constructor.
   * Bind the Executor with ReservedSpaceCatalog
   */
  public GetSpaceMetaDataExecutor() {
   ...
   */
  public GetSpaceMetaDataOutputData doIt(GetSpaceMetaDataInputData data) {
    ...
    for (int i = 0; i < sizeBulkRequest; i++) {
      ...
      if (metadata == null) { // There are some problems...
        ...
        if (metadata.getSpaceType().equals(TSpaceType.VOSPACE)) {
          ...
          log.debug("srmGetSpaceMetaData: Space Token '" + tk + "' is pointing to a STATIC space reservation.");
          ...
          try {
            ...
            try {
              ...
              log.error("srmGetSpaceMetaData: freeSpace ("+freeSpace+" KBytes) returned by Quota is wrong.");
              log.error("srmGetSpaceMetaData: QuotaInfo returned was: "+quotaInfo);
            }

            //Setting TOTAL SIZE
            try {
              TSizeInBytes totalSize = (TSizeInBytes.make(quotaInfo.getBlockSoftLimit(), quotaInfo.getSizeUnit() ));
              ...
              log.error("srmGetSpaceMetaData: TotalSize ("+quotaInfo.getBlockSoftLimit()+" KBytes) returned by Quota is wrong.");
              log.error("srmGetSpaceMetaData: QuotaInfo returned was: "+quotaInfo);
            }
            ...
            try {
              ...
              log.error("srmGetSpaceMetaData: GuaranteedSize ("+quotaInfo.getBlockSoftLimit()+" KBytes) returned by Quota is wrong.");
              log.error("srmGetSpaceMetaData: QuotaInfo returned was: "+quotaInfo);
            }
          ...
          // Into the TMetaDataSpace constructor (from the SpaceData object retrieved from catalog)
          // is setted the correct status for the request.
          // In case of lifetime expired , SRM_SPACE_LIFETIME_EXPIRED is setted,
    ...
    try {
      if (requestSuccess) {
        ...
        log.info(formatLogMessage(SUCCESS, GLOBALSTATUS, data.getUser(), null, data.getSpaceTokenArray(), globalStatus));
      }
      else if (requestFailure) {
        globalStatus = new TReturnStatus(TStatusCode.SRM_FAILURE, "No valid space tokens");
        log.error(formatLogMessage(FAILURE, GLOBALSTATUS, data.getUser(), null, data.getSpaceTokenArray(), globalStatus));
      }
      else {
        globalStatus = new TReturnStatus(TStatusCode.SRM_PARTIAL_SUCCESS, "Check space tokens statuses for details");
        log.error(formatLogMessage(SUCCESS, GLOBALSTATUS, data.getUser(), null, data.getSpaceTokenArray(), globalStatus));
      }
    }
  ...
  private QuotaInfoInterface retrieveInfoFromQuota(TSpaceToken token) throws QuotaException {
    ...
    if (! (fsType.toLowerCase().equals("gpfs"))) {
      ...
      log.warn("srmGetSpaceMetaData: Quota command is enabled only on GPFS filesystem." +
      ...
      switch (quotaT) {
        case 0 : param1 = "-j "+quota.getQuotaID().getValue(); break;
        default : throw new QuotaException("Unable to execute a quota command because Quota Type '"+QuotaType.string(quotaT)+"' is not supported");
      }
   ...
   * @param globalStatus boolean
   * @param user GridUserInterface
   * @param token TSpaceToken
   * @param arrayOfToken ArrayOfTSpaceToken
