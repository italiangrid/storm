...
public class TFN {
    ...
    private boolean empty = true; //boolean indicating whether this is an empty TFN
     ...
     * that Machine, and the PhysicalFileName pfn. An InvalidTFNAttributesException
     * is thrown if any is null or empty.
     */
    public static TFN make(Machine m, Port p, PFN pfn) throws InvalidTFNAttributesException {
	  if ((m==null) || (p==null) || (pfn==null) || m.isEmpty() ||pfn.isEmpty()) throw new InvalidTFNAttributesException(m,p,pfn);
	  return new TFN(m,p,pfn,false);
    }

    /**
     * Static method that returns a TFN with the specified PFN, and EmptyMachine as
     * well as EmptyPort. The intended use of this method is to create TFN to be used
     * in TURLs with FILE protocol, where no machine and port are needed. An
     * InvalidTFNAttributesException is thrown if pfn is null or empty: notice that
     * the exception will also show Machine and Port as null.
     */
    ...
    /**
     * Static method that returns a TFN from a String representation. If the supplied String is
     * null or malformed, an InvalidTFNAttributesException is thrown.
     */
    public static TFN makeFromString(String s) throws InvalidTFNAttributesException {
        ...
        if (colon==-1) {
            ...
            if ((slash==-1) || (slash==0)) throw new InvalidTFNAttributesException(null,null,null); //no colon and no slash or slash right at the beginning!
            //machine
            ...
            //colon follows existing slash: the colon does NOT stand as port number delimiter
            //treat it as missing port specification
            //machine
            if (slash==0) throw new InvalidTFNAttributesException(null,null,null); //slash right at the beginning!
            String mString = s.substring(0,slash);
            Machine m = null;
            try {
                m = Machine.make(mString);
            ...
            //machine
            if (colon==0) throw new InvalidTFNAttributesException(null,null,null); //colon right at the beginning!
    ...
    /**
     * Method that returns the Machine specified in this TFN. If this is an
     * empty TFN, then an empty Machine is returned.
     */
    ...
    public boolean equals(Object o) {
        ...
        return (!empty) && (!tfno.empty) && m.equals(tfno.m) && p.equals(tfno.p) && pfn.equals(tfno.pfn);
    }
...
/*
    public static void main(String[] args) {
        //
        //Testing empty TFN
        System.out.println("Testing empty TFN objects...");
        TFN te1 = TFN.makeEmpty(); System.out.println("te1 is an empty TFN; should see Empty TFN:"+te1+"; should see hashCode 0:"+te1.hashCode()+"; it is empty so should see true:"+te1.isEmpty());
        TFN te2 = TFN.makeEmpty(); System.out.println("te2 is an empty TFN; should see Empty TFN:"+te2+"; should see hashCode 0:"+te2.hashCode()+"; it is empty so should see true:"+te2.isEmpty());
        System.out.println("te1.equals(te2) should see true:"+ te1.equals(te2)+"; te2.equals(te1) should see true:"+te2.equals(te1));
        System.out.println("te1 should have all empty parts - Machine: "+te1.machine()+", Port: "+te1.port()+", PFN: "+te1.pfn());
        //
        ...
        try {
            ...
            TFN tfn1 = TFN.make(m1,p1,pfn1); System.out.println("TFN 1 - should see "+ tfn1s +":"+tfn1+"; hashCode:"+tfn1.hashCode()+"; isEmpty should be false:"+tfn1.isEmpty());
            TFN tfn2 = TFN.make(m2,p2,pfn2); System.out.println("TFN 2 - should see "+ tfn2s +":"+tfn2+"; hashCode:"+tfn2.hashCode()+"; isEmpty should be false:"+tfn2.isEmpty());
            ...
            System.out.println("tfn1.equals(Object) false: "+tfn1.equals(new Object()));
            System.out.println("tfn1.equals(empty) false: "+tfn1.equals(TFN.makeEmpty()));
            System.out.println("empty.equals(tfn1) false: "+TFN.makeEmpty().equals(tfn1));
            System.out.println("tfn1 is "+tfn1+" - Machine: "+tfn1.machine()+", Port: "+tfn1.port()+", PFN: "+tfn1.pfn());

            TFN tfn3 = TFN.makeByPFN(pfn3); System.out.println("\nTFN 3 - should see "+ tfn3s +":"+tfn3+"; hashCode:"+tfn3.hashCode()+"; isEmpty should be false:"+tfn3.isEmpty());
            ...
            System.out.println("tfn3.equals(Object) false: "+tfn3.equals(new Object()));
            System.out.println("tfn3.equals(empty) false: "+tfn3.equals(TFN.makeEmpty()));
            System.out.println("empty.equals(tfn3) false: "+TFN.makeEmpty().equals(tfn3));
            System.out.println("tfn3 is "+tfn3+" - Machine: "+tfn3.machine()+", Port: "+tfn3.port()+", PFN: "+tfn3.pfn());
        ...
        try {
            ...
            PFN pfn1 = PFN.make(pfn1s);
            String tfn1s = m1s+":"+p1i+pfn1s;
            TFN tfn1 = TFN.make(m1,p1,pfn1); System.out.println("Successfully created "+tfn1);

            System.out.print("Now attempting creation with null Machine: ");
            try {
                TFN.make(null,p1,pfn1);
                System.out.println("Should not see this!");
            ...
            System.out.print("Now attempting creation with null PFN: ");
            try {
                TFN.make(m1,p1,null);
                System.out.println("Should not see this!");
            } catch (InvalidTFNAttributesException e) {
            ...
            System.out.print("Now attempting creation with empty PFN: ");
            try {
