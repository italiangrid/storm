...
public class StFN {
    ...
    private boolean startingSlash = false;
    ...
    /**
     * Public static method that requires a String representing the pathname of the SFN:
     * it cannot be null or empty otherwise an InvalidStFNAttributeException is
     * thrown. Likewise if it contains two consecutive dots (..). or does not begin
     * with a slash (/).
     */
    public static StFN make(String name) throws InvalidStFNAttributeException {
        if (invalid(name)) throw new InvalidStFNAttributeException(name);
        return new StFN(normalize(name),/* name.charAt(0)=='/',*/ false, checkDirectory(name));
    }

    /**
     * Public static method that returns true if the supplied String ends with the Separator,
     * thereby indicating a directory.
     */
     ...
     * contains two consecutive dots (..), or does not begin with a slash (/).
     */
     ...
     * example clarifies what is meant by parent StFNs, and by stopping at root parent.
     *
     * Original StFN:
     *       /EGRID/original/data/nyse/file.txt
     * Parent StFNs:
     *       /EGRID/original/data/nyse
     *       /EGRID/original/data
     *       /EGRID/original
     *       /EGRID
     *
     ...
     *       Empty collection!
     *
     * An empty collection is returned if any error occurs during creation of parent StFNs.
     * Likewise if This is an EmptyStFN.
     */
    public Collection getParents() {
        Collection aux = new ArrayList();
        if (this.empty) return aux; //empty StFN!
        int size = this.name.size(); //number of elements in this StFN
        if ((size==0) || (size==1)) return aux; //StFN directly on root, or with only _one_ element!
        for (int i=1; i<size; i++) {
            aux.add(new StFN(new ArrayList(this.name.subList(0,i))/*,this.startingSlash*/,false, true)); //recall sublist goes from 0 inclusive, to i _EXCLUDED_!!!
        }
        return aux;
    }

    /**
     * Method that returns the parent StFN. The following example clarifies
     * what is meant by parent StFN. Beware of the root of the StFN: the parent
     * is calculated from the root!
     *
     * Original StFN:
     *       /EGRID/original/data/nyse/file.txt
     * Parent StFN:
     ...
     *       Empty StFN!
     *
     * An empty StFN is returned if any error occurs during creation of parent.
     * Likewise if This is an EmptyStFN.
     */
    public StFN getParent() {
        if (this.empty) return makeEmpty(); //empty StFN!
        int size = this.name.size(); //number of elements in this StFN
        if ((size==0) || (size==1)) return makeEmpty(); //either directly on root, or only one element!
        return new StFN(new ArrayList(this.name.subList(0,size-1))/*,this.startingSlash*/,false, true);
    }
    ...
    public String toString() {
        if (empty) return "Empty StFN";
        int size = this.name.size();
        if (size==0) return "/"; //This is not an Empty StFN because empty==false; yet its elements List is empty: it can only be the root StFN! That is the "/"!!!
        ...
        if (this.directory) sb.append(NamingConst.SEPARATOR);
        return sb.toString();
    }

    public boolean equals(Object o) {
        if (o==this) return true;
        if (!(o instanceof StFN)) return false;
        StFN po = (StFN) o;
        if (po.empty && empty) return true;
        if ((!empty) && (!po.empty) && (name.size()==0) && (po.name.size()==0)) return true;
        return (!empty) && (!po.empty) /*&& (startingSlash==po.startingSlash)*/ && (directory==po.directory) && name.equals(po.name);
    }

    public int hashCode() {
        if (empty) return 0;
        int hash = 17;
        if (name.size()!=0) hash = 37*hash + name.hashCode();
        hash = 37*hash + (directory?1:0);
