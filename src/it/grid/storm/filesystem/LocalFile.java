...
/**
  ...
  From {@link java.io.File} the following methods are inherited:
  {@link java.io.File#delete() delete}, {@link java.io.File#exists()
  exists}, {@link java.io.File#getAbsolutePath() getAbsolutePath},
  {@link java.io.File#getName() getName}, {@link
  java.io.File#isDirectory() isDirectory}, {@link
  java.io.File#isFile() isFile}, {@link java.io.File#list() list},
  {@link java.io.File#mkdir() mkdir}, {@link java.io.File#mkdirs()
  mkdirs}; the methods {@link java.io.File#getParentFile()
  getParentFile} and {@link java.io.File#listFiles() listFiles} have
  been overridden to return {@link it.grid.storm.filesystem.File}
  objects instead of {@link java.io.File} ones.

  Additional methods are provided to manipulate this file's ACL in a
  filesystem-indepenent way; all ACL operations take a LocalUser
  identity (see {@link it.grid.storm.griduser.LocalUser}) and a
  {@link FilesystemPermission} permission representation.

  A pathname need not exist for an associated
  it.grid.storm.filesystem.File object to be created; indeed, you
  should create an instance associated to a non existing pathname and
  then invoke {@link #create()} or {@link #makeDirectory()} to create
  a file or a directory, respectively.

  The constructor for this class requires a pathname and a {@link
  Filesystem} object.  The passed {@link Filesystem} object need to
  match the filesystem-type of the filesystem the passed pathname
  resides on.  Therefore, {@link it.grid.storm.filesystem.File}
  objects can only be created from a factory method that has all the
  required pieces of information; {@link
  it.grid.storm.namespace.StoRI} looks like the right class for that.
...
public class LocalFile {
    ...
    /**
     * Constructor, taking string pathname and the hosting filesystem
     * object.  The <code>path</code> parameter must be a non-empty
     * absolute pathname.
     *
     * @param pathname  The pathname wrapped by this PFN; <em>must</em>
     *                  be absolute, or code will fail in an assertion.
     *
     * @param fs        The {@link it.grid.storm.filesystem.Filesystem}
     *                  object to use for operations on this file.
     *
    ...
    /**
     * Constructor, taking parent pathname (as a <code>String</code>),
     * child name (as a <code>String</code>) and the hosting
     * filesystem object.  From <i>parent</i> and <i>child</i> an
     * absolute pathname must result, following the rules in {@link
    ...
    /**
     * Constructor, taking parent pathname (as a {@link File}),
     * child name (as a <code>String</code>) and the hosting
     * filesystem object.  From <i>parent</i> and <i>child</i> an
     * absolute pathname must result, following the rules in {@link
     * java.io.File#File(java.io.File,String)}.
     *
     ...
     */
    public LocalFile(final LocalFile parent, final String name, final Filesystem fs) throws NullPointerException {
    ...
    /**
     * Return <code>true</code> if the parent directory of this
     * pathname exists.
     */
    ...
    /**
     * Method that creates a new empty file, as per contract of java.io.File: refer there
     * for further info.
     */
    ...
    /**
     * Method that creates a new directory, as per contract of java.io.File: refer there
     * for further info.
     */
    ...
    /**
     * Method that renames This file, as per contract of java.io.File: refer there for
     * further info. The only notable difference is that this method requires a String
     * rather than java.io.File parameter.
     */
    ...
    /**
     * Method that returns the size in bytes of This file, as per contract of java.io.File:
    ...
    /**
     * Return the (possibly outdated) size in bytes of this file.  Same
     * as {@link #length()}, although this uses the host Filesystem
     * native <code>stat()</code>-like calls.
     *
     * May return inaccurate results, as some filesystems (notably
     * GPFS) provide the choice between filesystem-specific calls
     * for accurate reporting (slower, as it may imply synchronizing
     * the metadata cache) or standard libc calls that report possibly
    ...
    /** Return (possibly cached) file last modification time, as a
     * UNIX epoch.  Same as {@link #lastModified()}, although this
     * uses the host Filesystem native <code>stat()</code>-like calls.
     *
     * May return inaccurate results, as some filesystems (notably
     * GPFS) provide the choice between filesystem-specific calls
     * for accurate reporting (slower, as it may imply synchronizing
     ...
     * outdated information.
     *
     * @return time (seconds since the epoch) this file was last
     * modified.
     *
     * @see #lastModified()
    ...
    /** Return up-to-date file size in bytes.  Returned value may differ
    ...
    /** Return up-to-date file last modification time, as a UNIX epoch.
    ...
    /**
     * Return <code>true</code> if the local user (to which the
     * specified grid user is mapped to) can operate on the specified
     * <code>fileOrDirectory</code> in the mode given by
     * <code>accessMode</code>, according to the permissions set on
     * the filesystem.
     */
    ...
    /**
     * Return the <em>effective</em> permission a group has on this
     * file.
     *
     * Loads the ACL for this file or directory, and return the
     * permission associated with the local account primary group of
     * the given {@link LocalUser} instance <i>u</i>.  If no ACE
     * for that group is found, return {@link Filesystem#NONE}.
     *
     * @param u the LocalUser whose local account
     * primary GID's permissions are to be retrieved.
     *
     * @return <em>effective</em> permission associated to the local
     * account primary GID of the given LocalUser <i>u</i> in the
     * given file ACL, or <code>null</code> if no ACL entry for that
     * group was found.
     */
    ...
    /**
     * Return the <em>effective</em> permission a user has on this
     * file.
     *
     * Loads the ACL for this file or directory, and return the
     * permission associated with the local account UID of the given
     * LocalUser <i>u</i>.  If no ACE for that user is found, return
     * {@link Filesystem#NONE}.
     *
     * @param u the LocalUser whose permissions
     * are to be retrieved.
     *
     * @return <em>effective</em> permission associated to the local
     * account UID of the given LocalUser <i>u</i> in this file ACL,
     * or <code>null</code> if no ACL entry for that user was found.
     */
     ...
     * primary GID's permissions are to be retrieved.
     *
     * @return permission associated to the local account primary GID
     * of the given LocalUser <i>u</i> in the given
     * file ACL. or {@link Filesystem#NONE} if no ACE for that
     * group was found.
     */
     ...
     * are to be retrieved.
     *
     * @return permission associated to the local account UID of the
     * given LocalUser <i>u</i> in this file ACL, or {@link
     * Filesystem#NONE} if no ACE for that user was found.
     */
    ...
    /**
     * Grant specified permission to a group, and return the former
     * permission.
     *
     * <p>Adds the specified permission to the ones that the primary
     * group of the given LocalUser <i>u</i> already
     * holds on this file or directory: all permission
     * bits that are set in <i>permission</i> will be set in the
     * appropriate group ACE in the file ACL.
     *
     * <p>If no ACE is present for the specified group, then one is
     * created and its permission value is set to <i>permission</i>.
     *
     * @param u the LocalUser whose local account primary GID's
     * ACE is to be altered.
     *
     * @param permission Capabilities to grant.
     *
     * @return permission formerly associated to the local account
     * primary GID of the given LocalUser <i>u</i> in this file
     * ACL, or {@link Filesystem#NONE} if no ACE for that
     * group was found.
     */
    public FilesystemPermission grantGroupPermission(final LocalUser u, final FilesystemPermission permission) throws CannotMapUserException {
        return fs.grantGroupPermission(u, localFile.getAbsolutePath(), permission);
    }

    /**
     * Grant specified permission to a user, and return the former
     * permission.
     *
     * <p>Adds the specified permissions to the ones that the
     * local account UID of the given LocalUser <i>u</i> already
     * holds on this file or directory: all permission
     * bits that are set in <i>permission</i> will be set in the
     * appropriate user ACE in the file ACL.
     *
     * <p>If no ACE is present for the specified user, then one is
     * created and its permission value is set to <i>permission</i>.
     *
     * @param u the LocalUser whose local account UID's ACE is
     * to be altered.
     *
     * @param permission Capabilities to grant.
     *
     * @return permission formerly associated to the local account UID
     * of the given LocalUser <i>u</i> in this file ACL, or
     * {@link Filesystem#NONE} if no ACE for that user was
     * found.
     */
    public FilesystemPermission grantUserPermission(final LocalUser u, final FilesystemPermission permission) throws CannotMapUserException {
     ...
     *
     * <p>Removes the ACE (if any) of the primary group of the given
     * LocalUser <i>u</i> from this file or directory ACL.
     * Returns the permission formerly associated with that group.
     *
     * <p>If the given group is the file owning group, then its ACE is
     * set to {@link Filesystem#NONE}, rather than removed.
     *
     * @param u the LocalUser whose local account primary GID's
     * ACE is to be altered.
     *
     ...
     *
     * <p>Removes the ACE (if any) of the primary user of the given
     * LocalUser <i>u</i> from this file or directory ACL.
     * Returns the permission formerly associated with that user.
     *
     * <p>If the given user is the file owner, then its ACE is
     * set to {@link Filesystem#NONE}, rather than removed.
     *
     * @param u the LocalUser whose local account UID's
     * ACE is to be altered.
     *
     * @return permission formerly associated to the local account UID
     * of the given LocalUser <i>u</i> in this file ACL, or
    ...
    /**
     * Revoke specified permission from a group's ACE, and return the
     * former permission.
     *
     * <p>Removes the specified permission from the ones that the
     * primary group of the given LocalUser <i>u</i> local account
     * already holds on this file or directory: all permission
     * bits that are <em>set</em> in <i>permission</i> will be
     * <em>cleared</em> in the appropriate group ACE in the file ACL.
     *
     * <p>If no ACE is present for the specified group, then one is
     * created and its permission value is set to {@link Filesystem#NONE}.
     *
     * @param u the LocalUser whose local account primary GID's
     * ACE is to be altered.
     *
    ...
    /**
     * Revoke specified permission from a user's ACE, and return the
     * former permission.
     *
     * <p>Removes the specified permission from the ones that the
     * primary user of the given LocalUser <i>u</i> local account
     * already holds on this file or directory: all permission
     * bits that are <em>set</em> in <i>permission</i> will be
     * <em>cleared</em> in the appropriate user ACE in the file ACL.
     *
     * <p>If no ACE is present for the specified user, then one is
     * created and its permission value is set to {@link
     * Filesystem#NONE}.
     *
     * @param u the LocalUser whose local account UID's
     * ACE is to be altered.
    ...
    /**
     * Set the specified permission in a group's ACE, and return the
     * former permission.
     *
     * <p>Sets the ACE of the primary group of the given LocalUser
     * <i>u</i> to the given <i>permission</i>.  Returns the
     * permission formerly associated with that group.
     *
     * @param u the localUser whose local account primary GID's ACE is
     * to be altered.
     *
    ...
    /**
     * Set the specified permission in a user's ACE on a file or
     * directory, and return the former permission.
     *
     * <p>Sets the ACE of the primary user of the given LocalUser
     * <i>u</i> to the given <i>permission</i>.  Returns the
     * permission formerly associated with that user.
     *
     * @param u the Grid user whose local account UID's
     * ACE is to be altered.
     *
