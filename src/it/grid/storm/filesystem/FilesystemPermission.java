...
/**
 * Provides an abstraction of all operations that can be performed on
 * a filesystem entry (file or directory).
 *
 * <p>Note: this class is an interface to the fs_acl::permission_t
 * type; if the low-level fs_acl::permission_t type ever gets
 * modified, then the {@link toFsAclPermission()} method should be
 * modified also.
 *
 * <p>To all effects, instances of this class are <em>immutable</em>.
 * Permissions are read off or enforced onto disk files; they should
 * not be altered by StoRM code.  Still, if there is need for
 * permission manipulation in StoRM, a MutableFilesystemPermission
 * derived class can be provided, that promotes to public visibility
 * the deny*() and permit*() methods.
 *
 *
...
public class FilesystemPermission implements java.io.Serializable
{
    ...
    public final static FilesystemPermission Read =
        new FilesystemPermission(READ_DATA);
    ...
    /** Permission to list directory contents. */
    public final static FilesystemPermission List =
        new FilesystemPermission(LIST_DIRECTORY);

    /** Permission to traverse directory (descend path where directory
     * is an intermediate step). */
    public final static FilesystemPermission Traverse =
        new FilesystemPermission(TRAVERSE_DIRECTORY);
    ...
    /** Permission to write. */
    public final static FilesystemPermission Write = new FilesystemPermission(FilesystemPermission.WRITE_DATA);
     ...
     */
    public final static FilesystemPermission CreateSubdirectory = new FilesystemPermission(FilesystemPermission.CREATE_SUBDIRECTORY);

    /**
     * Permission to delete file or directory
     */
    public final static FilesystemPermission Delete = new FilesystemPermission(FilesystemPermission.DELETE);

    /**
     *
     */
    public final static FilesystemPermission ListDirectory = new FilesystemPermission(FilesystemPermission.LIST_DIRECTORY);

    // --- constructors --- //

    /**
     * Copy constructor.  Takes another instance of the {@link
     * it.grid.storm.filesyste.FilesystemPermission} interface and
     * creates an instance of this class granting <em>exactly</em> the
     * same permissions.
     */
    ...
    /**
     * Constructor that takes a bitfield of permissions and creates an
     * instance of this class granting exactly <em>those</em>
     * permissions.  The bitfield argument has the same format of the
     * fs_acl::permission_t type, or could be constructed by
     * bitwise-OR'ing the <code>READ_DATA</code>,
     * <code>WRITE_DATA</code>, ... constants defined elsewhere in
     * this class.  For any bit that is set in the
     * <code>bitfield</code> argument, the corresponding permission
    ...
    /**
     * Default constructor: creates an instance that denies permission
     * on each and every operation.
     *
     * <p>This constructor's intended usage is in conjunction with the
     * permission manipulation functions (in derived classes):
     * <pre>
    ...
    /** Return an fs_acl::permission_t bitfield representing the same
     * permissions that this object encodes.
     *
     * @see fs_acl::permission_t
     */
    ...
    /** Return <code>true</code> if permission is granted to read file
     * contents.
     */
    public boolean canReadFile() { return 0 != (permission & READ_DATA); }

    /** Return <code>true</code> if permission is granted to write
     * file contents. No distinction can be enforced between
     * overwriting contents and appending to the file, so no
     * distinction is made here.
     */
    public boolean canWriteFile() { return 0 != (permission & WRITE_DATA); }

    /** Return <code>true</code> if permission is granted to list
     * directory contents.
     */
    public boolean canListDirectory() { return 0 != (permission & LIST_DIRECTORY); }

    /** Return <code>true</code> if permission is granted to descend
     * to a subdirectory.
     */
    public boolean canTraverseDirectory() { return 0 != (permission & TRAVERSE_DIRECTORY); }

    /** Return <code>true</code> if permission is granted to create a
     * new subdirectory.
     */
    public boolean canMakeDirectory() { return 0 != (permission & CREATE_SUBDIRECTORY); }

    /** Return <code>true</code> if permission is granted to create a
     * new file.
     */
    public boolean canCreateNewFile() { return 0 != (permission & CREATE_FILE); }

    /** Return <code>true</code> if permission is granted to change
     * filesystem entry (file or directory) ACL.
     */
    public boolean canChangeAcl() { return 0 != (permission & WRITE_ACL); }

    /** Return <code>true</code> if permission is granted to delete
     * entry (file or directory).
     */
    public boolean canDelete() { return 0 != (permission & DELETE); }


    /**
     * Return <code>true</code> if all permissions that are granted by
     * <i>other</i> FilesystemPermission instance are also granted by
     * this instance.  That is, test if <i>other</i> is more
     * restrictive than the this instance.
     */
     ...
     * <i>bitfield</i> are also set in this instance.  That is, test
     * if <i>bitfield</i> represents a more restrictive than the this
     * instance.
     */
     ...
     * <code>can...</code> calls will return <code>false</code>.
     *
     * <p>Returns the instance itself, so that calls to the permission
     * manipulation functions can be chained:
     * <pre>
     *   p = new FilesystemPermission();
     *   p.denyAll()
     *    .permitReadFile()
    ...
    /**
     * Change instance status so that all subsequent
     * <code>can...</code> calls will return <code>true</code>.
     * <em>Dangerous</em>, use with caution.
     *
     * <p>Returns the instance itself, so that calls to the permission
     * manipulation functions can be chained:
     * <pre>
    ...
    /** The permission set bitfield.  Must match the type and
     * representation used in fs_acl::permission_t: no conversion is
     * done, the code just assumes that it can pass the value back and
     * forth from Java to C++. */
