...
#include "../h/it_grid_storm_wrapper_SpaceReservationWrapper.h"
...
//	vm_args.ignoreUnrecognized = JNI_FALSE;
	...
	*/
	...
	if(DEBUG) cout<<"<::SpaceReservationLibrary> FileName: "<<file<<" in Path: "<<path<<" with delim pos: "<<file.rfind(delim,file.length())<<endl;
	...
	if(DEBUG) cout<<"<::SpaceReservationLibrary> : FileSystemType: "<<fp.f_type<<endl;
	...
	//{
		if(DEBUG) cout<<"<::SpaceReservationLibrary>: GPFS File System Identified!"<<endl;
		GPFSWrapper gpfs;
		return(gpfs.spaceAlloc(path,file,size));
	}
	...
	{
		if(DEBUG) cout<<"<::SpaceReservationLibrary>: LUSTRE File System identified!"<<endl;
		LustreWrapper lustre;
		return(	lustre.spaceAlloc(path,file,size));
	}
#endif
...
{
		if(DEBUG) cout<<"<::SpaceRerservationLibrary>: EXT3, EXT2... File System identified!"<<endl;
		PosixWrapper fs;
		return(	fs.spaceAlloc(path,file,size));
	}
